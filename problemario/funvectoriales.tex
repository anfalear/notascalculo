...
\begin{definition}[Función vectorial]\index{funcion vectorial@función vectorial}
...
\end{axis}
\end{tikzpicture}
\caption{Hélice $\mathbf{r}(t)=(\cos t,\sen t,\tfrac{t}{2})$,
         $t\in[0,2\pi]$, con vector tangente en $t=\tfrac{\pi}{2}$}
\label{fig:helice_tangente}
\end{figure}

\begin{prob}[Curva intersección de un cilindro y un plano]
Parametrice la curva $C$ de intersección del cilindro $x^2+y^2=4$ con el
plano $y+z=3$, y halle la ecuación de la recta tangente a $C$ en el punto
$P=(\sqrt{3},\,1,\,2)$.
\begin{myproof}
\textbf{Paso 1: Parametrización de $C$.}
Los puntos del cilindro $x^2+y^2=4$ (radio $2$) se describen con
$x=2\cos t$, $y=2\sen t$. La coordenada $z$ queda fijada por el plano:
$z=3-y=3-2\sen t$. Por tanto
\[
  \mathbf{r}(t)=\langle 2\cos t,\; 2\sen t,\; 3-2\sen t\rangle,
  \qquad t\in[0,2\pi).
\]

\textbf{Paso 2: Parámetro del punto $P$.}
Igualando $\mathbf{r}(t_0)=P$:
\[
  2\cos t_0=\sqrt{3}\ \Rightarrow\ \cos t_0=\tfrac{\sqrt3}{2},
  \qquad
  2\sen t_0=1\ \Rightarrow\ \sen t_0=\tfrac12
  \quad\Longrightarrow\quad t_0=\frac{\pi}{6}.
\]
La tercera componente es consistente: $z=3-2\cdot\tfrac12=2$. \checkmark

\textbf{Paso 3: Vector tangente.}
Derivando componente a componente,
\[
  \mathbf{r}'(t)=\langle -2\sen t,\; 2\cos t,\; -2\cos t\rangle,
\]
y en $t_0=\dfrac{\pi}{6}$,
\[
  \mathbf{r}'\!\left(\tfrac{\pi}{6}\right)
  =\big\langle -2\cdot\tfrac12,\; 2\cdot\tfrac{\sqrt3}{2},\;
              -2\cdot\tfrac{\sqrt3}{2}\big\rangle
  =\langle -1,\; \sqrt{3},\; -\sqrt{3}\rangle.
\]

\textbf{Paso 4: Recta tangente.}
Pasa por $P=(\sqrt3,1,2)$ con vector director
$\mathbf{r}'(\tfrac{\pi}{6})$:
\[
  \mathbf{L}(s)=P+s\,\mathbf{r}'\!\left(\tfrac{\pi}{6}\right)
              =\langle \sqrt{3}-s,\; 1+\sqrt{3}\,s,\; 2-\sqrt{3}\,s\rangle,
  \qquad s\in\mathbb{R}.
\]
\[
  \boxed{\ \mathbf{r}(t)=\langle 2\cos t,\, 2\sen t,\, 3-2\sen t\rangle,\qquad
  \mathbf{L}(s)=\langle \sqrt3-s,\, 1+\sqrt3\,s,\, 2-\sqrt3\,s\rangle.\ }
\]
\end{myproof}
\end{prob}

\begin{rem}
La parametrización trigonométrica recorre la curva \emph{completa} sin
ambigüedad. Si en su lugar se despejara $y=\pm\sqrt{4-x^2}$, cada signo
describiría solo media curva (\emph{dos cartas}) y el punto de empalme
$x=\pm2$ tendría tangente vertical, donde $dy/dx$ no está definida; el
parámetro $t$ evita ese problema.
\end{rem}
...
\begin{axis}[axis lines=center, xlabel=$x$, ylabel=$y$, zlabel=$z$,
             xmin=-2.6,xmax=2.6,ymin=-2.6,ymax=2.6,zmin=0,zmax=5.2,
             view={120}{18}, ticklabel style={font=\tiny},
             width=8.5cm, height=8cm,
             trig format plots=deg]
  % Plano y+z=3 (fill translucido)
  \fill[green!35!black, opacity=0.22]
    (axis cs:-2.5,-2,5) -- (axis cs:2.5,-2,5) --
    (axis cs:2.5,2,1)  -- (axis cs:-2.5,2,1) -- cycle;
  % Cilindro x^2+y^2=4
  \addplot3[surf, shader=flat corner, samples=20,
            colormap/cool, opacity=0.30,
            variable=\u, variable y=\v,
            domain=0:360, y domain=0:5]
           ({2*cos(\u)}, {2*sin(\u)}, {\v});
  % Curva interseccion C
  \addplot3[very thick, red!70!black, samples=60, samples y=0,
            domain=0:360]
           ({2*cos(x)}, {2*sin(x)}, {3-2*sin(x)});
  % Punto P y vector tangente
  \addplot3[only marks, mark=*, mark size=1.6pt, black]
           coordinates {(1.732,1,2)};
  \draw[->, very thick, red!70!black]
    (axis cs:1.732,1,2) -- (axis cs:0.732,2.732,0.268);
  \node[anchor=south west, font=\scriptsize] at (axis cs:1.732,1,2.1) {$P$};
  \node[anchor=west, font=\scriptsize, red!70!black]
    at (axis cs:0.732,2.732,0.268) {$\mathbf{r}'(\tfrac{\pi}{6})$};
\end{axis}
\end{tikzpicture}
\caption{Curva $C$ (roja) de intersección del cilindro $x^2+y^2=4$ con el
  plano $y+z=3$ (verde), y la recta tangente en $P=(\sqrt3,1,2)$ dirigida por
  $\mathbf{r}'(\tfrac{\pi}{6})=\langle -1,\sqrt3,-\sqrt3\rangle$.}
\label{fig:cilindro_plano_interseccion}
\end{figure}
...
\begin{definition}[Límite de una función vectorial]\index{funcion vectorial@función vectorial!limite@límite}
...
\begin{definition}[Continuidad de una función vectorial]\index{funcion vectorial@función vectorial!continuidad}
...
\begin{theorem}[Continuidad componente a componente]\index{funcion vectorial@función vectorial!continuidad componente a componente}
Una función vectorial $\mathbf{r}(t) = \langle f(t), g(t), h(t) \rangle$
...
\begin{rem}[Dominio de una función vectorial]
El dominio de $\mathbf{r}(t)=\langle f(t),g(t),h(t)\rangle$ es la
\emph{intersección} de los dominios de sus componentes,
$\operatorname{Dom}\mathbf{r}=\operatorname{Dom}f\cap\operatorname{Dom}g
\cap\operatorname{Dom}h$. Por el teorema anterior, $\mathbf{r}$ es continua
en cada punto de ese conjunto donde las tres componentes lo sean.
\end{rem}

\begin{prob}[Dominio de una función vectorial]
Halle el dominio de $\mathbf{r}(t)=\big\langle \ln(t^2-1),\;
\sqrt{\sen t},\; \cos(1/t)\big\rangle$.
\begin{myproof}
\textbf{Paso 1: Primera componente.}
$\ln(t^2-1)$ exige $t^2-1>0$, es decir $|t|>1$:
$t\in(-\infty,-1)\cup(1,\infty)$.

\textbf{Paso 2: Segunda componente.}
$\sqrt{\sen t}$ exige $\sen t\ge 0$, esto es
$t\in\bigcup_{k\in\mathbb{Z}}[2k\pi,(2k+1)\pi]$.

\textbf{Paso 3: Tercera componente.}
$\cos(1/t)$ exige $t\neq 0$, condición ya garantizada por $|t|>1$.

\textbf{Paso 4: Intersección.}
Intersecando las tres condiciones (para $t>1$ el primer intervalo válido es
$(1,\pi]$; para $t<-1$, los intervalos con $\sen t\ge0$ ya cumplen $|t|>1$):
\[
  \boxed{\ \operatorname{Dom}\mathbf{r}
  =\Big(\bigcup_{k\ge 1}[-2k\pi,\,-(2k-1)\pi]\Big)
   \;\cup\;(1,\pi]\;\cup\;
   \Big(\bigcup_{k\ge 1}[2k\pi,(2k+1)\pi]\Big).\ }
\]
\end{myproof}
...
\begin{definition}[Derivada de una función vectorial]\index{funcion vectorial@función vectorial!derivada}
...
\begin{theorem}[Derivada componente a componente]\index{funcion vectorial@función vectorial!derivada componente a componente}
...
\end{enumerate}
\begin{myproof}
\textbf{Paso 1.} Estrategia. La derivada de una función vectorial se obtiene
derivando componente a componente:
\[
  ...
  \mathbf{r}'(t)=\langle x'(t),y'(t),z'(t)\rangle,
\]
de modo que cada apartado se reduce a tres derivadas de una variable.

\textbf{Paso 2.} Apartado a) $\mathbf{r}(t)=\langle t^2+1,\;e^{2t},\;\cos(3t)\rangle$.
...
\textbf{Paso 3.} Apartado b) $\mathbf{r}(t)=\langle \ln(t+1),\;\arctan t,\;\sqrt{t+1}\rangle$.
...
\textbf{Paso 4.} Apartado c) $\mathbf{r}(t)=\langle \sen(2t),\;\cos(2t),\;e^{-t}\rangle$.
...
\begin{prob}[Recta tangente y desprendimiento por la tangente]
Una partícula se mueve en el plano siguiendo la trayectoria
\[
  \mathbf{r}(t)=\langle t^2+1,\; t^3-4t\rangle, \qquad t\ge 0,
\]
donde $t$ se mide en segundos.
\begin{enumerate}[a)]
  \item Halle la ecuación de la recta tangente a la trayectoria en el
        instante $t=2$.
  \item En el instante $t=2$ la partícula se desprende de la trayectoria y
        continúa en línea recta con la velocidad (constante) que tenía en
        ese instante. Determine su posición en $t=3$.
\end{enumerate}
\begin{myproof}
\textbf{Paso 1: Vector velocidad.}
Derivando componente a componente,
\[
  \mathbf{r}'(t)=\langle 2t,\; 3t^2-4\rangle.
\]

\textbf{Paso 2: Punto y velocidad en $t=2$.}
\[
  \mathbf{r}(2)=\langle 2^2+1,\; 2^3-4\cdot 2\rangle=\langle 5,\,0\rangle,
  \qquad
  \mathbf{r}'(2)=\langle 2\cdot 2,\; 3\cdot 2^2-4\rangle=\langle 4,\,8\rangle.
\]

\textbf{Paso 3: Recta tangente (apartado a).}
La recta tangente pasa por $\mathbf{r}(2)=(5,0)$ con vector director
$\mathbf{r}'(2)=\langle 4,8\rangle$:
\[
  \mathbf{L}(s)=\mathbf{r}(2)+s\,\mathbf{r}'(2)
              =\langle 5+4s,\; 8s\rangle, \qquad s\in\mathbb{R}.
\]
Eliminando el parámetro ($s=y/8$ y $x=5+4\cdot y/8$) se obtiene la forma
cartesiana
\[
  \boxed{\,y=2(x-5)=2x-10.\,}
\]

\textbf{Paso 4: Posición tras el desprendimiento (apartado b).}
A partir de $t=2$ la partícula viaja con velocidad constante
$\mathbf{r}'(2)$; su posición es
\[
  \mathbf{p}(t)=\mathbf{r}(2)+(t-2)\,\mathbf{r}'(2).
\]
En $t=3$ ha transcurrido $\Delta t=1\ \text{s}$, de modo que
\[
  \mathbf{p}(3)=\langle 5,0\rangle+(1)\langle 4,8\rangle
              =\boxed{\langle 9,\,8\rangle.}
\]
\end{myproof}
\end{prob}

\begin{rem}
El rayo de escape y la recta tangente son la \emph{misma} recta
(figura~\ref{fig:recta_tangente_escape}); lo que cambia es el papel del
parámetro: en el apartado (a) $s$ recorre toda la recta, mientras que en (b)
el desplazamiento real es $(t-2)\,\mathbf{r}'(2)$, proporcional al tiempo
transcurrido desde el desprendimiento.
\end{rem}
...
\begin{axis}[
  ...
  xmin=-0.5, xmax=10, ymin=-4, ymax=9,
  xtick={2,5,9}, ytick={-3,8},
  tick label style={font=\small},
  clip=false,
]
  % trayectoria r(t)
  \addplot[domain=0:2.25, samples=80, smooth, very thick, blue!70!black]
    ({x^2+1}, {x^3-4*x});
  % extensión hacia atrás de la tangente
  \draw[dashed, red!70!black]
    (axis cs:5,0) -- (axis cs:3.8,-2.4);
  % rayo de escape = vector velocidad r'(2)
  \draw[->, very thick, red!70!black]
    (axis cs:5,0) -- (axis cs:9,8);
  % puntos
  ...
  \addplot[only marks, mark=*, mark size=2pt, red!70!black]
    coordinates {(9,8)};
  % etiquetas
  \node[anchor=north west] at (axis cs:5,0) {$P=\mathbf{r}(2)$};
  \node[anchor=west] at (axis cs:9,8)
    {$\mathbf{r}(2)+\mathbf{r}'(2)$};
  \node[anchor=north west, red!70!black] at (axis cs:7,4)
    {$\mathbf{r}'(2)=\langle 4,8\rangle$};
  \node[anchor=north east, blue!70!black] at (axis cs:2,-3)
    {$\mathbf{r}(t)$};
\end{axis}
\end{tikzpicture}
\caption{Trayectoria $\mathbf{r}(t)=\langle t^2+1,\,t^3-4t\rangle$ (azul),
punto de desprendimiento $P=\mathbf{r}(2)=(5,0)$ y rayo tangente (rojo) que
sigue la partícula con velocidad $\mathbf{r}'(2)=\langle 4,8\rangle$ hasta
$(9,8)$ en $t=3$.}
\label{fig:recta_tangente_escape}
\end{figure}

\begin{rem}[Vector tangente unitario]
Si $\mathbf{r}'(t)\neq\mathbf{0}$, el vector velocidad $\mathbf{r}'(t)$ apunta
en la dirección tangente a la curva. Normalizándolo se obtiene el
\textbf{vector tangente unitario}
\[
  \hat{\mathbf{T}}(t)=\frac{\mathbf{r}'(t)}{\|\mathbf{r}'(t)\|},
\]
que conserva la dirección del movimiento pero tiene norma $1$; describe la
orientación de la trayectoria con independencia de la rapidez con que se
recorra. Es la base para definir la curvatura y el triedro de Frenet.
\end{rem}

\begin{prob}[Vector tangente unitario]
Halle el vector tangente unitario $\hat{\mathbf{T}}(t)$ de la curva
\[
  \mathbf{r}(t)=\langle e^{t}\cos t,\; e^{t}\sen t,\; e^{t}\rangle
\]
y evalúelo en $t=0$.
\begin{myproof}
\textbf{Paso 1: Vector velocidad.}
Derivando componente a componente (regla del producto en las dos primeras),
\[
  \mathbf{r}'(t)=\big\langle e^{t}(\cos t-\sen t),\;
                             e^{t}(\sen t+\cos t),\;
                             e^{t}\big\rangle
              = e^{t}\,\langle \cos t-\sen t,\; \sen t+\cos t,\; 1\rangle.
\]

\textbf{Paso 2: Rapidez $\|\mathbf{r}'(t)\|$.}
Como $e^{t}>0$,
\[
  \|\mathbf{r}'(t)\|
  = e^{t}\sqrt{(\cos t-\sen t)^2+(\sen t+\cos t)^2+1}.
\]
Desarrollando, $(\cos t-\sen t)^2+(\sen t+\cos t)^2=2$, de modo que
\[
  \|\mathbf{r}'(t)\|=e^{t}\sqrt{2+1}=\sqrt{3}\,e^{t}.
\]

\textbf{Paso 3: Normalizar.}
El factor $e^{t}$ se cancela:
\[
  \hat{\mathbf{T}}(t)=\frac{\mathbf{r}'(t)}{\|\mathbf{r}'(t)\|}
  =\frac{1}{\sqrt{3}}\,\langle \cos t-\sen t,\; \sen t+\cos t,\; 1\rangle.
\]

\textbf{Paso 4: Evaluar en $t=0$.}
Con $\cos 0=1$ y $\sen 0=0$,
\[
  \hat{\mathbf{T}}(0)=\frac{1}{\sqrt{3}}\,\langle 1,\;1,\;1\rangle.
\]
\[
  \boxed{\ \hat{\mathbf{T}}(t)
  =\frac{1}{\sqrt{3}}\langle \cos t-\sen t,\; \sen t+\cos t,\; 1\rangle,
  \qquad
  \hat{\mathbf{T}}(0)=\frac{1}{\sqrt{3}}\langle 1,1,1\rangle.\ }
\]
...
\begin{theorem}[Reglas de derivación]\index{funcion vectorial@función vectorial!reglas de derivacion@reglas de derivación}
...
\begin{definition}[Integral indefinida de una función vectorial]\index{funcion vectorial@función vectorial!integral indefinida}
...
\begin{definition}[Integral definida de una función vectorial]\index{funcion vectorial@función vectorial!integral definida}
...
\begin{theorem}[Teorema fundamental del cálculo para funciones vectoriales]\index{teorema fundamental del calculo@teorema fundamental del cálculo!para funciones vectoriales}
...
\begin{example}[Problema de valor inicial en cinemática]
Una partícula tiene aceleración constante $\mathbf a(t)=\langle 0,0,2\rangle$,
velocidad inicial $\mathbf v(0)=\langle 3,4,0\rangle$ y posición inicial
$\mathbf r(0)=\langle 0,0,-3\rangle$. Halle $\mathbf v(t)$ y $\mathbf r(t)$,
identifique el tipo de curva que describe y el instante en que la partícula
está más cerca del origen.
\begin{myproof}
\textbf{Paso 1. Integrar la aceleración.}
\[
  \mathbf v(t)=\int\mathbf a(t)\,dt=\langle 0,0,2t\rangle+\mathbf C_1.
\]
Con $\mathbf v(0)=\langle 3,4,0\rangle$ resulta $\mathbf C_1=\langle 3,4,0\rangle$,
luego $\mathbf v(t)=\langle 3,\,4,\,2t\rangle$.

\textbf{Paso 2. Integrar la velocidad.}
\[
  \mathbf r(t)=\int\mathbf v(t)\,dt=\langle 3t,\,4t,\,t^2\rangle+\mathbf C_2.
\]
Con $\mathbf r(0)=\langle 0,0,-3\rangle$ se obtiene $\mathbf C_2=\langle 0,0,-3\rangle$, así
\[
  \boxed{\ \mathbf r(t)=\langle 3t,\,4t,\,t^2-3\rangle.\ }
\]

\textbf{Paso 3. Tipo de curva.}
Como $x=3t$, $y=4t$ satisfacen $4x-3y=0$, la curva vive en el plano vertical
$4x=3y$; en él, con $s=\sqrt{x^2+y^2}=5|t|$ se tiene $z=t^2-3=\tfrac{s^2}{25}-3$:
es una \emph{parábola}.

\textbf{Paso 4. Instante de distancia mínima.}
\[
  \|\mathbf r(t)\|^2=9t^2+16t^2+(t^2-3)^2=t^4+19t^2+9.
\]
Derivando, $\dfrac{d}{dt}\|\mathbf r\|^2=4t^3+38t=2t(2t^2+19)$, que se anula
solo en $t=0$ (pues $2t^2+19>0$). Allí hay mínimo, y
\[
  \boxed{\ t=0:\quad \|\mathbf r(0)\|=\|\langle 0,0,-3\rangle\|=3.\ }
\]
\end{myproof}
\end{example}
...
\begin{theorem}[Longitud de arco en $\mathbb{R}^3$]\index{longitud de arco!en $\mathbb{R}^3$}
...
\begin{prob}[Astroide: un punto no suave]
Considere el astroide $\mathbf r(t)=\langle \cos^3 t,\ \sen^3 t\rangle$.
\begin{enumerate}[a)]
  \item Muestre que $\mathbf r'(t)=\mathbf 0$ en los múltiplos de $\pi/2$, de
        modo que la curva no es suave allí (tiene cúspides).
  \item Calcule la longitud del arco correspondiente a $0\le t\le\pi/2$.
\end{enumerate}
\end{prob}
...
\begin{definition}[Función longitud de arco]\index{funcion longitud de arco@función longitud de arco}
...
\begin{definition}[Función de dos variables]\index{funcion@función!de dos variables}
...
\begin{definition}[Función de tres variables]\index{funcion@función!de tres variables}
...
\begin{example}[Dominio con un producto bajo la raíz: análisis por casos]
Determine el dominio de
\[
  f(x,y)=\sqrt{(x+y)\,\ln(x-y)}.
\]
\begin{myproof}
\textbf{Paso 1. Restricciones.}
El logaritmo exige $x-y>0$; la raíz exige radicando no negativo:
$(x+y)\ln(x-y)\ge 0$. Un producto es $\ge 0$ cuando ambos factores tienen el
mismo signo, así que estudiamos el signo de $\ln(x-y)$.

\textbf{Paso 2. Signo del logaritmo.}
$\ln(x-y)\ge 0\iff x-y\ge 1$ y $\ln(x-y)\le 0\iff 0<x-y\le 1$.

\textbf{Paso 3. Los dos casos.}
\begin{itemize}
  \item \emph{Ambos factores $\ge 0$:} $x-y\ge 1$ y $x+y\ge 0$.
  \item \emph{Ambos factores $\le 0$:} $0<x-y\le 1$ y $x+y\le 0$.
\end{itemize}
En $x-y=1$ el radicando es $0$ (permitido).

\textbf{Paso 4. Dominio.}
\[
  \boxed{\ D=\{x-y\ge 1,\ x+y\ge 0\}\ \cup\ \{0<x-y\le 1,\ x+y\le 0\}.\ }
\]
La región se muestra en la figura~\ref{fig:dominio_producto_raiz}.
\end{myproof}
\end{example}
...
\centering
\begin{tikzpicture}[scale=0.85]
  \fill[blue!12] (0.5,-0.5)--(3,2)--(3,-3)--cycle;
  \fill[blue!12] (0,0)--(-3,-3)--(-2,-3)--(0.5,-0.5)--cycle;
  \draw[gray!25,thin] (-3,-3) grid (3,3);
  \draw[->] (-3.3,0)--(3.5,0) node[right]{$x$};
  \draw[->] (0,-3.3)--(0,3.5) node[above]{$y$};
  \draw[thick,dashed,red!70!black] (-3,-3)--(3,3) node[above left]{\small $y=x$};
  \draw[thick,blue!70!black] (-2,-3)--(3,2) node[right]{\small $x-y=1$};
  \draw[thick,green!45!black] (-3,3)--(3,-3) node[below right]{\small $x+y=0$};
  \node[blue!45!black] at (2.1,-1.1) {$R_1$};
  \node[blue!45!black] at (-1.6,-2.2) {$R_2$};
\end{tikzpicture}
\caption{Dominio de $f(x,y)=\sqrt{(x+y)\ln(x-y)}$: $R_1$ ($x-y\ge 1$,
  $x+y\ge 0$) y $R_2$ ($0<x-y\le 1$, $x+y\le 0$). La recta $y=x$ (rayada) está
  excluida; $x-y=1$ y $x+y=0$ están incluidas.}
\label{fig:dominio_producto_raiz}
\end{figure}

\begin{prob}
Determine el dominio de
$\displaystyle f(x,y)=\frac{\ln(2-x+y)}{\sqrt{(x-y+1)\,\ln(x+y+1)}}$,
describiendo la región del plano mediante análisis de signos por casos.
\end{prob}

\begin{prob}
Determine el dominio de la función $f(x,y)=\sqrt{-5x^2-5y^2+25}$. Haga una representación gráfica de este dominio en el plano.
\begin{myproof}
\textbf{Paso 1.} Condición de existencia. La raíz cuadrada solo está definida
sobre radicandos no negativos, de modo que el dominio de
$f(x,y)=\sqrt{-5x^2-5y^2+25}$ es el conjunto de puntos $(x,y)$ que satisfacen
$-5x^2-5y^2+25\geq 0.$

\textbf{Paso 2.} Simplificación de la desigualdad.
\begin{align*}
-5x^2-5y^2+25\geq 0 \Leftrightarrow -(5x^2+5y^2)+25\geq 0 \Leftrightarrow 5x^2+5y^2\leq 25
\Leftrightarrow x^2+y^2\leq 5.
\end{align*}

\textbf{Paso 3.} Descripción de la región. La condición $x^2+y^2\leq 5$ define
el disco cerrado centrado en el origen y de radio $\sqrt{5}$ (interior más
frontera de la circunferencia $x^2+y^2=5$):
\[
  \boxed{D_f=\{(x,y)\in\mathbb{R}^2:\;x^2+y^2\leq 5\}.}
\]
La gráfica de este dominio se presenta a continuación como la región sombreada:
\begin{figure}[H]
\centering
...
\begin{myproof}
\textbf{Paso 1.} Condición de existencia. La raíz cuadrada exige radicando no
negativo, así que el dominio de $f(x,y)=\sqrt{-4x^2-y^2+8}$ lo forman los
puntos $(x,y)$ con $-4x^2-y^2+8\geq 0.$

\textbf{Paso 2.} Simplificación de la desigualdad.
\begin{align*}
-4x^2-y^2+8\geq 0 \Leftrightarrow -(4x^2+y^2)+8\geq 0 \Leftrightarrow 4x^2+y^2\leq 8
\Leftrightarrow \frac{x^2}{2}+\frac{y^2}{8}\leq 1.
\end{align*}

\textbf{Paso 3.} Descripción de la región. Se trata de la elipse
$\dfrac{x^2}{2}+\dfrac{y^2}{8}=1$ junto con su interior: centro en el origen,
semieje horizontal $\sqrt{2}$ y semieje vertical $\sqrt{8}=2\sqrt{2}$:
\[
  \boxed{D_f=\{(x,y)\in\mathbb{R}^2:\;4x^2+y^2\leq 8\}.}
\]
La gráfica de este dominio se presenta a continuación como la región sombreada:
\begin{figure}[H]
\centering
...
\begin{myproof}
\textbf{Paso 1.} Condición de existencia. El radicando debe ser no negativo, de
manera que el dominio de $f(x,y)=\sqrt{-x^2-3y^2+2}$ está formado por los
puntos $(x,y)$ con $-x^2-3y^2+2\geq 0.$

\textbf{Paso 2.} Simplificación de la desigualdad.
\begin{align*}
-x^2-3y^2+2\geq 0 \Leftrightarrow -(x^2+3y^2)+2\geq 0 \Leftrightarrow x^2+3y^2\leq 2
\Leftrightarrow \frac{x^2}{2}+\frac{y^2}{2/3}\leq 1.
\end{align*}

\textbf{Paso 3.} Descripción de la región. Es la elipse
$\dfrac{x^2}{2}+\dfrac{y^2}{2/3}=1$ con su interior: centro en el origen,
semieje horizontal $\sqrt{2}$ y semieje vertical $\sqrt{2/3}$:
\[
  \boxed{D_f=\{(x,y)\in\mathbb{R}^2:\;x^2+3y^2\leq 2\}.}
\]
La gráfica de este dominio se presenta a continuación como la región sombreada:
\begin{figure}[H]
\centering
...
\begin{myproof}
\textbf{Paso 1.} Condición de existencia. La raíz cuadrada requiere radicando no
negativo, luego el dominio de $f(x,y)=\sqrt{-3x^2-3y^2+9}$ son los puntos
$(x,y)$ que cumplen $-3x^2-3y^2+9\geq 0.$

\textbf{Paso 2.} Simplificación de la desigualdad.
\begin{align*}
-3x^2-3y^2+9\geq 0 \Leftrightarrow -(3x^2+3y^2)+9\geq 0 \Leftrightarrow 3x^2+3y^2\leq 9
\Leftrightarrow x^2+y^2\leq 3.
\end{align*}

\textbf{Paso 3.} Descripción de la región. Como los coeficientes de $x^2$ y
$y^2$ coinciden, la región es un disco cerrado: el centrado en el origen de
radio $\sqrt{3}$ (interior y frontera de la circunferencia $x^2+y^2=3$):
\[
  \boxed{D_f=\{(x,y)\in\mathbb{R}^2:\;x^2+y^2\leq 3\}.}
\]
La gráfica de este dominio se presenta a continuación como la región sombreada:
\begin{figure}[H]
\centering
...
\begin{prob}[Dominio con estructura de tablero]
Halle y grafique el dominio de $f(x,y)=\sqrt{\sen x\,\sen y}$.
\begin{myproof}
\textbf{Paso 1: Condición del radicando.}
Se requiere $\sen x\,\sen y\ge 0$: el producto de los dos senos no puede
ser negativo.

\textbf{Paso 2: Signo del seno por franjas.}
En cada franja $(k\pi,(k+1)\pi)$ el seno tiene signo constante:
$\operatorname{sgn}(\sen x)=(-1)^k$ para $x\in(k\pi,(k+1)\pi)$, y análogamente
para $\sen y$.

\textbf{Paso 3: Producto no negativo.}
$\sen x\,\sen y\ge 0$ ocurre cuando ambos factores tienen el mismo signo, es
decir cuando $x\in(k\pi,(k+1)\pi)$ y $y\in(m\pi,(m+1)\pi)$ con
$(-1)^k=(-1)^m$, o sea $k+m$ \emph{par}. Además el producto es cero sobre las
rectas $x=k\pi$ y $y=m\pi$, que también pertenecen al dominio.

\textbf{Paso 4: Descripción del dominio.}
El dominio es un \emph{tablero de ajedrez}:
\[
  \boxed{\ \operatorname{Dom}f=\bigcup_{\substack{k,m\in\mathbb{Z}\\ k+m\ \text{par}}}
  [k\pi,(k+1)\pi]\times[m\pi,(m+1)\pi].\ }
\]
Los cuadrados sombreados de la figura~\ref{fig:dominio_tablero} lo ilustran en
$[-2\pi,2\pi]^2$.
\end{myproof}
\end{prob}
...
\begin{tikzpicture}
  \def\P{1.15}
  % cuadrados del dominio: k+m par
  \foreach \k in {-2,...,1}{
    \foreach \m in {-2,...,1}{
      \pgfmathsetmacro{\s}{Mod(\k+\m+4,2)}
      \ifdim\s pt<0.5pt
        \fill[blue!20] (\k*\P,\m*\P) rectangle ++(\P,\P);
      \fi
    }
  }
  % cuadricula
  \draw[step=\P, gray!45, very thin] (-2*\P,-2*\P) grid (2*\P,2*\P);
  % ejes
  \draw[->,thin] (-2.45*\P,0)--(2.5*\P,0) node[right]{$x$};
  \draw[->,thin] (0,-2.45*\P)--(0,2.5*\P) node[above]{$y$};
  % marcas
  \foreach \i/\lab in {-2/{-2\pi},-1/{-\pi},1/{\pi},2/{2\pi}}{
    \draw (\i*\P,2pt)--(\i*\P,-2pt) node[below,font=\tiny]{$\lab$};
    \draw (2pt,\i*\P)--(-2pt,\i*\P) node[left,font=\tiny]{$\lab$};
  }
\end{tikzpicture}
\caption{Dominio de $f(x,y)=\sqrt{\sen x\,\sen y}$ (azul): tablero de cuadrados
  $[k\pi,(k+1)\pi]\times[m\pi,(m+1)\pi]$ con $k+m$ par, unidos por las rectas
  $x=k\pi$ y $y=m\pi$.}
\label{fig:dominio_tablero}
\end{figure}
...
\subsection{Secciones cónicas: repaso}

Antes de estudiar las superficies cuádricas conviene recordar las
\textbf{secciones cónicas}, sus análogas en el plano. Una ecuación de
segundo grado sin término cruzado,
\[
  Ax^2+By^2+Cx+Dy+E=0,
\]
representa (salvo casos degenerados) una elipse si $A$ y $B$ tienen el mismo
signo, una hipérbola si tienen signos opuestos y una parábola si uno de ellos
es nulo. El procedimiento para identificarla consiste en \textbf{completar
cuadrados} y llevarla a su forma canónica.

\begin{prob}[Elementos de una hipérbola por completar cuadrados]
Identifique la cónica $16x^2-9y^2-64x-18y-89=0$ y determine su centro,
vértices, focos, asíntotas y excentricidad.
\begin{myproof}
\textbf{Paso 1. Agrupar y completar cuadrados.} Separamos por variable y
factorizamos los coeficientes principales:
\[
  16(x^2-4x)-9(y^2+2y)=89.
\]
Como $x^2-4x=(x-2)^2-4$ y $y^2+2y=(y+1)^2-1$,
\[
  16\bigl[(x-2)^2-4\bigr]-9\bigl[(y+1)^2-1\bigr]=89,
\]
\[
  16(x-2)^2-9(y+1)^2-64+9=89
  \;\Longrightarrow\;
  16(x-2)^2-9(y+1)^2=144.
\]

\textbf{Paso 2. Forma canónica.} Dividiendo entre $144$:
\[
  \frac{(x-2)^2}{9}-\frac{(y+1)^2}{16}=1.
\]
Es una \textbf{hipérbola} de eje transversal horizontal, con $a^2=9$,
$b^2=16$ y centro $(h,k)=(2,-1)$.

\textbf{Paso 3. Parámetros.} $a=3$, $b=4$ y
$c=\sqrt{a^2+b^2}=\sqrt{9+16}=5$; la excentricidad es
$e=\dfrac{c}{a}=\dfrac{5}{3}$.

\textbf{Paso 4. Elementos y bosquejo.}
\begin{itemize}
  \item Centro: $C(2,-1)$.
  \item Vértices: $(h\pm a,\,k)=(5,-1)$ y $(-1,-1)$.
  \item Focos: $(h\pm c,\,k)=(7,-1)$ y $(-3,-1)$.
  \item Asíntotas: $y+1=\pm\dfrac{b}{a}(x-2)=\pm\dfrac{4}{3}(x-2)$.
\end{itemize}
El trazado se muestra en la figura~\ref{fig:hiperbola_canonica}.
\[
  \boxed{\ \frac{(x-2)^2}{9}-\frac{(y+1)^2}{16}=1,\quad
  C(2,-1),\ a=3,\ b=4,\ c=5,\ e=\tfrac53.\ }
\]
\end{myproof}
\end{prob}
...
\begin{tikzpicture}[scale=0.5]
  % Ejes
  \draw[->,thin] (-5,0)--(9.3,0) node[anchor=north west]{$x$};
  \draw[->,thin] (0,-7.3)--(0,5.7) node[anchor=south east]{$y$};
  % Asíntotas: y+1 = ±(4/3)(x-2)
  \draw[dashed,gray!70] (-2.5,-7)--(6.5,5);
  \draw[dashed,gray!70] (-2.5,5)--(6.5,-7);
  % Rama derecha: (2+3cosh t, -1+4senh t)
  \draw[very thick,blue!70!black,domain=-1.1:1.1,samples=60,smooth]
    plot ({2+1.5*(exp(\x)+exp(-\x))},{-1+2*(exp(\x)-exp(-\x))});
  % Rama izquierda
  \draw[very thick,blue!70!black,domain=-1.1:1.1,samples=60,smooth]
    plot ({2-1.5*(exp(\x)+exp(-\x))},{-1+2*(exp(\x)-exp(-\x))});
  % Centro
  \filldraw[black] (2,-1) circle(0.11)
    node[anchor=south west,font=\small]{$C$};
  % Vértices
  \filldraw[blue!70!black] (5,-1) circle(0.11);
  \filldraw[blue!70!black] (-1,-1) circle(0.11);
  \node[anchor=north,font=\scriptsize] at (5,-1.35){$V_1(5,-1)$};
  \node[anchor=north,font=\scriptsize] at (-1,-1.35){$V_2(-1,-1)$};
  % Focos
  \filldraw[red!70!black] (7,-1) circle(0.13);
  \filldraw[red!70!black] (-3,-1) circle(0.13);
  \node[anchor=south,font=\scriptsize,red!70!black] at (7,-0.6){$F_1(7,-1)$};
  \node[anchor=south,font=\scriptsize,red!70!black] at (-3,-0.6){$F_2(-3,-1)$};
\end{tikzpicture}
\caption{Hipérbola $\dfrac{(x-2)^2}{9}-\dfrac{(y+1)^2}{16}=1$: centro $C(2,-1)$,
  vértices $V_1,V_2$, focos $F_1,F_2$ y asíntotas $y+1=\pm\tfrac43(x-2)$.}
\label{fig:hiperbola_canonica}
\end{figure}
...
\begin{example}[Elipsoide con términos lineales]
Clasifique la superficie $4x^2+9y^2+36z^2-8x+36y+4=0$: halle su centro, sus
semiejes y sus trazas con los planos coordenados trasladados.
\begin{myproof}
\textbf{Paso 1. Agrupar y completar cuadrados.}
\[
  4(x^2-2x)+9(y^2+4y)+36z^2+4=0.
\]
Como $x^2-2x=(x-1)^2-1$ y $y^2+4y=(y+2)^2-4$,
\[
  4(x-1)^2-4+9(y+2)^2-36+36z^2+4=0
  \;\Rightarrow\; 4(x-1)^2+9(y+2)^2+36z^2=36.
\]

\textbf{Paso 2. Forma canónica.}
Dividiendo entre $36$,
\[
  \frac{(x-1)^2}{9}+\frac{(y+2)^2}{4}+\frac{z^2}{1}=1.
\]
Es un \emph{elipsoide} de centro $C(1,-2,0)$ y semiejes $a=3$, $b=2$, $c=1$.

\textbf{Paso 3. Trazas con los planos por el centro.}
$z=0$: $\dfrac{(x-1)^2}{9}+\dfrac{(y+2)^2}{4}=1$ (elipse);
$x=1$: $\dfrac{(y+2)^2}{4}+z^2=1$ (elipse);
$y=-2$: $\dfrac{(x-1)^2}{9}+z^2=1$ (elipse).

\textbf{Paso 4. Resultado.}
\[
  \boxed{\ \frac{(x-1)^2}{9}+\frac{(y+2)^2}{4}+z^2=1:\ \text{elipsoide},\
  C(1,-2,0),\ a=3,\,b=2,\,c=1.\ }
\]
La superficie se muestra en la figura~\ref{fig:elipsoide_trasladado}.
\end{myproof}
\end{example}
...
\begin{tikzpicture}
\begin{axis}[axis lines=center, xlabel=$x$, ylabel=$y$, zlabel=$z$,
             xmin=-2.5,xmax=4.5,ymin=-4.5,ymax=0.8,zmin=-1.6,zmax=1.6,
             view={125}{22}, ticklabel style={font=\tiny},
             width=8cm, height=7cm, trig format plots=deg]
  \addplot3[surf, shader=flat corner, samples=20,
            variable=\u, variable y=\v,
            domain=0:180, y domain=0:360,
            colormap/cool, opacity=0.80]
    ({1+3*sin(\u)*cos(\v)}, {-2+2*sin(\u)*sin(\v)}, {cos(\u)});
\end{axis}
\end{tikzpicture}
\caption{Elipsoide $\dfrac{(x-1)^2}{9}+\dfrac{(y+2)^2}{4}+z^2=1$ de centro
  $(1,-2,0)$ y semiejes $3$, $2$, $1$.}
\label{fig:elipsoide_trasladado}
\end{figure}

\subsection{Cilindros}

Una \textbf{superficie cilíndrica} (o \textbf{cilindro}) es la superficie
generada por una recta —la \emph{generatriz}— que se desplaza paralela a un
eje fijo apoyándose sobre una curva plana —la \emph{directriz}—. En
coordenadas cartesianas, toda ecuación en la que \textbf{falta una de las
tres variables} representa un cilindro cuyas generatrices son paralelas al
eje de la variable ausente. Por ejemplo, $x^2+y^2=1$ en $\mathbb{R}^3$ es un
cilindro circular de eje $z$, cuya directriz es la circunferencia
$x^2+y^2=1$ del plano $xy$.

\begin{prob}[Cilindro elíptico]
Identifique la superficie $4x^2+9y^2-16x+18y-11=0$, escríbala en forma
canónica, describa sus secciones con los planos $z=k$ y esbócela.
\begin{myproof}
\textbf{Paso 1. Reconocer el tipo.} La ecuación no contiene la variable $z$:
se trata de un \textbf{cilindro} con generatrices paralelas al eje $z$; su
forma queda determinada por la curva directriz del plano $xy$.
...
\]
La directriz es una \textbf{elipse} de centro $(2,-1)$, con semieje $a=3$ en
la dirección $x$ y semieje $b=2$ en la dirección $y$.

\textbf{Paso 3. Secciones $z=k$.} Como $z$ no aparece en la ecuación, la
sección con cualquier plano horizontal $z=k$ es \emph{la misma} elipse
$\dfrac{(x-2)^2}{9}+\dfrac{(y+1)^2}{4}=1$. La superficie es un
\textbf{cilindro elíptico} cuyo eje es la recta vertical $x=2$, $y=-1$
(figura~\ref{fig:cilindro_eliptico}).

\textbf{Paso 4. Resultado.}
\[
  \boxed{\ \frac{(x-2)^2}{9}+\frac{(y+1)^2}{4}=1:\ \text{cilindro elíptico de
  eje } (2,-1,z).\ }
\]
\end{myproof}
\end{prob}
...
\begin{tikzpicture}
\begin{axis}[axis lines=center, xlabel=$x$, ylabel=$y$, zlabel=$z$,
             xmin=-2,xmax=6,ymin=-4,ymax=2,zmin=0,zmax=5,
             view={130}{25}, ticklabel style={font=\tiny},
  ...
  \addplot3[surf, shader=flat corner, samples=20,
            ...
            domain=0:360, y domain=0:5,
            trig format plots=deg]
           ({2+3*cos(\u)}, {-1+2*sin(\u)}, {\v});
\end{axis}
\end{tikzpicture}
\caption{Cilindro elíptico $\dfrac{(x-2)^2}{9}+\dfrac{(y+1)^2}{4}=1$, con
  generatrices paralelas al eje $z$.}
\label{fig:cilindro_eliptico}
\end{figure}

\begin{prob}
Bosqueje en $\mathbb{R}^3$ la superficie $z=\cos x$ y explique por qué es un
cilindro. Identifique su directriz y la dirección de las generatrices.
\emph{Sugerencia:} la ecuación no contiene a la variable $y$
(figura~\ref{fig:cilindro_coseno}).
\end{prob}
...
\begin{tikzpicture}
\begin{axis}[axis lines=center, xlabel=$x$, ylabel=$y$, zlabel=$z$,
             view={120}{22}, ticklabel style={font=\tiny},
             width=8.5cm, height=6.5cm,
             xtick={-3,0,3}, ztick={-1,1}]
  \addplot3[surf, shader=flat corner, samples=20,
            domain=-3.6:3.6, y domain=0:3,
            colormap/cool, opacity=0.80] {cos(deg(x))};
\end{axis}
\end{tikzpicture}
\caption{Cilindro $z=\cos x$: la directriz es la curva $z=\cos x$ del plano
  $xz$ y las generatrices son paralelas al eje $y$.}
\label{fig:cilindro_coseno}
\end{figure}
...
\begin{definition}[Curva de nivel]\index{curva de nivel}
...
\begin{definition}[Superficie de nivel]\index{superficie de nivel}
...
\end{enumerate}
\begin{myproof}
\textbf{Paso 1.} Dominio (apartado a). El logaritmo exige argumento
estrictamente positivo, es decir $1+8x^2+2y^2>0.$ Como $8x^2\geq 0$ y
$2y^2\geq 0$ para todo $(x,y)$, el argumento es siempre $\geq 1>0$: la
desigualdad se satisface en todo punto del plano y por lo tanto
\[
  \boxed{D_f=\mathbb{R}^2.}
\]
Un bosquejo puede ser el siguiente
\begin{figure}[H]
\centering
...
\end{figure}
\textbf{Paso 2.} Ecuación general de las curvas de nivel (apartado b). Igualando
$f(x,y)=z$ y aplicando la exponencial a ambos lados:
\begin{align*}
z=\ln\left( 1+8x^2+2y^2 \right)
&\Rightarrow \mathrm{e}^{z}=\mathrm{e}^{\ln\left( 1+8x^2+2y^2 \right)}=1+8x^2+2y^2\\
&\Rightarrow 8x^2+2y^2=\mathrm{e}^{z}-1.
\end{align*}
Para $z>0$ el lado derecho es positivo y la curva es la elipse
\[
  \frac{x^2}{(\mathrm{e}^{z}-1)/8}+\frac{y^2}{(\mathrm{e}^{z}-1)/2}=1,
  \qquad a_x=\sqrt{\frac{\mathrm{e}^{z}-1}{8}},\quad a_y=\sqrt{\frac{\mathrm{e}^{z}-1}{2}},
\]
de modo que $a_y=2a_x$ en todos los niveles. Para $z=0$ el lado derecho se anula
y la curva degenera en un punto; para $z<0$ no hay solución (el rango de $f$ es
$[0,\infty)$).

\textbf{Paso 3.} Los cuatro niveles pedidos.
\begin{itemize}
\item Para $z=0$: $8x^2+2y^2=0$, lo que obliga a $x=0$ y $y=0$; la curva de nivel se reduce al punto $(0,0).$
...
\end{itemize}
\[
  \boxed{8x^2+2y^2=\mathrm{e}^{z}-1:\;\text{elipses concéntricas para }z>0,\;
  \text{el punto }(0,0)\text{ para }z=0.}
\]

\textbf{Paso 4.} Gráfica. Las curvas de nivel se presentan a continuación:
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.85]
...
\begin{myproof}
\textbf{Paso 1.} Dominio (apartado a). El logaritmo exige argumento
estrictamente positivo, esto es $1+4x^2+y^2>0.$ Puesto que $4x^2\geq 0$ y
$y^2\geq 0$, el argumento vale al menos $1$ en todo punto, de modo que la
condición se cumple en todo el plano:
\[
  \boxed{D_f=\mathbb{R}^2.}
\]
Un bosquejo puede ser el siguiente
...
\end{figure}
\textbf{Paso 2.} Ecuación general de las curvas de nivel (apartado b). Igualando
$f(x,y)=z$ y exponenciando:
\begin{align*}
z=\ln\left( 1+4x^2+y^2 \right)
&\Rightarrow \mathrm{e}^{z}=1+4x^2+y^2
\Rightarrow 4x^2+y^2=\mathrm{e}^{z}-1.
\end{align*}
Para $z>0$ se obtiene la elipse
\[
  \frac{x^2}{(\mathrm{e}^{z}-1)/4}+\frac{y^2}{\mathrm{e}^{z}-1}=1,
  \qquad a_x=\sqrt{\frac{\mathrm{e}^{z}-1}{4}},\quad a_y=\sqrt{\mathrm{e}^{z}-1},
\]
de nuevo con $a_y=2a_x$. Para $z=0$ la curva degenera en el origen y para $z<0$
el conjunto de nivel es vacío.

\textbf{Paso 3.} Los cuatro niveles pedidos.
\begin{itemize}
\item Para $z=0$: $4x^2+y^2=0$, es decir el punto $(0,0)$.
...
\end{itemize}
\[
  \boxed{4x^2+y^2=\mathrm{e}^{z}-1:\;\text{elipses concéntricas para }z>0,\;
  \text{el punto }(0,0)\text{ para }z=0.}
\]
...
\begin{tikzpicture}[scale=0.5]
  \draw[->](-2.7,0)--(2.7,0); \draw[->](0,-2.7)--(0,2.7);
  \foreach \r in {0.8,1.5,2.2} \draw[blue!70!black,thick] (0,0) circle(\r);
  \node at (0,-3.15) {\small (I)};
\end{tikzpicture}
\end{minipage}\hfil
\begin{minipage}{0.30\textwidth}\centering
\begin{tikzpicture}[scale=0.5]
  \draw[->](-2.7,0)--(2.7,0); \draw[->](0,-2.7)--(0,2.7);
  \foreach \k in {-1.6,-0.2,1.2}
    \draw[blue!70!black,thick,domain=-2:2,samples=40,smooth,variable=\x]
      plot ({\x},{0.45*\x*\x+\k});
  \node at (0,-3.15) {\small (II)};
\end{tikzpicture}
\end{minipage}
...
\begin{minipage}{0.30\textwidth}\centering
\begin{tikzpicture}[scale=0.5]
  \draw[gray!55,dashed](-2.4,-2.4)--(2.4,2.4);
  \draw[gray!55,dashed](-2.4,2.4)--(2.4,-2.4);
  \draw[->](-2.7,0)--(2.7,0); \draw[->](0,-2.7)--(0,2.7);
  \draw[blue!70!black,thick,domain=-1.4:1.4,samples=40,variable=\t] plot ({cosh(\t)},{sinh(\t)});
  \draw[blue!70!black,thick,domain=-1.4:1.4,samples=40,variable=\t] plot ({-cosh(\t)},{sinh(\t)});
  \draw[red!60!black,thick,domain=-1.4:1.4,samples=40,variable=\t] plot ({sinh(\t)},{cosh(\t)});
  \draw[red!60!black,thick,domain=-1.4:1.4,samples=40,variable=\t] plot ({sinh(\t)},{-cosh(\t)});
  \node at (0,-3.15) {\small (III)};
\end{tikzpicture}
\end{minipage}\hfil
\begin{minipage}{0.30\textwidth}\centering
\begin{tikzpicture}[scale=0.5]
  \draw[->](-2.7,0)--(2.7,0); \draw[->](0,-2.7)--(0,2.7);
  \draw[blue!70!black,thick,domain=0.42:2.4,samples=40,variable=\x] plot ({\x},{1/\x});
  \draw[blue!70!black,thick,domain=-2.4:-0.42,samples=40,variable=\x] plot ({\x},{1/\x});
  \draw[red!60!black,thick,domain=0.42:2.4,samples=40,variable=\x] plot ({\x},{-1/\x});
  \draw[red!60!black,thick,domain=-2.4:-0.42,samples=40,variable=\x] plot ({\x},{-1/\x});
  \node at (0,-3.15) {\small (IV)};
\end{tikzpicture}
\end{minipage}
\caption{Mapas de contorno: (I) circunferencias, (II) parábolas, (III)
  hipérbolas con asíntotas $y=\pm x$, (IV) hipérbolas con asíntotas los ejes.}
\label{fig:emparejar_contornos}
\end{figure}

\begin{example}[Emparejar funciones con sus mapas de contorno]
Relacione cada función con su mapa de curvas de nivel
(figura~\ref{fig:emparejar_contornos}), justificando por la forma de las
curvas y sus simetrías:
\[
  f_1=x^2+y^2,\quad f_2=y-x^2,\quad f_3=x^2-y^2,\quad f_4=xy.
\]
\begin{myproof}
\textbf{Paso 1. $f_1=x^2+y^2$.} Las curvas $x^2+y^2=k$ ($k>0$) son
circunferencias concéntricas; el valor crece con el radio. $\Rightarrow$ (I).

\textbf{Paso 2. $f_2=y-x^2$.} Las curvas $y=x^2+k$ son parábolas idénticas
trasladadas verticalmente. $\Rightarrow$ (II).

\textbf{Paso 3. $f_3=x^2-y^2$.} Las curvas $x^2-y^2=k$ son hipérbolas con
asíntotas $y=\pm x$ (abren en $x$ si $k>0$, en $y$ si $k<0$). $\Rightarrow$ (III).

\textbf{Paso 4. $f_4=xy$.} Las curvas $xy=k$ son hipérbolas equiláteras cuyas
asíntotas son los ejes coordenados. $\Rightarrow$ (IV).
\[
  \boxed{\ f_1\!\to\!\text{(I)},\quad f_2\!\to\!\text{(II)},\quad
  f_3\!\to\!\text{(III)},\quad f_4\!\to\!\text{(IV)}.\ }
\]
\end{myproof}
\end{example}
...
\begin{prob}
Calcule $\displaystyle\lim_{t\to 0}\left\langle \cos t,\ t^2\sen(1/t),\ t^2
\right\rangle$. Justifique el límite de la componente central mediante el
teorema de compresión (emparedado).
\end{prob}
...
\begin{prob}
Una partícula recorre la curva plana $\mathbf{r}(t)=\langle t^2-t,\; t^3\rangle$.
\begin{enumerate}[a)]
  \item Halle la recta tangente a la trayectoria en $t=1$.
  \item Si en $t=1$ la partícula se suelta y continúa en línea recta con la
        velocidad constante que tenía en ese instante, ¿en qué punto se
        encuentra en $t=2$?
\end{enumerate}
\end{prob}

\begin{prob}
Considere la curva plana
$\mathbf{f}(t)=\langle a\sinh t,\; b+c\cosh t\rangle$, con $a,c>0$ y $t\in\mathbb{R}$.
\begin{enumerate}[a)]
  \item Usando la identidad $\cosh^2 t-\sinh^2 t=1$, demuestre que la imagen de
        $\mathbf{f}$ está contenida en una hipérbola y halle su ecuación
        cartesiana.
  \item Determine cuál de las dos ramas recorre la curva y justifique su
        respuesta a partir del signo de $\cosh t$.
\end{enumerate}
\end{prob}

\begin{prob}
Considere las curvas en el espacio
\[
  \mathbf{r}(t)=\langle 2\cos t,\; 2\sen t,\; 3t\rangle,\quad t\in[0,2\pi),
  \qquad
  \mathbf{s}(u)=\langle 2,\; u,\; u^2\rangle,\quad u\in\mathbb{R}.
\]
Determine si $\mathbf{r}$ y $\mathbf{s}$ se intersectan y, en caso afirmativo,
halle todos los puntos de intersección junto con los valores de los
parámetros. \emph{Sugerencia:} $t$ y $u$ son independientes, así que iguale
componente a componente y no olvide verificar la tercera ecuación.
\end{prob}

\begin{prob}
Demuestre que la curva
$\mathbf{r}(t)=\langle \arccos(2t^2-1),\; \arcsen t\rangle$, con
$t\in[0,1]$, describe un \emph{segmento de recta}. Halle la ecuación
cartesiana de la recta que lo contiene y las coordenadas de los extremos del
segmento. \emph{Sugerencia:} sustituya $t=\sen\theta$ con
$\theta\in[0,\tfrac{\pi}{2}]$ y use las identidades de las funciones inversas.
\end{prob}
...
\begin{prob}
Identifique la cónica $9x^2-16y^2+36x+96y+36=0$ completando cuadrados y
determine su centro, vértices, focos y asíntotas.
\end{prob}

\begin{prob}
Para el paraboloide $-4x^2+9y^2=36z$, halle la traza con el plano $x=0$ e
identifique el vértice, el foco y la directriz de la parábola resultante.
\end{prob}

\begin{prob}
Identifique la superficie $9y^2+4z^2-18y+16z-11=0$: escríbala en forma
canónica, indique el eje del cilindro y describa sus secciones con los
planos $x=k$.
\end{prob}

\begin{prob}
Para el hiperboloide $4x^2+9y^2-z^2=1$ y el plano $z=c$ (con $c$ constante),
muestre que la traza es una elipse. Halle sus semiejes en función de $c$, sus
vértices y sus focos.
\end{prob}

\begin{prob}
Clasifique cada superficie completando cuadrados; halle su centro o vértice y
sus elementos:
\begin{enumerate}[a)]
  \item $z=x^2+2y^2-2x+8y$ (paraboloide elíptico).
  \item $9x^2+4y^2-36z^2=36$ (hiperboloide de una hoja).
  \item $z=x^2-y^2+2x-4y$ (paraboloide hiperbólico).
\end{enumerate}
\end{prob}
...
Se dan las esferas
\[
  S_1:\ x^2+y^2+z^2-2x+4y-4=0,\qquad
  S_2:\ x^2+y^2+z^2+6x-2y+2z-5=0,
\]
y el plano $\pi:\ 2x-y+2z=7$. Completando cuadrados, halle los centros de
$S_1$ y $S_2$ y determine la ecuación vectorial de la recta perpendicular a
$\pi$ que pasa por el punto medio del segmento que une dichos centros.
\end{prob}

\begin{prob}
Considere la cónica $\mathcal{C}:\ x^2+y^2-2xy-2(x+y)+1=0$.
\begin{enumerate}[a)]
  \item Calcule el discriminante $B^2-4AC$ y clasifique la cónica.
  \item Con el cambio de variables $u=x+y$, $v=x-y$, escriba la ecuación en
        las coordenadas $(u,v)$ y obtenga una parametrización
        $\mathbf{r}(s)=\langle x(s),y(s)\rangle$ de $\mathcal{C}$.
  \item Determine los puntos de $\mathcal{C}$ con tangente horizontal y con
        tangente vertical.
\end{enumerate}
La figura~\ref{fig:parabola_rotada} muestra $\mathcal{C}$ y los ejes girados.
\end{prob}
...
\begin{axis}[axis lines=middle, xlabel=$x$, ylabel=$y$,
             ...
             ticklabel style={font=\tiny}, width=9.5cm]
  % ejes girados u=x+y (recta y=x) y v=x-y (recta y=-x)
  \draw[dashed, gray!70] (axis cs:-1.4,-1.4) -- (axis cs:4.2,4.2)
    node[anchor=south west, black, font=\small] {$u$};
  \draw[dashed, gray!70] (axis cs:-1.4,1.4) -- (axis cs:1.4,-1.4)
    node[anchor=north west, black, font=\small] {$v$};
  % conica parametrizada: x=(s+1)^2/4, y=(s-1)^2/4
  \addplot[domain=-3:3, samples=90, smooth, very thick, blue!70!black]
    ({(x+1)^2/4}, {(x-1)^2/4});
  % vertice V=(1/4,1/4)
  \addplot[only marks, mark=*, mark size=1.5pt, red!70!black]
    coordinates {(0.25,0.25)};
  \node[anchor=north west, font=\scriptsize, red!70!black]
    at (axis cs:0.25,0.25) {$V$};
\end{axis}
\end{tikzpicture}
\caption{La cónica $x^2+y^2-2xy-2(x+y)+1=0$ (azul) y los ejes girados
  $u=x+y$, $v=x-y$; su vértice es $V=\left(\tfrac14,\tfrac14\right)$.}
\label{fig:parabola_rotada}
\end{figure}
