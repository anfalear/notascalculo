...
\begin{definition}[Curva plana paramétrica]\index{curva parametrica@curva paramétrica}
...
\begin{theorem}[Derivada paramétrica]\index{curva parametrica@curva paramétrica!derivada}
...
La Figura~\ref{fig:param_tangente_nodo} muestra la curva, el nodo y la tangente.
...
\end{axis}
\end{tikzpicture}
\caption{Curva paramétrica con un nodo: la recta tangente en $(4,2)$ tiene pendiente $m=9/4$ y ecuación $y=\tfrac{9}{4}x-7$.}
\label{fig:param_tangente_nodo}
\end{figure}
...
La Figura~\ref{fig:param_area_bajo_eje} muestra la región.
...
\end{axis}
\end{tikzpicture}
\caption{Curva paramétrica situada íntegramente por debajo del eje $x$ entre $P_0(0,-5)$ y $P_1(8,-1)$: el área encerrada resulta con signo negativo, por lo que se toma su valor absoluto.}
\label{fig:param_area_bajo_eje}
\end{figure}
...
\begin{theorem}[Longitud de arco paramétrica]\index{longitud de arco!curva parametrica@curva paramétrica}
...
Para $x = R\cos t$, $y = R\sen t$, $t \in [0, 2\pi]$, calcule la longitud
...
La Figura~\ref{fig:param_circunferencia} muestra la circunferencia y el radio.
...
\addplot[thick, blue, domain=0:360, samples=200,
        ...
        ({cos(x)}, {sin(x)});
\addlegendentry{$C:\,(R\cos t,\;R\sen t)$}
% Radio ilustrativo
...
\end{axis}
\end{tikzpicture}
\caption{Circunferencia de radio $R$ centrada en el origen, parametrizada y recorrida en sentido antihorario al crecer $t$ (se ilustra con $R=1$).}
\label{fig:param_circunferencia}
\end{figure}

\textbf{Paso 2: Elemento diferencial de arco.}
Derivamos las funciones componentes respecto a $t$:
\[
  \frac{dx}{dt} = -R\sen t, \qquad \frac{dy}{dt} = R\cos t.
\]
El elemento de longitud de arco $ds = \sqrt{(dx/dt)^2 + (dy/dt)^2}\,dt$
se simplifica mediante la identidad pitagórica:
\[
  ds = \sqrt{R^2\sen^2 t + R^2\cos^2 t}\;dt
     = \sqrt{R^2\underbrace{(\sen^2 t + \cos^2 t)}_{=\,1}}\;dt
     = R\,dt.
\]
...
$x = a(t - \sen t)$, $y = a(1 - \cos t)$, $t \in [0, 2\pi]$.
...
La Figura~\ref{fig:cicloide} muestra un arco completo.
...
\end{axis}
\end{tikzpicture}
\caption{Un arco de cicloide: parte del origen en $t=0$, alcanza la cúspide $(\pi a,2a)$ en $t=\pi$ y regresa al eje $x$ en $(2\pi a,0)$ cuando $t=2\pi$.}
\label{fig:cicloide}
\end{figure}
...
\begin{align*}
  ...
  &= a^2(1-\cos t)^2 + a^2\sen^2 t \\
  &= a^2\bigl(1 - 2\cos t + \cos^2 t + \sen^2 t\bigr)
   = a^2(2 - 2\cos t)\\
  &= 2a^2(1 - \cos t).
\end{align*}
...
$1 - \cos t = 2\sen^2\!\tfrac{t}{2}$. Por tanto:
\[
  ds = \sqrt{2a^2 \cdot 2\sen^2\tfrac{t}{2}}\;dt
     = 2a\left|\sen\tfrac{t}{2}\right|dt.
\]
En $[0, 2\pi]$ se tiene $\sen(t/2) \geq 0$, con lo que el valor absoluto
desaparece.

\textbf{Paso 4: Evaluación.}
\begin{align*}
  L &= \int_{0}^{2\pi} 2a\sen\tfrac{t}{2}\;dt
...
\begin{definition}[Coordenadas polares]\index{coordenadas polares}
...
La Figura~\ref{fig:coordenadas_polares_punto} ilustra $r$ y $\theta$.
...
\begin{tikzpicture}[scale=2.2]
  ...
  \draw[dashed, gray!60]
    ...
    -- ({cos(40)},{sin(40)});
  \draw[dashed, gray!60]
    (0,{sin(40)}) node[left, black, font=\small] {$y=r\sen\theta$}
    -- ({cos(40)},{sin(40)});
...
\end{tikzpicture}
\caption{Coordenadas polares de un punto: el radio $r$ y el ángulo $\theta$ medido desde el eje polar, junto con las relaciones $x=r\cos\theta$, $y=r\sen\theta$.}
\label{fig:coordenadas_polares_punto}
\end{figure}
\end{definition}
...
La Figura~\ref{fig:cardioide} muestra la cardioide.
...
\end{axis}
\end{tikzpicture}
\caption{Cardioide $r=a(1+\cos\theta)$: curva cerrada con cúspide en el polo, radio máximo $r=2a$ en $\theta=0$ y simetría respecto al eje polar.}
\label{fig:cardioide}
\end{figure}
...
La Figura~\ref{fig:rosas_polares} compara ambos casos de paridad.
...
\end{axis}
\end{tikzpicture}
\caption{Rosas polares $r=\cos(n\theta)$: con $n$ impar se obtienen $n$ pétalos (aquí $r=\cos 3\theta$, 3 pétalos) y con $n$ par se obtienen $2n$ pétalos (aquí $r=\cos 2\theta$, 4 pétalos).}
\label{fig:rosas_polares}
\end{figure}
...
La Figura~\ref{fig:lemniscata} muestra los dos bucles.
...
\end{axis}
\end{tikzpicture}
\caption{Lemniscata $r^2=a^2\cos(2\theta)$: dos bucles simétricos definidos donde $\cos(2\theta)\geq 0$, con ancho máximo $r=a$ en $\theta=0$.}
...
\begin{align*}
  ...
  &= \frac{a^2}{2}\int_{-\pi/4}^{\pi/4}\cos(2\theta)\,d\theta
   = \frac{a^2}{2}\left[\frac{\sen 2\theta}{2}\right]_{-\pi/4}^{\pi/4}
   = \frac{a^2}{2}\cdot 1.
\end{align*}
\[
...
La Figura~\ref{fig:espiral_arquimedes} muestra la espiral.
...
\end{axis}
\end{tikzpicture}
\caption{Espiral de Arquímedes $r=a\theta$: a diferencia de las curvas cerradas, se aleja indefinidamente del polo; con $a=1/(2\pi)$ cada vuelta aumenta $r$ en una unidad.}
...
\begin{theorem}[Área de una región polar]\index{area@área!en coordenadas polares}
...
\begin{example}[Área de un pétalo de la rosa $r = 3\sen(3\theta)$]
Analice la curva polar $r = 3\sen(3\theta)$ y calcule el área de un solo
pétalo.
...
$\sen(3(\pi-\theta)) = \sen(3\pi - 3\theta) = -\sen(3\theta - 3\pi) = \sen(3\theta)$
...
El pétalo principal: $r = 0$ cuando $\sen(3\theta) = 0$, es decir,
$3\theta = 0$ y $3\theta = \pi$, luego $\theta \in [0, \pi/3]$.

La Figura~\ref{fig:rosa_area_petalo} muestra el pétalo cuya área se calcula.
...
\end{axis}
\end{tikzpicture}
\caption{Rosa $r=\sen(3\theta)$: el pétalo principal se genera al variar $\theta\in[0,\pi/3]$, intervalo donde $r\geq 0$; su área se calcula con $\tfrac12\int r^2\,d\theta$.}
\label{fig:rosa_area_petalo}
\end{figure}

\textbf{Paso 2: Elemento diferencial.}
\[
  dA = \frac{1}{2}r^2\,d\theta = \frac{1}{2}[3\sen(3\theta)]^2\,d\theta
     = \frac{9}{2}\sen^2(3\theta)\,d\theta.
\]

\textbf{Paso 3: Límites de integración.}
El pétalo principal corresponde a $\theta \in [0, \pi/3]$, intervalo donde
$r = 3\sen(3\theta) \geq 0$.

\textbf{Paso 4: Evaluación.}
Utilizamos la identidad $\sen^2 u = \frac{1-\cos 2u}{2}$ con $u = 3\theta$:
\begin{align*}
  A &= \int_{0}^{\pi/3} \frac{9}{2}\sen^2(3\theta)\,d\theta
     = \frac{9}{2}\int_{0}^{\pi/3} \frac{1 - \cos 6\theta}{2}\,d\theta \\[4pt]
    &= \frac{9}{4}\left[\theta - \frac{\sen 6\theta}{6}\right]_{0}^{\pi/3}\\[4pt]
    &= \frac{9}{4}\left[\left(\frac{\pi}{3} - \frac{\sen 2\pi}{6}\right)
...
$r = 1 + \cos\theta$ y $r = 1 - \sen\theta$, y plantee la integral para el
...
$r = 1 - \sen\theta$ apunta hacia abajo. Se cruzan en dos puntos visibles
y comparten el polo.

La Figura~\ref{fig:dos_cardioides_area} muestra ambas cardioides y sus cortes.
...
\end{axis}
\end{tikzpicture}
\caption{Dos cardioides, $r=1+\cos\theta$ (hacia la derecha) y $r=1-\sen\theta$ (hacia abajo): se cruzan en dos puntos y comparten el polo, lo que delimita la región común.}
\label{fig:dos_cardioides_area}
\end{figure}
...
Igualamos:
\[
  1 + \cos\theta = 1 - \sen\theta
  \implies \cos\theta + \sen\theta = 0
  \implies \tan\theta = -1.
\]
...
\begin{align*}
  ...
  &= \frac{1}{2}
     \left[\theta + 2\sen\theta
           + \frac{\theta}{2} + \frac{\sen 2\theta}{4}
...
\[
  ...
  \quad A_{\text{parcial}} = \tfrac{3\pi}{4} - \sqrt{2}}
\]
\end{myproof}
\end{example}
...
\begin{theorem}[Longitud de arco en coordenadas polares]\index{longitud de arco!en coordenadas polares}
...
\]
\begin{proof}
Usando las relaciones $x = r\cos\theta$, $y = r\sen\theta$ y diferenciando:
$x' = r'\cos\theta - r\sen\theta$, $y' = r'\sen\theta + r\cos\theta$.
Entonces
$(x')^2 + (y')^2 = (r')^2(\cos^2\theta + \sen^2\theta)
 + r^2(\sen^2\theta + \cos^2\theta) = (r')^2 + r^2$.
...
La Figura~\ref{fig:espiral_longitud_arco} muestra el arco medido.
...
\end{axis}
\end{tikzpicture}
\caption{Arco de espiral cuyo longitud se calcula: crecimiento lineal $r$ frente a $\theta$, desde el polo hasta el punto $(2\pi,0)$ en cartesianas tras una vuelta completa.}
\label{fig:espiral_longitud_arco}
\end{figure}
...
\begin{enumerate}
    ...
    \item $x = 4\sen t - 2,\quad y = 3\cos t + 1;\quad 0 \leq t \leq 2\pi$
\end{enumerate}
\end{prob}
...
\begin{prob}
La cicloide está definida paramétricamente por $x = t - \sen t$ e
...
\begin{enumerate}
    ...
    \item $x = \cos t + t\sen t,\quad y = \sen t - t\cos t,\quad
          0 \leq t \leq 2\pi$
\end{enumerate}
\end{prob}
...
\begin{enumerate}
    ...
    \item $r^2 - 6r(\cos\theta + \sen\theta) + 9 = 0$
\end{enumerate}
\end{prob}
...
\begin{enumerate}
    ...
    \item El área encerrada por un solo pétalo de la rosa polar
          $r = 3\sen(3\theta)$. Indique el número total de pétalos y
...
\begin{enumerate}
    \item Determine el área total de la región acotada por el cardioide
          $r = 5 - 5\cos\theta$.
    \item Encuentre los puntos de intersección de $r = 5\sen\theta$ y
          $r = 2 + \sen\theta$. Calcule el área de la región que se
          encuentra dentro del círculo $r = 5\sen\theta$ y fuera del
          caracol $r = 2 + \sen\theta$.
\end{enumerate}
\end{prob}
...
\begin{enumerate}
    ...
    \item El cardioide $r = a(1 - \cos\theta)$, para $0 \leq \theta \leq 2\pi$.
          ...
          $1 - \cos\theta = 2\sen^2(\theta/2)$ para resolver la raíz
