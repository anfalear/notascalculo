...
\begin{definition}\index{sistema de ecuaciones diferenciales}
...
\begin{example}[Eliminación con raíces reales distintas]
Resolver por eliminación el sistema $x'=2x+3y$, $y'=x+2y$.

\begin{myproof}
\textbf{Paso 1.} Despejar $y$ de la primera ecuación y derivar:
\[
y=\frac{x'-2x}{3},\qquad y'=\frac{x''-2x'}{3}.
\]

\textbf{Paso 2.} Sustituir en la segunda ecuación $y'=x+2y$ y simplificar:
\[
\frac{x''-2x'}{3}=x+2\cdot\frac{x'-2x}{3}\;\Longrightarrow\; x''-4x'+x=0.
\]

\textbf{Paso 3.} Ecuación característica $r^2-4r+1=0$, con raíces $r=2\pm\sqrt{3}$:
\[
x(t)=c_1 e^{(2+\sqrt{3})t}+c_2 e^{(2-\sqrt{3})t}.
\]

\textbf{Paso 4.} Recuperar $y=(x'-2x)/3$:
\[
\boxed{x(t)=c_1 e^{(2+\sqrt{3})t}+c_2 e^{(2-\sqrt{3})t},\quad y(t)=\tfrac{\sqrt{3}}{3}\big(c_1 e^{(2+\sqrt{3})t}-c_2 e^{(2-\sqrt{3})t}\big).}
\]
\end{myproof}
\end{example}
...
\]
Por tanto, $\lambda$ debe ser un valor propio de $A$ y $\mathbf{v}$ un vector propio asociado. Los métodos para calcular valores y vectores propios, junto con una primera aplicación de la diagonalización a estos sistemas, se desarrollaron en el Capítulo~\ref{vvpropios}.

\begin{theorem}[Estructura del espacio de soluciones]\label{thm:cap29:estructura}\index{sistema de ecuaciones diferenciales!estructura del espacio de soluciones}
El conjunto de soluciones del sistema homogéneo $\mathbf{X}'=A\mathbf{X}$, con $A\in\mathbb{R}^{n\times n}$ constante, es un \textbf{espacio vectorial de dimensión $n$}. Si $\mathbf{X}_1,\dots,\mathbf{X}_n$ son soluciones linealmente independientes, la \textbf{matriz fundamental} $\Phi(t)=[\,\mathbf{X}_1(t)\;\cdots\;\mathbf{X}_n(t)\,]$ reúne una base y toda solución se escribe $\mathbf{X}(t)=\Phi(t)\mathbf{c}$ para un único $\mathbf{c}\in\mathbb{R}^n$. Las soluciones son independientes si y solo si su wronskiano $\det\Phi(t)$ es no nulo (basta comprobarlo en un punto, por la fórmula de Abel).
\end{theorem}

\begin{rem}
Este resultado enlaza directamente con el álgebra lineal del Capítulo~\ref{vvpropios}: proponer soluciones $\mathbf{X}=e^{\lambda t}\mathbf{v}$ traduce el problema al \emph{espectro} de $A$. Los tres casos que siguen —valores propios reales distintos, complejos conjugados y repetidos— son exactamente las tres formas del espectro, y la diagonalizabilidad de $A$ decide si existe una base completa de vectores propios o hay que recurrir a vectores propios generalizados.
\end{rem}

\begin{theorem}[Solución general por vectores propios]\label{thm:cap29:solucion-general}\index{sistema de ecuaciones diferenciales!solucion por vectores propios@solución por vectores propios}
...
\begin{example}[Valores propios reales distintos (matriz simétrica)]
Resolver $\mathbf{X}'=\begin{pmatrix}2&1\\1&2\end{pmatrix}\mathbf{X}$.

\begin{myproof}
\textbf{Paso 1.} Valores propios: $\det(A-\lambda I)=(2-\lambda)^2-1=(\lambda-1)(\lambda-3)$, luego $\lambda_1=1$, $\lambda_2=3$.

\textbf{Paso 2.} Vectores propios: para $\lambda=1$, $(A-I)\mathbf{v}=0\Rightarrow v_1+v_2=0$, $\mathbf{v}_1=(1,-1)^T$; para $\lambda=3$, $-v_1+v_2=0$, $\mathbf{v}_2=(1,1)^T$.

\textbf{Paso 3.} Cada par $(\lambda_i,\mathbf{v}_i)$ produce una solución $e^{\lambda_i t}\mathbf{v}_i$; al ser independientes, forman un conjunto fundamental.

\textbf{Paso 4.} Solución general:
\[
\boxed{\mathbf{X}(t)=c_1 e^{t}\begin{pmatrix}1\\-1\end{pmatrix}+c_2 e^{3t}\begin{pmatrix}1\\1\end{pmatrix}.}
\]
\end{myproof}
\end{example}
...
\begin{example}[Valores propios complejos conjugados]
Resolver $\mathbf{X}'=\begin{pmatrix}1&-2\\2&1\end{pmatrix}\mathbf{X}$.

\begin{myproof}
\textbf{Paso 1.} $\det(A-\lambda I)=(1-\lambda)^2+4=0\Rightarrow \lambda=1\pm 2i$.

\textbf{Paso 2.} Vector propio para $\lambda=1+2i$: $(A-(1+2i)I)\mathbf{v}=0\Rightarrow -2i\,v_1-2v_2=0$, es decir $v_2=-i v_1$; tomamos $\mathbf{v}=(1,-i)^T=(1,0)^T+i(0,-1)^T$.

\textbf{Paso 3.} Con $\alpha=1$, la parte real y la imaginaria de $e^{(1+2i)t}\mathbf{v}$ proporcionan las dos soluciones reales independientes.

\textbf{Paso 4.} Solución general:
\[
\boxed{\mathbf{X}(t)=e^{t}\left(c_1\begin{pmatrix}\cos 2t\\ \sen 2t\end{pmatrix}+c_2\begin{pmatrix}-\sen 2t\\ \cos 2t\end{pmatrix}\right).}
\]
\end{myproof}
\end{example}
...
\begin{example}[Valor propio repetido defectivo (matriz 2x2)]
Resolver $\mathbf{X}'=\begin{pmatrix}5&1\\-1&3\end{pmatrix}\mathbf{X}$.

\begin{myproof}
\textbf{Paso 1.} $\det(A-\lambda I)=(5-\lambda)(3-\lambda)+1=\lambda^2-8\lambda+16=(\lambda-4)^2$: valor propio doble $\lambda=4$.

\textbf{Paso 2.} $(A-4I)\mathbf{v}=0\Rightarrow v_1+v_2=0$; hay un \emph{único} vector propio $\mathbf{v}=(1,-1)^T$ (matriz defectiva).

\textbf{Paso 3.} Vector propio generalizado: $(A-4I)\mathbf{w}=\mathbf{v}\Rightarrow w_1+w_2=1$; elegimos $\mathbf{w}=(1,0)^T$.

\textbf{Paso 4.} Solución general con la cadena $\{\mathbf{v},\mathbf{w}\}$:
\[
\boxed{\mathbf{X}(t)=e^{4t}\!\left(c_1\begin{pmatrix}1\\-1\end{pmatrix}+c_2\left(t\begin{pmatrix}1\\-1\end{pmatrix}+\begin{pmatrix}1\\0\end{pmatrix}\right)\right).}
\]
\end{myproof}
\end{example}

Los tres casos anteriores tienen una lectura geométrica en el \textbf{plano de fases}, donde cada solución se dibuja como una trayectoria $(x_1(t),x_2(t))$. El signo y la naturaleza de los valores propios determinan la forma del retrato (Figura~\ref{fig:plano_fases}): valores reales del mismo signo dan un \emph{nodo}, complejos conjugados un \emph{foco} (espiral), y reales de signo opuesto un \emph{punto silla}.
...
\draw[->,gray!60](-2.1,0)--(2.1,0); \draw[->,gray!60](0,-2.1)--(0,2.1);
\foreach \sx in {1,-1}{\foreach \sy in {1,-1}{
\draw[blue!70!black,thick,->] plot[domain=0:3,samples=40,variable=\t]({\sx*1.7*exp(-\t)},{\sy*1.7*exp(-2*\t)});
}}
\end{tikzpicture}\\[-2pt]
{\scriptsize (a) nodo estable\\ $\lambda_{1,2}<0$ reales}
\end{minipage}\hfill
\begin{minipage}{0.32\textwidth}
\centering
\begin{tikzpicture}[scale=0.85]
\draw[->,gray!60](-2.1,0)--(2.1,0); \draw[->,gray!60](0,-2.1)--(0,2.1);
\draw[blue!70!black,thick,->] plot[domain=0:900,samples=200,variable=\t]({1.9*exp(-0.002*\t)*cos(\t)},{1.9*exp(-0.002*\t)*sin(\t)});
\end{tikzpicture}\\[-2pt]
{\scriptsize (b) foco estable\\ $\lambda=\alpha\pm i\beta$, $\alpha<0$}
\end{minipage}\hfill
\begin{minipage}{0.32\textwidth}
\centering
\begin{tikzpicture}[scale=0.85]
\draw[->,gray!60](-2.1,0)--(2.1,0); \draw[->,gray!60](0,-2.1)--(0,2.1);
\foreach \s in {1,-1}{\foreach \u in {1,-1}{
\draw[blue!70!black,thick] plot[domain=-1.3:1.3,samples=40,variable=\t]({\s*0.45*exp(\t)},{\u*0.9*exp(-\t)});
}}
\draw[red!70!black,thick,->](0.15,0)--(1.95,0);
\draw[red!70!black,thick,->](-0.15,0)--(-1.95,0);
\draw[teal!70!black,thick,->](0,1.95)--(0,0.15);
\draw[teal!70!black,thick,->](0,-1.95)--(0,-0.15);
\end{tikzpicture}\\[-2pt]
{\scriptsize (c) punto silla\\ $\lambda_1<0<\lambda_2$}
\end{minipage}
\caption{Retratos de fase de $\mathbf{X}'=A\mathbf{X}$ según el espectro de $A$: (a) \emph{nodo} —valores propios reales del mismo signo, trayectorias que entran (o salen) del origen; (b) \emph{foco} —valores propios complejos, espiral; (c) \emph{punto silla} —valores propios reales de signo opuesto, con separatrices estable (verde, entra) e inestable (roja, sale).}
\label{fig:plano_fases}
\end{figure}
...
\begin{example}[Coeficientes indeterminados: forzamiento constante, espectro complejo]
Resolver $\mathbf{X}'=\begin{pmatrix}1&2\\-2&1\end{pmatrix}\mathbf{X}+\begin{pmatrix}0\\1\end{pmatrix}$.

\begin{myproof}
\textbf{Paso 1.} Homogénea: $\lambda=1\pm 2i$, luego $\mathbf{X}_h=e^{t}\left(c_1(\cos 2t,-\sen 2t)^T+c_2(\sen 2t,\cos 2t)^T\right)$.

\textbf{Paso 2.} Como el forzamiento es constante y $A$ es invertible, proponemos $\mathbf{X}_p=\mathbf{a}$ constante: $A\mathbf{a}+(0,1)^T=0$.

\textbf{Paso 3.} Resolver $a_1+2a_2=0$, $-2a_1+a_2=-1$: obtenemos $a_1=\tfrac{2}{5}$, $a_2=-\tfrac{1}{5}$.

\textbf{Paso 4.} Solución general $\mathbf{X}=\mathbf{X}_h+\mathbf{a}$:
\[
\boxed{\mathbf{X}(t)=e^{t}\!\left(c_1\begin{pmatrix}\cos 2t\\-\sen 2t\end{pmatrix}+c_2\begin{pmatrix}\sen 2t\\ \cos 2t\end{pmatrix}\right)+\begin{pmatrix}2/5\\-1/5\end{pmatrix}.}
\]
\end{myproof}
\end{example}
...
Consideremos dos masas $m_1,m_2$ unidas entre sí y a paredes fijas mediante resortes de constantes $k_1,k_2,k_3$, como se muestra en la Figura~\ref{fig:masas_acopladas}.
...
\end{tikzpicture}
\caption{Dos masas $m_1,m_2$ acopladas por tres resortes ($k_1,k_2,k_3$) entre paredes fijas. Los desplazamientos $x_1,x_2$ desde el equilibrio satisfacen un sistema lineal de segundo orden acoplado.}
\label{fig:masas_acopladas}
\end{figure}
...
\end{aligned}
\]
Si $L_1L_2-M^2\neq 0$, podemos despejar las derivadas y obtener un sistema de primer orden. La Figura~\ref{fig:dos_mallas} muestra las dos mallas con sus corrientes $i_1,i_2$ y el acoplamiento por inductancia mutua $M$.

\begin{figure}[H]
\centering
\begin{circuitikz}[american, scale=1.0, transform shape]
% malla izquierda
\draw (0,0) to[V, l=$E_1(t)$] (0,3)
      to[R, l=$R_1$] (2.6,3)
      to[L, l=$L_1$] (2.6,0) -- (0,0);
% malla derecha
\draw (2.6,3) to[R, l=$R_2$] (5.2,3)
      to[L, l=$L_2$] (5.2,1.4)
      to[V, l=$E_2(t)$] (5.2,0) -- (2.6,0);
% acoplamiento mutuo entre L1 y L2
\draw[<->,dashed,gray] (2.8,1.75)--(5.0,1.75);
\node[gray,fill=white,inner sep=1.5pt] at (3.9,1.75) {\small $M$};
% corrientes de malla
\node at (1.3,1.4) {\small $i_1$};
\draw[->,gray] ([shift=(30:0.45)]1.3,1.4) arc (30:310:0.45);
\node at (3.9,0.85) {\small $i_2$};
\draw[->,gray] ([shift=(30:0.42)]3.9,0.85) arc (30:310:0.42);
\end{circuitikz}
\caption{Circuito de dos mallas acopladas por inductancia mutua $M$. Las corrientes $i_1(t),i_2(t)$ satisfacen $L_1i_1'+R_1i_1+Mi_2'=E_1$ y $Mi_1'+L_2i_2'+R_2i_2=E_2$.}
\label{fig:dos_mallas}
\end{figure}
...
% Básico
\begin{prob}
Convierta la EDO $y''+3y'+2y=0$ en un sistema de primer orden de dos ecuaciones.
\end{prob}

\begin{prob}
Resolver $\mathbf{X}'=\begin{pmatrix}3&0\\0&-2\end{pmatrix}\mathbf{X}$.
\end{prob}

\begin{prob}
Resolver $\mathbf{X}'=\begin{pmatrix}1&2\\0&3\end{pmatrix}\mathbf{X}$.
\end{prob}

\begin{prob}
Clasificar el origen de $\mathbf{X}'=\begin{pmatrix}0&2\\-2&0\end{pmatrix}\mathbf{X}$.
\end{prob}

% Intermedio
\begin{prob}
Resolver por eliminación el sistema $x'=x+y$, $y'=4x+y$.
\end{prob}

\begin{prob}
Resolver $\mathbf{X}'=\begin{pmatrix}3&-2\\4&-1\end{pmatrix}\mathbf{X}$ (valores propios complejos).
\end{prob}

\begin{prob}
Resolver $\mathbf{X}'=\begin{pmatrix}3&-1\\1&1\end{pmatrix}\mathbf{X}$ (valor propio repetido).
\end{prob}

\begin{prob}
Resolver $\mathbf{X}'=\begin{pmatrix}-1&1\\0&-2\end{pmatrix}\mathbf{X}+\begin{pmatrix}2\\3\end{pmatrix}$ (coeficientes indeterminados).
\end{prob}

\begin{prob}
Resolver $\mathbf{X}'=\begin{pmatrix}1&0\\0&2\end{pmatrix}\mathbf{X}+\begin{pmatrix}e^{-t}\\ t\end{pmatrix}$.
\end{prob}

\begin{prob}
Resolver y clasificar el origen de $\mathbf{X}'=\begin{pmatrix}1&3\\3&1\end{pmatrix}\mathbf{X}$.
\end{prob}

\begin{prob}
Resolver y clasificar el origen de $\mathbf{X}'=\begin{pmatrix}-2&1\\1&-2\end{pmatrix}\mathbf{X}$.
\end{prob}

\begin{prob}
Clasificar el origen de $\mathbf{X}'=\begin{pmatrix}-1&-4\\1&-1\end{pmatrix}\mathbf{X}$.
\end{prob}

% Desafiante
\begin{prob}
Resolver el sistema $3\times 3$ $\mathbf{X}'=\begin{pmatrix}1&0&0\\0&0&-2\\0&1&0\end{pmatrix}\mathbf{X}$ (un valor propio real y un par complejo).
\end{prob}

\begin{prob}
Resolver por variación de parámetros $\mathbf{X}'=\begin{pmatrix}0&1\\-1&0\end{pmatrix}\mathbf{X}+\begin{pmatrix}\sec t\\0\end{pmatrix}$.
\end{prob}

\begin{prob}
Resolver $\mathbf{X}'=\begin{pmatrix}2&0\\0&2\end{pmatrix}\mathbf{X}+\begin{pmatrix}e^{2t}\\0\end{pmatrix}$ e identifique el efecto de la resonancia (la forzante coincide con el valor propio).
\end{prob}
