...
\begin{theorem}[Integrabilidad]\index{integral doble}\index{integral doble!integrabilidad}
...
\begin{theorem}[Fubini]\index{Fubini, teorema de}
...
\begin{definition}[Región tipo I ($y$-simple)]\index{region@región!tipo I}
...
\begin{definition}[Región tipo II ($x$-simple)]\index{region@región!tipo II}
...
\textbf{Paso 1. Integral interior en $y$.}
\[
\int_0^{\pi/2} \cos(x+y)\,dy = \bigl[\sen(x+y)\bigr]_0^{\pi/2} = \sen\!\left(x+\tfrac{\pi}{2}\right)-\sen x = \cos x - \sen x.
\]

\textbf{Paso 2. Integral exterior en $x$.}
\[
\int_0^{\pi/2}(\cos x - \sen x)\,dx = \bigl[\sen x + \cos x\bigr]_0^{\pi/2}
= (1+0)-(0+1) = 0.
\]
\textbf{Paso 3. Resultado.}
\[
...
\begin{tikzpicture}[scale=2.2]
  ...
  \draw[dashed,gray!80] (0.819,0.574)--(0,0.574)
    node[anchor=east,font=\scriptsize] {$r\sen\theta$};
  % Ángulo theta
  \draw[thin] (0.20,0) arc(0:35:0.20);
  \node[font=\small] at (0.30,0.08) {$\theta$};
\end{tikzpicture}
\caption{Coordenadas polares: el punto $P$ satisface $x=r\cos\theta$, $y=r\sen\theta$;
...
\[
\iint_D f(x,y)\,dA = \int_{\alpha}^{\beta}\int_{r_1(\theta)}^{r_2(\theta)} f(r\cos\theta,r\sen\theta)\,r\,dr\,d\theta.
\]
...
= \frac{1}{2}\left[\frac{3}{2}\theta+2\sen\theta+\frac{1}{4}\sen2\theta\right]_0^{2\pi}
...
\[
V=\int_0^{\pi/2}\int_0^1(2r\cos\theta+3r\sen\theta+1)\,r\,dr\,d\theta
=\int_0^{\pi/2}\int_0^1(2r^2\cos\theta+3r^2\sen\theta+r)\,dr\,d\theta.
\]

\textbf{Paso 2. Integral interior en $r$.}
\[
\int_0^1(2r^2\cos\theta+3r^2\sen\theta+r)\,dr
=\left[\frac{2r^3}{3}\cos\theta+r^3\sen\theta+\frac{r^2}{2}\right]_0^1
=\frac{2\cos\theta}{3}+\sen\theta+\frac{1}{2}.
\]

\textbf{Paso 3. Integral exterior en $\theta$.}
\[
V=\int_0^{\pi/2}\!\left(\frac{2\cos\theta}{3}+\sen\theta+\frac{1}{2}\right)d\theta
=\left[\frac{2\sen\theta}{3}-\cos\theta+\frac{\theta}{2}\right]_0^{\pi/2}
...
\]
\[
A=\frac{1}{2}\bigl[\theta+\sen2\theta\bigr]_{-\pi/3}^{\pi/3}
=\frac{1}{2}\!\left(\frac{2\pi}{3}+2\sen\frac{2\pi}{3}\right)
...
\begin{definition}[Transformación y jacobiano en $\mathbb{R}^2$]\index{jacobiano!en $\mathbb{R}^2$}
...
\begin{theorem}[Cambio de variables en integrales dobles]\index{cambio de variables!integrales dobles}
...
$\displaystyle\int_0^1\!\int_x^1 \sen(y^2)\,dy\,dx$.
\end{prob}
