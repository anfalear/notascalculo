...
\]
siempre que esta integral converja para algún valor de $s$. El resultado, denotado por $F(s)$ o $\mathcal{L}\{f\}$, es una función de la variable compleja $s$ (aunque en la práctica suele considerarse $s$ real y $s>a$ para cierta constante $a$).
\end{definition}
...
\begin{prob}[Convolución para inversa]
Hallar $\mathcal{L}^{-1}\left\{\frac{1}{(s^2+1)^2}\right\}$.
\begin{myproof}
\textbf{Paso 1.} $F(s)=G(s)=\frac{1}{s^2+1}$, $f=g=\sen t$.
\textbf{Paso 2.} Convolución: $\int_0^t \sen\tau \sen(t-\tau)d\tau$.
\textbf{Paso 3.} Usar identidad: $\frac{1}{2}\int_0^t [\cos(2\tau-t)-\cos t]d\tau = \frac{1}{2}[\frac{\sen(2\tau-t)}{2} - \tau\cos t]_0^t$.
Evaluando: $\frac{1}{2}[\frac{\sen t}{2} - t\cos t - (-\frac{\sen(-t)}{2})] = \frac{1}{2}[\frac{\sen t}{2} - t\cos t + \frac{\sen t}{2}] = \frac{1}{2}[\sen t - t\cos t]$.
\[
\boxed{\frac{\sen t - t\cos t}{2}.}
\]
\end{myproof}
\end{prob}
...
\begin{prob}[PVI con forzamiento definido por tramos]
Resolver $y'' + 4y = \begin{cases}0,& t<1\\ 2,& t\ge1\end{cases}$, $y(0)=0,\ y'(0)=0$.
\begin{myproof}
\textbf{Paso 1.} $F(s)=2e^{-s}/s$. $(s^2+4)Y = 2e^{-s}/s \Rightarrow Y = 2e^{-s}\frac{1}{s(s^2+4)}$.
\textbf{Paso 2.} $\frac{1}{s(s^2+4)} = \frac{1/4}{s} - \frac{1}{4}\frac{s}{s^2+4}$.
\textbf{Paso 3.} $y(t)=\frac{1}{2}u(t-1)(1 - \cos(2(t-1)))$.
\[
\boxed{\frac{1}{2}u(t-1)(1-\cos(2t-2)).}
\]
\end{myproof}
\end{prob}

\begin{prob}[Sistema 2×2 homogéneo]
Resolver $x' = x + y$, $y' = 4x + y$, con $x(0)=1,\ y(0)=0$.
\begin{myproof}
\textbf{Paso 1.} $(s-1)X - Y = 1$, $-4X + (s-1)Y = 0$.
\textbf{Paso 2.} $Y = \frac{4}{s-1}X$. Sustituir: $(s-1)X - \frac{4}{s-1}X = 1 \Rightarrow X\frac{(s-1)^2-4}{s-1} = 1 \Rightarrow X = \frac{s-1}{s^2-2s-3} = \frac{s-1}{(s-3)(s+1)}$.
$Y = \frac{4}{(s-3)(s+1)}$.
\textbf{Paso 3.} Descomponer $X$: $\frac{s-1}{(s-3)(s+1)} = \frac{A}{s-3}+\frac{B}{s+1}$. $s-1 = A(s+1)+B(s-3)$. $s=3$: $2=4A\Rightarrow A=1/2$. $s=-1$: $-2=-4B\Rightarrow B=1/2$. $X = \frac{1/2}{s-3}+\frac{1/2}{s+1}$.
$Y = \frac{4}{(s-3)(s+1)} = \frac{C}{s-3}+\frac{D}{s+1}$. $4 = C(s+1)+D(s-3)$. $s=3$: $4=4C\Rightarrow C=1$. $s=-1$: $4=-4D\Rightarrow D=-1$.
\textbf{Paso 4.} $x(t)=\frac{1}{2}e^{3t}+\frac{1}{2}e^{-t}$, $y(t)=e^{3t}-e^{-t}$.
\[
\boxed{x(t)=\frac{1}{2}(e^{3t}+e^{-t}),\quad y(t)=e^{3t}-e^{-t}.}
\]
\end{myproof}
\end{prob}
