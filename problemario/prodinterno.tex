...
\textbf{b)} Vectores:
- \( \overrightarrow{BA} = (-3, -10, -11) \), \( \overrightarrow{AC} = (-3, -4, 0) \).
- Proyección de \( \overrightarrow{BA} \) sobre \( \overrightarrow{AC} \):
\[
...
\]
Ecuaciones simétricas de la recta que pasa por \(B=(11,17,22)\) con dirección \(\mathbf{h}\):
\[
...
\]
Las filas son linealmente independientes, así que rango \( = 3 \). Por el teorema de rango-nulidad, nulidad \( = 4-3 = 1 \).

\textbf{b)} Complemento ortogonal del espacio fila es el espacio nulo de \( A \). Resolviendo \( A\mathbf{x} = 0 \):
\[
\begin{cases}
x_1 + x_3 + x_4 = 0 \\
x_2 + x_3 = 0 \\
...
\textbf{b)} Aplicamos Gram-Schmidt a \( h_1 = 1+2x^2 \), \( h_2 = 2+x+x^2 \):
\[
v_1 = h_1 = 1+2x^2.
\]
\[
...
\end{enumerate}
\begin{myproof}
\textbf{a)} Resolviendo el sistema:
...
\textbf{b)} \( W^\perp \) es el espacio fila de la matriz del sistema:
...
\textbf{c)} Proyección de \( \mathbf{u}=(5,6,7,2) \) sobre \( W \):
\[
...
\textbf{d)} \( \mathbf{u} - \operatorname{proy}_W \mathbf{u} = (5,6,7,2) - (0,-1,1,1) = (5,7,6,1) \).
\[
\boxed{(5,7,6,1)}.
\]

\textbf{e)} \( \|(5,7,6,1)\| = \sqrt{25+49+36+1} = \sqrt{111} \).
\[
\boxed{\sqrt{111}}.
\]
\end{myproof}
...
\end{enumerate}
\begin{myproof}
\textbf{a)} Resolvemos el sistema. De las dos primeras: restamos la primera de la segunda: \( (2x_1+x_2-x_3+x_4) - (x_1-x_2+x_3+x_4) = x_1+2x_2-2x_3 = 0 \Rightarrow x_1 = -2x_2+2x_3 \).  
...
\textbf{b)} \( W^\perp \) es el espacio fila de la matriz de coeficientes. Para obtener base ortogonal, aplicamos Gram-Schmidt a las filas \( r_1=(1,-1,1,1), r_2=(2,1,-1,1), r_3=(4,-1,1,3) \). Pero como el espacio fila tiene dimensión 2 (el sistema tiene rango 2, pues las filas son dependientes), podemos tomar dos filas independientes y luego ortogonalizar. Tomamos \( r_1 \) y \( r_2 \):
...
\textbf{c)} Proyección de \( \mathbf{u}=(9,4,5,2) \) sobre \( W^\perp \):
...
\textbf{d)} \( \mathbf{u} - \operatorname{proy}_{W^\perp}\mathbf{u} = \mathbf{u} - \frac{1}{27}(193,-1,1,129) = \frac{1}{27}(243-193, 108+1, 135-1, 54-129) = \frac{1}{27}(50,109,134,-75) \).
Su norma es:
\[
...
% --- Norma, ángulo y desigualdad de Cauchy-Schwarz ---
...
% --- Ortogonalidad y complemento ortogonal ---
