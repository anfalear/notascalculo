...
\begin{definition}[Espacio vectorial con producto interno]\index{producto interno}
...
\begin{definition}[Norma inducida]\index{norma inducida}
...
\begin{definition}[Bola unitaria]\index{bola unitaria}
...
\begin{definition}[Producto interno en espacios vectoriales complejos]\index{producto interno!complejo}
...
\begin{theorem}[Propiedades del producto interno en espacios vectoriales complejos]\index{producto interno!propiedades (complejo)}
...
\begin{definition}[Ortogonalidad y ángulo]\index{ortogonalidad}
...
\begin{example}[Ortogonalidad de $\cos x$ y $\sen x$ en ${\mathcal{C}[0,\pi]}$]
Calcule $\langle \cos x, \sen x \rangle$ en $\mathcal{C}[0, \pi]$ con $\langle f, g \rangle = \int_0^\pi f(x)g(x)\,dx$ y determine si $f \perp g$.
\begin{myproof}
\textbf{Paso 1. Evaluar el producto interno.}
\[
\langle \cos x, \sen x \rangle = \int_0^\pi \sen x \cos x\,dx = \left[ \frac{\sen^2 x}{2} \right]_0^\pi = \frac{\sen^2 \pi}{2} - \frac{\sen^2 0}{2} = 0.
\]

\textbf{Paso 2. Concluir ortogonalidad.} Como $\langle \cos x, \sen x \rangle = 0$, los vectores son ortogonales.
$$\boxed{\langle \cos x,\, \sen x \rangle = 0,\quad \cos x \perp \sen x \text{ en } \mathcal{C}[0,\pi].}$$
\end{myproof}
\end{example}


\begin{theorem}[Desigualdad de Cauchy-Schwarz]\label{thm:cap07:cauchy-schwarz}\index{Cauchy-Schwarz, desigualdad de}
...
\begin{theorem}[Consecuencias de Cauchy-Schwarz]\index{Cauchy-Schwarz, desigualdad de!consecuencias}
...
\begin{theorem}[Teorema de Pitágoras]\index{Pitagoras, teorema de@Pitágoras, teorema de}
...
\begin{definition}[Complemento ortogonal]\index{complemento ortogonal}
...
\begin{theorem}[Propiedades del complemento ortogonal]\index{complemento ortogonal!propiedades}
...
\begin{theorem}[Relaciones fundamentales para una matriz]\index{espacios fundamentales!relaciones fundamentales (producto interno)}
...
\begin{definition}[Conjunto ortogonal y ortonormal]\index{conjunto ortogonal y ortonormal}
...
\begin{theorem}[Expansión en base ortogonal]\index{base (espacio vectorial)!ortogonal, expansion@ortogonal, expansión}
...
\begin{theorem}[Descomposición ortogonal]\index{descomposicion ortogonal@descomposición ortogonal}
...
\begin{theorem}[Proceso de Gram-Schmidt]\label{thm:cap07:gram-schmidt}\index{Gram-Schmidt, proceso de}
...
\begin{definition}[Aproximación por mínimos cuadrados]\index{minimos cuadrados@mínimos cuadrados}
...
\begin{theorem}[Mejor aproximación]\index{minimos cuadrados@mínimos cuadrados!mejor aproximacion@mejor aproximación}
...
\begin{theorem}[Ecuaciones normales]\index{minimos cuadrados@mínimos cuadrados!ecuaciones normales}
...
\begin{theorem}[Solución única]\index{minimos cuadrados@mínimos cuadrados!solucion unica@solución única}
...
\begin{definition}[Serie de Fourier]\index{serie de Fourier}
Para $f$ periódica en $[0, 2\pi]$, con producto interno $\langle f, g \rangle = \int_0^{2\pi} f(x) g(x)  dx$:
\[
f(x) \approx a_0 + \sum_{k=1}^n (a_k \cos kx + b_k \sen kx)
\]
donde:
\begin{align*}
a_0 &= \frac{1}{2\pi} \int_0^{2\pi} f(x)  dx \\
a_k &= \frac{1}{\pi} \int_0^{2\pi} f(x) \cos kx  dx \\
b_k &= \frac{1}{\pi} \int_0^{2\pi} f(x) \sen kx  dx
\end{align*}
\end{definition}
...
\end{enumerate}
\begin{myproof}
\textbf{Paso 1. Estrategia.} Las tres funciones son polinomios, así que basta aplicar las fórmulas de la definición. Como todos los integrandos se reducen a combinaciones de $x^j\cos kx$ y $x^j\sen kx$ con $j \leq 2$, calculamos una vez por partes las integrales auxiliares sobre $[0,2\pi]$, válidas para todo entero $k \geq 1$:
\[
\int_0^{2\pi}\! \cos kx\,dx = \int_0^{2\pi}\! \sen kx\,dx = 0, \qquad
\int_0^{2\pi}\! x\cos kx\,dx = 0, \qquad
\int_0^{2\pi}\! x\sen kx\,dx = -\frac{2\pi}{k},
\]
\[
\int_0^{2\pi}\! x^2\cos kx\,dx = \frac{4\pi}{k^2}, \qquad
\int_0^{2\pi}\! x^2\sen kx\,dx = -\frac{4\pi^2}{k}.
\]
...
\]
\[
f(x) \sim \pi - 2 \sum_{k=1}^{\infty} \frac{\sen(kx)}{k}
\]

\textbf{Paso 3. Parte $b)$: $f(x)=8$.} Al ser constante, $a_0 = \frac{1}{2\pi}\cdot 8 \cdot 2\pi = 8$, mientras que $a_k$ y $b_k$ son múltiplos de $\int_0^{2\pi}\cos kx\,dx$ y $\int_0^{2\pi}\sen kx\,dx$, ambas nulas. Luego $a_k = b_k = 0$ y la serie se reduce a su término constante:
\[
f(x) \sim 8
\]
...
\boxed{
\begin{aligned}
a)\ & f(x) \sim \pi - 2 \sum_{k=1}^{\infty} \frac{\sen(kx)}{k} \\
b)\ & f(x) \sim 8 \\
c)\ & f(x) \sim \left( \frac{4\pi^2}{3} + 2\pi + 1 \right) + \sum_{k=1}^{\infty} \left[ \frac{4}{k^2} \cos(kx) - \frac{4(\pi+1)}{k} \sen(kx) \right]
\end{aligned}
}
\]
\end{myproof}
...
\begin{theorem}[Fórmula de Euler]\label{demformeuler}\index{Euler, formula de@Euler, fórmula de}
...
\end{align*}
Se verifica $e^{ix} = \cos x + i \sen x$.
\end{proof}
\end{theorem}
...
\textbf{Paso 3. Coeficientes $b_k$.} Con $\int_0^{2\pi} x\sen kx\,dx = -\frac{2\pi}{k}$ y $\int_0^{2\pi} x^2\sen kx\,dx = -\frac{4\pi^2}{k}$:
\[
...
\textbf{Paso 4. Serie.}
\[
\boxed{f(x) \sim (12\pi^2 + 4\pi) + \sum_{k=1}^{\infty} \left[ \frac{36}{k^2} \cos(kx) - \frac{36\pi + 8}{k} \sen(kx) \right]}
\]
\end{myproof}
\end{prob}
...
\textbf{Paso 3. Coeficientes $b_k$.} Con $\int_0^{2\pi} \sen kx\,dx = 0$ y $\int_0^{2\pi} x\sen kx\,dx = -\frac{2\pi}{k}$:
\[
...
\textbf{Paso 4. Serie.}
\[
\boxed{f(x) \sim (9\pi + 7) - 18 \sum_{k=1}^{\infty} \frac{\sen(kx)}{k}}
\]
\end{myproof}
\end{prob}
...
\textbf{Paso 3. Coeficientes $b_k$.} Con $\int_0^{2\pi} x\sen kx\,dx = -\frac{2\pi}{k}$:
\[
...
\textbf{Paso 4. Serie.}
\[
\boxed{f(x) \sim (5\pi + 7) - 10 \sum_{k=1}^{\infty} \frac{\sen(kx)}{k}}
\]
\end{myproof}
\end{prob}
...
\textbf{Paso 4. Serie.}
\[
\boxed{f(x) \sim (7\pi + 2) - 14 \sum_{k=1}^{\infty} \frac{\sen(kx)}{k}}
\]
\end{myproof}
\end{prob}
