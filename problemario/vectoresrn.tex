...
\section{Vectores y operaciones básicas}

\begin{definition}[Suma y producto por escalar de vectores de manera geométrica]\label{defvectoresgeometrica}\index{vector!definicion geometrica@definición geométrica}
...
\begin{definition}[Vectores en términos de coordenadas]\label{def:vectorescoordenadas}\index{vector!en coordenadas}
...
\begin{definition}[Operaciones con vectores usando coordenadas]\index{vector!operaciones en coordenadas}
...
\label{fig:escalar_vector_r3}
\end{figure}
\end{enumerate}
\end{myproof}
\end{example}

\begin{example}[Componentes de un vector mediante trigonometría]\label{probvectrigo}
...
\label{fig:probvectrigo_datos}
\end{figure}

\columnbreak			

\begin{myproof}
\textbf{Paso 1. Componentes por trigonometría.} Ambos vectores tienen norma $1$; sus ángulos con el eje $x$ positivo son $30°$ y $240°$:

$\mathbf{u} = \cos 30°\,\mathbf{i} + \sen 30°\,\mathbf{j} = \frac{\sqrt{3}}{2}\mathbf{i} + \frac{1}{2}\mathbf{j}$

$\mathbf{v} = \cos 240°\,\mathbf{i} + \sen 240°\,\mathbf{j} = -\frac{1}{2}\mathbf{i} - \frac{\sqrt{3}}{2}\mathbf{j}$

\textbf{Paso 2. Suma y diferencia.} Operando componente a componente:

$\boxed{\mathbf{u}+\mathbf{v} = \frac{\sqrt{3}-1}{2}\mathbf{i} + \frac{1-\sqrt{3}}{2}\mathbf{j}}$
...
\section{Norma y vectores unitarios}

\begin{definition}[Norma de un vector]\index{norma de un vector}
...
\label{fig:norma_vector}
\end{figure}
\end{multicols}
\end{myproof}
\end{example}

\begin{definition}[Vector unitario]\index{vector!unitario}
...
\label{fig:vector_unitario}
\end{figure}
\end{multicols}
\end{myproof}
\end{example}

\section{Producto punto y ángulo entre vectores}

\begin{definition}[Producto punto]\index{producto punto}
...
\label{fig:angulo_vectores}
\end{figure}
\end{rem}


\begin{definition}[Ángulo entre vectores]\index{angulo entre vectores@ángulo entre vectores}
...
\begin{definition}[Producto cruz] \label{defprodcruz}\index{producto cruz}
...
\begin{definition}[Plano en el espacio]\index{plano en el espacio}
...
\begin{myproof}
\textbf{Paso 1. Forma vectorial.} Con $\vec{r_0}=(2,-1,3)$ y $\vec{n}=(4,-2,5)$:
\[
\vec{n} \cdot (\vec{r} - \vec{r_0}) = 0 \implies (4, -2, 5) \cdot (x-2, y+1, z-3) = 0
\]
\textbf{Paso 2. Desarrollo del producto punto.}
\[
4(x-2) - 2(y+1) + 5(z-3) = 0
\]
\[
...
\begin{example}[Geometría de un triángulo en el espacio]\label{probtrianguloperimetro}
...
\label{fig:probtriangulo_datos}
\end{figure}

\textbf{Paso 1. Estrategia y vectores base.} Todos los literales se resuelven a partir de los vectores que unen los vértices:
\begin{align*}
...
Perímetro: $\boxed{\sqrt{14} + \sqrt{35} + \sqrt{21}} \approx 14.240$
...
\theta_C &= \boxed{\arccos\left(\frac{21}{\sqrt{735}}\right)}
\end{align*}
...
Área = $\frac{1}{2}\|\overrightarrow{AB} \times \overrightarrow{AC}\| = \frac{1}{2}\sqrt{49 + 49 + 196} = \boxed{\frac{7\sqrt{6}}{2}} \approx 8.573$

\item Vector normal al plano: $\boxed{\mathbf{n} = (-7, -7, -14)}$
...
$$\boxed{\frac{x}{-7} = \frac{y}{-7} = \frac{z}{-14}}$$

\item Distancia del punto $(3,1,-2)$ al plano:


$d = \frac{|-7(3) - 7(1) - 14(-2) + 49|}{\sqrt{49 + 49 + 196}} = \frac{49}{7\sqrt{6}} = \boxed{\frac{7\sqrt{6}}{6}} \approx 2.858.$
\end{enumerate}
\end{multicols}
\end{myproof}
\end{example}
...
\begin{example}[Volumen de un paralelepípedo mediante el producto mixto]
Calcule el volumen del paralelepípedo de la figura.
...
\textbf{Paso 1. Estrategia.} El volumen del paralelepípedo es el valor absoluto del producto mixto de los tres vectores que lo definen.

Sean ${\bf u} = \begin{pmatrix} -1 \\ 2 \\ 4 \end{pmatrix},$  ${\bf v} = \begin{pmatrix} -5 \\ 1 \\ 0 \end{pmatrix}$ y ${\bf w} = \begin{pmatrix} 0 \\ 4 \\ 0 \end{pmatrix}$.

\textbf{Paso 2. Producto mixto.} Calculamos $\abs{({\bf u} \times {\bf v}) \cdot {\bf w}}$:
...
\textbf{Paso 3. Volumen.} Por tanto, el volumen del paralelepípedo es $V=\abs{-80}=\boxed{80}$ unidades cúbicas.
...
\begin{definition}[Proyección ortogonal]\index{proyeccion ortogonal@proyección ortogonal}
...
\end{tikzpicture}
    ...
    \label{fig:proyeccion_vectores}
\end{figure}
\end{definition}
\begin{example}
Calcule $\text{proy}_{\mathbf{v}}\mathbf{u}$ para $\mathbf{u} = (3, 4)$ y $\mathbf{v} = (5, 1)$.
\begin{myproof}
\textbf{Paso 1. Productos punto.} $\mathbf{u} \cdot \mathbf{v}= (3)(5) + (4)(1) = 15 + 4 = 19$ y $\mathbf{v} \cdot \mathbf{v} = (5)^2 + (1)^2 = 25 + 1 = 26.$

\textbf{Paso 2. Proyección.}
\[
\text{proy}_{\mathbf{v}}\mathbf{u} = \frac{\mathbf{u} \cdot \mathbf{v}}{\mathbf{v} \cdot \mathbf{v}}\,\mathbf{v} = \frac{19}{26}(5, 1) = \boxed{\left(\frac{95}{26}, \frac{19}{26}\right)}.
\]
\end{myproof}
\end{example}

\begin{theorem}[Propiedades de la proyección ortogonal]\index{proyeccion ortogonal@proyección ortogonal!propiedades} Dados dos vectores $\mathbf{u}$ y $\mathbf{v}$ se cumple que:
\begin{enumerate}
...
\begin{definition}[Recta en el espacio]\index{recta en el espacio}
...
\begin{myproof}
\textbf{Paso 1. Ecuación vectorial.} \(\vec{r} = (1, -2, 3) + t(2, -1, 4)\).

\textbf{Paso 2. Ecuaciones paramétricas.} \(
\begin{cases}
x = 1 + 2t \\
y = -2 - t \\
z = 3 + 4t
...
\textbf{Paso 3. Ecuaciones simétricas.} \( \boxed{\frac{x - 1}{2} = \frac{y + 2}{-1} = \frac{z - 3}{4}}
\).
\end{myproof}
...
\begin{example}[Recta perpendicular a otra mediante proyección ortogonal]\label{proyrn}
Halle las ecuaciones simétricas de la recta que pasa por el punto $(-2,1,-3)$ y es perpendicular a la recta $l_1=\{(7,4,1)+t(2,0,-4)\}$.
\begin{myproof}
...
\end{tikzpicture}
\caption{Construcción del vector director de la recta perpendicular pedida en el problema \ref{proyrn}: $\mathbf{u}-\text{proy}_{\mathbf{v}}\mathbf{u}$.}
\label{fig:proyrn_construccion}
\end{figure}

\columnbreak

\textbf{Paso 1. Estrategia.} Para construir la recta necesitamos un vector director y un punto. El punto dado es $A=(-2,1,-3)$.
...
\textbf{Paso 2. Cálculo del vector director.}
...
\textbf{Paso 3. Ecuaciones simétricas.} Multiplicando por $5$, las ecuaciones simétricas son $\boxed{\frac{x+2}{-44}=\frac{y-1}{-15}=\frac{z+3}{-22}}.$
\end{myproof}
\end{example}


\begin{example}[Reflexión de un rayo de luz]
(\cite{espinoza2006Algebralineal}, p. 87, Problema 78)
...
\end{scriptsize}
\end{tikzpicture}
\caption{Rayo incidente, normal a la superficie reflectante y rayo reflejado en el punto de incidencia.}
\label{fig:rayo_reflejado}
\end{figure}

\begin{myproof}
Para hallar la ecuación de la recta reflejada necesitamos un punto y un vector director. La Figura \ref{fig:rayo_reflejado} muestra la situación: el punto es el de incidencia sobre el plano, y el vector director se obtiene reflejando la dirección del rayo respecto de la normal.
...
\textbf{Paso 2.} Sea $\mathbf{u}$ el vector director de la recta en sentido contrario, $\mathbf{n}$ el vector normal al plano y $\mathbf{v}$ el vector director del rayo reflejado. Como se aprecia en la Figura \ref{fig:rayo_construccion_vector}, al descomponer $\mathbf{u}$ en su proyección sobre $\mathbf{n}$ y su componente ortogonal, el reflejado conserva la primera e invierte la segunda:
\begin{equation}
...
\end{tikzpicture}
\caption{Construcción del vector director $\mathbf{v}$ del rayo reflejado a partir de $\operatorname{proy}_{\mathbf{n}}\mathbf{u}$ y de la componente de $\mathbf{u}$ ortogonal a $\mathbf{n}$.}
\label{fig:rayo_construccion_vector}
\end{figure}
...
% --- Vectores: operaciones, proyecciones y valor de verdad ---
...
\begin{myproof}
\textbf{Paso 1. Coordenadas.} Reescribiendo en coordenadas: $\mathbf{u} = (3,-3,1)$, $\mathbf{v} = (-2,1,-6)$, $\mathbf{w} = (-1,0,5)$.

\textbf{Paso 2. Cálculo por literal.}
\begin{enumerate}[$(a)$]
\item $\mathbf{u} + 5\mathbf{v} - \mathbf{w} = (3,-3,1) + 5(-2,1,-6) - (-1,0,5) = \boxed{(-6,2,-34)}$

\item $\|\mathbf{u} + \mathbf{w}\| = \|(2,-3,6)\| = \sqrt{4 + 9 + 36} = \boxed{7}$

\item $\|\mathbf{u}\| + \|\mathbf{w}\| = \boxed{\sqrt{19} + \sqrt{26}}$

\item $\frac{\mathbf{u}}{\|\mathbf{u}\|} = \frac{(3,-3,1)}{\sqrt{19}} = \boxed{\left(\frac{3}{\sqrt{19}}, -\frac{3}{\sqrt{19}}, \frac{1}{\sqrt{19}}\right)}$

\item De $2\mathbf{u} - \mathbf{v} + \mathbf{x} = 4\mathbf{x} + \mathbf{w}$:
$$3\mathbf{x} = 2\mathbf{u} - \mathbf{v} - \mathbf{w}$$
$$\mathbf{x} = \frac{1}{3}(9,-7,3) = \boxed{\left(3, -\frac{7}{3}, 1\right)}$$

\item $\mathbf{u} \cdot \mathbf{v} = 3(-2) + (-3)(1) + 1(-6) = \boxed{-15}$
$$\mathbf{u} \times \mathbf{v} = \begin{vmatrix}
\mathbf{i} & \mathbf{j} & \mathbf{k} \\
3 & -3 & 1 \\
-2 & 1 & -6
\end{vmatrix} = \boxed{(17,16,-3)}$$

\item $\cos(\theta) = \frac{-15}{\sqrt{19}\sqrt{41}}$, $\theta = \boxed{\arccos\left(\frac{-15}{\sqrt{19}\sqrt{41}}\right) \approx 2.138}$ rad

\item $\mathbf{u} \times \mathbf{w} = \boxed{(-15,-16,-3)}$

\item $\text{proy}_{\mathbf{v}}\mathbf{u} = \frac{\mathbf{u} \cdot \mathbf{v}}{\|\mathbf{v}\|^2}\mathbf{v} = \frac{-15}{41}(-2,1,-6) = \boxed{\left(\frac{30}{41}, -\frac{15}{41}, \frac{90}{41}\right)}$

\item $\mathbf{s} \cdot \mathbf{v} = 0 \Rightarrow -2 - \alpha - 18 = 0 \Rightarrow \boxed{\alpha = -20}$

\item Para paralelismo: $\mathbf{s} = k\mathbf{v}$. Comparando componentes: $\boxed{\alpha = \frac{1}{2}}$
\end{enumerate}
\end{myproof}
\end{prob}
...
\begin{myproof}
\textbf{Paso 1. Estrategia.} Para refutar una afirmación basta un contraejemplo; para las verdaderas aplicamos las definiciones de norma, producto punto y ángulo.
...
\[
\boxed{a)\ \text{F},\qquad b)\ \text{V},\qquad c)\ \text{F},\qquad d)\ \text{F}.}
\]
\end{myproof}
\end{prob}

% --- Rectas, planos y geometría del triángulo ---
...
\begin{myproof}
\textbf{Paso 1. Estrategia.} $\overrightarrow{AB}$ es normal al plano mediatriz y director de la recta $L$; el simétrico se construye con proyecciones y la distancia con la fórmula punto-plano.

\textbf{Paso 2. Parte (a).} El vector $\overrightarrow{AB}=(-4,-2,-4)$ es normal al plano mediatriz. El punto medio del segmento es $(0,2,-2)$.
...
Por tanto, $\boxed{a=2,\ b=1,\ c=2}$.

\textbf{Paso 3. Parte (b).} El vector $\overrightarrow{AB}=(-4,-2,-4)$ es perpendicular al plano, así que las ecuaciones simétricas de la recta que pasa por el origen son:
\begin{equation}
\frac{x}{-4}=\frac{y}{-2}=\frac{z}{-4}
\end{equation}

\textbf{Paso 4. Parte (c).}
...
\end{tikzpicture}
\caption{Construcción del punto simétrico $S$ del punto $P(0,1,4)$ respecto a la recta $L$.}
\label{fig:mediatriz_simetrico}
\end{figure}
...
$$S=\text{proy}_{\overrightarrow{AB}} \mathbf{v}-(\mathbf{v}-\text{proy}_{\overrightarrow{AB}} \mathbf{v})=(2,1,2)-(-2,0,2)=\boxed{(4,1,0)}$$

\textbf{Paso 5. Parte (d).} Usando la fórmula de distancia punto-plano con $-4x-2y-4z=4$ y el punto $(4,2,0)$:
$$d=\frac{|-4(4)-2(2)-4(0)-4|}{\sqrt{(-4)^2+(-2)^2+(-4)^2}}=\frac{|-24|}{\sqrt{36}}=\boxed{4}$$
\end{myproof} 
\end{prob}
...
\textbf{Paso 1. Estrategia.} La bisectriz del ángulo en $B$ tiene vector director $\mathbf{w}=\frac{\overrightarrow{BA}}{||\overrightarrow{BA}||}+\frac{\overrightarrow{BC}}{||\overrightarrow{BC}||}$ (Figura~\ref{fig:bisectriz_direccion}).

\begin{multicols}{2}

\textbf{Paso 2. Cálculos.}
...
\textbf{Paso 3. Resultado.} Multiplicando por $7$, la ecuación de la recta es:
\begin{equation}
\boxed{\frac{x-1}{-1}=\frac{y-2}{3}=\frac{z+7}{8}}
\end{equation}
...
\end{tikzpicture}
\caption{La dirección de la bisectriz en el vértice $B$ es la suma de los vectores unitarios $\frac{\overrightarrow{BA}}{\|\overrightarrow{BA}\|}$ y $\frac{\overrightarrow{BC}}{\|\overrightarrow{BC}\|}$.}
\label{fig:bisectriz_direccion}
\end{figure}
\end{multicols}
...
\textbf{Paso 1. Vector director de la perpendicular común.} Sean $\mathbf{u}=(2,5,1)$ y $\mathbf{v}=(-1,3,0)$ los vectores directores de $l_1$ y $l_2$ respectivamente. El vector director de la recta perpendicular es:
...
\textbf{Paso 2. Plano auxiliar.} Para encontrar un punto, construimos un plano que contenga $l_1$ y sea paralelo a $\mathbf{p}$. El vector normal del plano es:
...
\textbf{Paso 3. Intersección con $l_2$.} Sustituimos las ecuaciones paramétricas de $l_2$:
...
\textbf{Paso 4. Resultado.} Por tanto, la recta tiene ecuación:
$$\boxed{\left(\frac{768}{131},\; -\frac{208}{131},\; 2\right) + t(-3,-1,11)}$$
...
\textbf{Paso 1. El producto mixto como determinante.} Si $\mathbf{u}=\begin{pmatrix} u_1\\u_2\\u_3\end{pmatrix}$, $\mathbf{v}=\begin{pmatrix} v_1\\v_2\\v_3\end{pmatrix}$ y $\mathbf{w}=\begin{pmatrix} w_1\\w_2\\w_3\end{pmatrix}$, entonces: $\mathbf{u} \cdot (\mathbf{v} \times \mathbf{w})=\begin{vmatrix} u_1&u_2&u_3\\v_1&v_2&v_3\\w_1&w_2&w_3 \end{vmatrix}$

\textbf{Paso 2. ($\Rightarrow$).} Si $\mathbf{u}, \mathbf{v}$ y $\mathbf{w}$ son coplanares, entonces uno puede escribirse como combinación lineal de los otros dos. Sin pérdida de generalidad, sea $\mathbf{w}=k_1\mathbf{u}+k_2\mathbf{v}$ para algunos escalares $k_1, k_2$.
...
\textbf{Paso 3. ($\Leftarrow$).} Si $\mathbf{u} \cdot (\mathbf{v} \times \mathbf{w}) = 0$, entonces el determinante es cero, lo que implica que las filas son linealmente dependientes. Por tanto, uno de los vectores puede escribirse como combinación lineal de los otros dos, es decir, son coplanares.
...
\draw[->, black, ultra thick] (A) -- (C) node[midway, below left] {$\overrightarrow{AC}$};
\end{tikzpicture}
\caption{Triángulo con vértices $A(5,1,2)$, $B(2,8,4)$ y $C(1,-3,7)$ y los vectores $\overrightarrow{AB}$ y $\overrightarrow{AC}$.}
\label{fig:triangulo_euler_datos}
\end{figure}
...
\textbf{Paso 2. Solución por literal.}
\begin{enumerate}[a)]
...
\end{tikzpicture}
\caption{Mediana trazada desde $A$ hasta el punto medio $D$ del lado $\overline{BC}$.}
\label{fig:mediana_desde_A}
\end{figure}
\end{multicols}
...
\end{tikzpicture}
\caption{Mediana trazada desde $B$ hasta el punto medio $E$ del lado $\overline{AC}$.}
\label{fig:mediana_desde_B}
\end{figure}
\end{multicols}
...
\end{enumerate}
\begin{myproof}
\textbf{Paso 1. Estrategia.} Para refutar basta un contraejemplo; para las verdaderas aplicamos las definiciones y propiedades de la norma y el producto punto.
...
\textbf{Paso 3. Resumen.}
\[
\boxed{a)\ \text{V},\qquad b)\ \text{V},\qquad c)\ \text{sí es posible},\qquad d)\ \text{V},\qquad e)\ \text{V}.}
\]
\end{myproof}
...
\begin{myproof}
\textbf{Paso 1. Planteamiento.} Llamemos $\pi$ al plano buscado. Queremos encontrar su ecuación general \(
...
\textbf{Paso 2. Sistema para el vector normal.} Tenemos el sistema:
\[
\begin{cases}
2a - 2b + c = 0 \\
3a + b + 2c = 0
...
\textbf{Paso 3. Resolución.} De la primera ecuación:
\[
...
\textbf{Paso 4. Resultado.} Tomando $b = 1$ (cualquier múltiplo sirve), obtenemos \(a = 5,\quad b = 1,\quad c = -8 \) y con el valor \(d = -3c = -3(-8) = 24\) se obtiene la ecuación del plano 
\[
\boxed{5x + y - 8z = 24}
\]
\end{myproof}
...
\begin{myproof}
\textbf{Paso 1. Punto y dirección de la recta.} Queremos hallar la ecuación del plano que contiene al punto $P = (5,0,-4)$ y a la recta \(\frac{x-3}{-1} = \frac{y+5}{1} = \frac{z+2}{1}.\)
...
\textbf{Paso 2. Dos vectores en el plano.} Formamos dos vectores en el plano:
\[
...
\textbf{Paso 3. Vector normal.} El vector normal al plano es el producto cruz de $\vec{v}_1$ y $\vec{v}_2$:
\[
...
\textbf{Paso 4. Ecuación del plano.} La ecuación general del plano que pasa por $P$ y tiene vector normal $\vec{n} = (-7,0,-7)$ es:
\[
-7(x - 5) + 0(y - 0) - 7(z + 4) = 0
\]
\[
...
\begin{myproof}
\textbf{Paso 1. Forma paramétrica.} La recta que pasa por los puntos $A(-6,6,5)$ y $B(15,-6,1)$ puede escribirse en forma paramétrica como:
\[
\begin{cases}
x = -6 + (15 - (-6))t = -6 + 21t \\
y = 6 + (-6 - 6)t = 6 - 12t \\
...
\textbf{Paso 2. Intersección con el plano $x=0$.}
...
\textbf{Paso 3. Intersección con el plano $y=0$.}
\[
...
\textbf{Paso 4. Intersección con el plano $z=0$.}
\[
...
\textbf{Paso 5. Conclusión.} Los puntos de intersección de la recta con los planos coordenados son:
\[
\boxed{
\begin{aligned}
...
\textbf{Paso 1. Punto medio $M$ del lado $AB$.}
...
\textbf{Paso 2. Vector director de la mediana $CM$.}

\[
\vec{CM} = M - C = (-1 - 4,\ 4 - (-7),\ 0 - (-2)) = (-5,\ 11,\ 2)
\]

\textbf{Paso 3. Ecuaciones paramétricas de la mediana.}
...
\textbf{Paso 4. Ecuación simétrica de la mediana.}
...
\textbf{Paso 1. Vector director del lado $AC$.}
\[
\vec{AC} = C - A = (5-1,\ 1-(-2),\ -7-(-4)) = (4,\ 3,\ -3)
\]

\textbf{Paso 2. Pie de la altura.}
...
\textbf{Paso 1. Punto medio $M$ de $\overline{CA}$.}
\[
  M = \left(\frac{3+(-5)}{2},\; \frac{-1+14}{2},\; \frac{-1+(-3)}{2}\right)
    = \left(-1,\; \frac{13}{2},\; -2\right).
\]

\textbf{Paso 2. Vectores relevantes.}
...
\textbf{Paso 3. Proyección ortogonal de $\vec{w}$ sobre $\vec{u}$.}
...
\textbf{Paso 4. Componente de $\vec{w}$ perpendicular a $\vec{u}$.}
...
\textbf{Paso 5. Ecuaciones simétricas de la mediatriz.}
...
% Básico
\begin{prob}
Dados los vectores $\mathbf{u}=(1,2,-2)$ y $\mathbf{v}=(3,0,4)$, calcule:
\begin{multicols}{4}
\begin{enumerate}[$a)$]
\item $\mathbf{u}+2\mathbf{v}$
\item $\|\mathbf{u}\|$ y $\|\mathbf{v}\|$
\item Un vector unitario con la dirección de $\mathbf{v}$
...
\begin{prob}
Calcule el ángulo entre los vectores $\mathbf{u}=(1,1,0)$ y $\mathbf{v}=(0,1,1)$.
\end{prob}

\begin{prob}
Calcule $\mathbf{u}\times\mathbf{v}$ para $\mathbf{u}=(2,1,0)$ y $\mathbf{v}=(1,3,0)$. Verifique que el vector obtenido es perpendicular a $\mathbf{u}$ y a $\mathbf{v}$, y calcule el área del paralelogramo generado por ambos vectores.
\end{prob}

\begin{prob}
Halle la ecuación general del plano que pasa por los puntos $A(1,0,0)$, $B(0,2,0)$ y $C(0,0,3)$.
\end{prob}

\begin{prob}
Escriba las ecuaciones vectorial, paramétricas y simétricas de la recta que pasa por el punto $P(2,-1,3)$ y tiene vector director $\vec{d}=(4,1,-2)$.
\end{prob}

% Intermedio
\begin{prob}
Determine todos los valores de $a$ para los cuales los vectores $\mathbf{u}=(a,2,-1)$ y $\mathbf{v}=(a,-a,4)$ son ortogonales.
\end{prob}

\begin{prob}
Calcule el volumen del paralelepípedo cuyas aristas concurrentes están dadas por los vectores $\mathbf{u}=(1,2,0)$, $\mathbf{v}=(0,1,3)$ y $\mathbf{w}=(2,0,1)$.
\end{prob}

\begin{prob}
Determine el valor de $\lambda$ para el cual los vectores $(1,1,0)$, $(0,1,1)$ y $(1,0,\lambda)$ son coplanares.
\end{prob}

\begin{prob}
Calcule $\text{proy}_{\mathbf{v}}\mathbf{u}$ para $\mathbf{u}=(3,4,0)$ y $\mathbf{v}=(1,0,1)$, y verifique que $\mathbf{u}-\text{proy}_{\mathbf{v}}\mathbf{u}$ es perpendicular a $\mathbf{v}$.
\end{prob}

\begin{prob}
Calcule la distancia del punto $(1,2,3)$ al plano $2x-y+2z=4$.
\end{prob}

\begin{prob}
Calcule la distancia del punto $P(1,0,2)$ a la recta $l=\left\lbrace (0,1,0)+t(1,1,1) \right\rbrace$.
\end{prob}

\begin{prob}
Halle las ecuaciones paramétricas de la recta de intersección de los planos $x+y+z=1$ y $x-y+2z=0$.
\end{prob}

% Desafiante
\begin{prob}
Demuestre la identidad de Lagrange:
\[
\|\mathbf{u}\times\mathbf{v}\|^2=\|\mathbf{u}\|^2\|\mathbf{v}\|^2-(\mathbf{u}\cdot\mathbf{v})^2,
\]
y dedúzcase de ella que $\|\mathbf{u}\times\mathbf{v}\|=\|\mathbf{u}\|\|\mathbf{v}\|\sen\theta$, donde $\theta$ es el ángulo entre $\mathbf{u}$ y $\mathbf{v}$.
\end{prob}

\begin{prob}
Un rombo es un paralelogramo cuyos cuatro lados tienen la misma longitud. Use el producto punto para demostrar que las diagonales de un rombo son perpendiculares.
\end{prob}

\begin{prob}
Halle el punto simétrico del punto $P(3,1,2)$ respecto al plano $x+y+z=1$.
\end{prob}
