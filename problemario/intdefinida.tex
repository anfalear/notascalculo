...
La Figura~\ref{fig:exhaucion_circulo} ilustra el método sobre el círculo.
...
\end{tikzpicture}
\caption{Método de exhaución de Arquímedes: polígonos regulares inscritos en
el círculo (azul, $n=6$; rojo, $n=12$). Al crecer $n$ el área del polígono se
aproxima al área del círculo (negro) por defecto.}
\label{fig:exhaucion_circulo}
\end{figure}
...
\begin{theorem}[Fórmulas de sumatorias fundamentales]\index{sumatorias, formulas de@sumatorias, fórmulas de}
...
\begin{definition}\index{particion de un intervalo@partición de un intervalo}
...
Geométricamente, cada término $f(\bar{x}_i)\,\Delta x_i$ es el área con signo de un rectángulo de base $\Delta x_i$ y altura $f(\bar{x}_i)$: positiva si $f(\bar{x}_i) > 0$, negativa si $f(\bar{x}_i) < 0$. La Figura~\ref{fig:suma_riemann} muestra la construcción.
...
\end{tikzpicture}
\caption{Suma de Riemann con $n=7$ subintervalos, tomando como punto de
muestra el extremo derecho de cada uno. La suma de las áreas de los
rectángulos aproxima el área bajo la curva; al refinar la partición
($\|P\|\to 0$) la aproximación tiende a la integral definida.}
\label{fig:suma_riemann}
\end{figure}
...
\begin{definition}\index{integral definida}
...
\begin{theorem}[Integrabilidad de funciones continuas]\index{integral definida!integrabilidad de funciones continuas}
...
\end{tikzpicture}
\caption{Interpretación de la integral definida como \emph{área neta}: la
región sobre el eje $x$ (verde, $A^+$) aporta con signo positivo y la región
bajo el eje (roja, $A^-$) con signo negativo, de modo que
$\int_a^b f = A^+ - A^-$.}
\label{fig:area_neta}
\end{figure}
...
\begin{theorem}[Propiedades algebraicas]\index{integral definida!propiedades algebraicas}
...
\begin{theorem}[Aditividad en intervalos]\index{integral definida!aditividad en intervalos}
...
\begin{theorem}[Simetría para funciones pares e impares]\index{integral definida!simetria@simetría}
...
\begin{theorem}[Propiedad de acotamiento y comparación]\index{integral definida!acotamiento y comparacion@acotamiento y comparación}
...
\begin{definition}\index{funcion de acumulacion@función de acumulación}
...
\end{tikzpicture}
\caption{Función de acumulación $F(x)=\int_a^x f(t)\,dt$: el área sombreada
depende del extremo derecho móvil $x$. El TFC1 afirma que la razón a la que
crece esta área al desplazar $x$ es exactamente $f(x)$.}
\label{fig:funcion_acumulacion}
\end{figure}

\begin{theorem}[Primer Teorema Fundamental del Cálculo --- TFC1]\index{teorema fundamental del calculo@teorema fundamental del cálculo!primero}
...
\begin{proof}
Fijemos $x\in(a,b)$. Para $h\neq 0$ suficientemente pequeño, por la
aditividad de la integral,
\[
  F(x+h)-F(x)=\int_a^{x+h}f(t)\,dt-\int_a^{x}f(t)\,dt=\int_{x}^{x+h}f(t)\,dt.
\]
Como $f$ es continua en $x$, dado $\varepsilon>0$ existe $\delta>0$ tal que
$|f(t)-f(x)|<\varepsilon$ siempre que $|t-x|<\delta$. Entonces, para
$0<|h|<\delta$, usando que $\int_{x}^{x+h}f(x)\,dt=f(x)\,h$,
\[
  \left|\frac{F(x+h)-F(x)}{h}-f(x)\right|
  =\left|\frac{1}{h}\int_{x}^{x+h}\bigl(f(t)-f(x)\bigr)\,dt\right|
  \leq\frac{1}{|h|}\,|h|\,\varepsilon=\varepsilon.
\]
Como $\varepsilon>0$ es arbitrario, el límite del cociente incremental existe
y vale $f(x)$, es decir, $F'(x)=f(x)$.
\end{proof}
...
\begin{theorem}[Segundo Teorema Fundamental del Cálculo --- TFC2]\index{teorema fundamental del calculo@teorema fundamental del cálculo!segundo}
...
\begin{proof}
Sea $F(x)=\int_a^x f(t)\,dt$ la función de acumulación. Por el TFC1,
$F'(x)=f(x)$, de modo que $F$ es una antiderivada de $f$ en $[a,b]$. Si $G$ es
cualquier otra antiderivada, entonces $(G-F)'=f-f=0$ en $(a,b)$, y una función
con derivada nula en un intervalo es constante; luego $G(x)=F(x)+C$ para alguna
constante $C$. Evaluando en los extremos y restando, la constante se cancela:
\[
...
Las técnicas de integración del Capítulo~\ref{cap:tecintegracion} se
enunciaron para integrales indefinidas. La regla de sustitución admite una
versión directa para integrales definidas, que evita el paso de regresar a
la variable original.

\begin{theorem}[Regla de sustitución --- integral definida]\index{sustitucion, regla de@sustitución, regla de!integral definida}
\label{thm:sustitucion-definida}
...
\begin{definition}\index{valor promedio de una funcion@valor promedio de una función}
...
\begin{theorem}[Teorema del Valor Medio para Integrales]\index{teorema del valor medio!para integrales}
...
\end{tikzpicture}
\caption{Teorema del Valor Medio para Integrales: existe $c\in(a,b)$ tal que
el rectángulo de base $b-a$ y altura $f(c)=\bar f$ (verde) tiene exactamente
la misma área que la región bajo la curva, es decir
$f(c)(b-a)=\int_a^b f(x)\,dx$.}
\label{fig:tvm_integrales}
\end{figure}
...
\begin{theorem}[Segundo Teorema del Valor Medio para Integrales]\index{teorema del valor medio!segundo, para integrales}
