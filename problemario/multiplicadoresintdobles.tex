...
\begin{definition}[Extremos absolutos y locales]\index{extremos!en varias variables}
...
\begin{theorem}[Condición necesaria para extremo local]\index{extremos!condicion necesaria@condición necesaria}
...
\begin{definition}[Punto crítico]\index{punto critico@punto crítico}
...
\begin{theorem}[Teorema del Valor Extremo de Weierstrass]\index{Weierstrass!teorema del valor extremo}
...
\begin{definition}[Discriminante o Hessiano]\index{hessiano}
...
\begin{theorem}[Criterio de la segunda derivada]\index{extremos!criterio de la segunda derivada, varias variables}
...
%%==========================================================
\begin{prob}[Un continuo de máximos: cuando el criterio $D$ falla]
Determine y clasifique los puntos críticos de
$f(x,y)=(x^2+y^2)\,e^{-(x^2+y^2)}$.
\begin{myproof}
\textbf{Paso 1. Puntos críticos.}
Como $f$ depende de $x,y$ solo a través de $u=x^2+y^2$, conviene escribir
\[
f_x=2x\,e^{-u}(1-u),\qquad f_y=2y\,e^{-u}(1-u),\qquad u=x^2+y^2.
\]
Puesto que $e^{-u}>0$, el sistema $f_x=f_y=0$ se satisface cuando
\[
x(1-u)=0\quad\text{y}\quad y(1-u)=0.
\]
Hay dos posibilidades: el origen $(0,0)$, o bien $u=1$, es decir, \emph{toda}
la circunferencia $x^2+y^2=1$.

\textbf{Paso 2. El origen.}
Como $f(x,y)=u\,e^{-u}\ge 0$ para todo $(x,y)$ y $f(0,0)=0$, el origen es un
\textbf{mínimo absoluto}.

\textbf{Paso 3. La circunferencia: el criterio $D$ falla.}
Sobre $x^2+y^2=1$ el criterio de la segunda derivada es inconcluso ($D=0$):
los puntos críticos no están aislados, forman una curva completa, y el
hessiano es singular en la dirección tangente a ella. Se recurre a un
análisis radial.

\textbf{Paso 4. Análisis radial.}
Escribimos $f$ como función de $u=x^2+y^2\ge 0$:
\[
g(u)=u\,e^{-u},\qquad g'(u)=e^{-u}(1-u).
\]
Entonces $g'(u)>0$ para $0\le u<1$ y $g'(u)<0$ para $u>1$: $g$ crece hasta
$u=1$ y luego decrece hacia $0$. Por tanto $u=1$ da el máximo de $g$, con
valor $g(1)=e^{-1}$.
...
&\text{máximos absolutos en toda la circunferencia }x^2+y^2=1,\ \text{con }f=e^{-1}.
\end{aligned}\;}
\]
El valor máximo se alcanza en un conjunto infinito de puntos —una
circunferencia completa—, situación que el criterio de la segunda derivada,
pensado para puntos aislados, no puede diagnosticar.
\end{myproof}
\end{prob}
...
\begin{myproof}
\textbf{Paso 1. Función a minimizar.}
...
\textbf{Paso 3. Verificación mínimo (criterio 2da derivada).}
\begin{multicols}{3}
$$\dfrac{\partial^2 s}{\partial x^2}=4,$$ $$\dfrac{\partial^2 s}{\partial y^2}=4,$$ $$\dfrac{\partial^2 s}{\partial x\partial y}=-2.$$
\end{multicols}
$D=\left(4\right)\left(4\right)-\left(-2\right)^2=12>0$ y $\dfrac{\partial^2 s}{\partial x^2}=4>0$, luego es un mínimo.
El punto más cercano es $\left(\dfrac{8}{3},\dfrac{1}{3},\dfrac{20}{3}\right)$.

\textbf{Paso 4. Resultado.}
\[
\boxed{\text{Punto más cercano: }\left(\dfrac{8}{3},\,\dfrac{1}{3},\,\dfrac{20}{3}\right);\quad d_{\min}=\dfrac{5\sqrt{3}}{3}}
\]
\end{myproof}
\end{prob}
...
\draw (2.1419813147199327,1.936625586551849) node[anchor=north west] {$x \sen \theta$};
\draw (4.5,1.9216248599541543) node[anchor=north west] {$x \sen \theta$};
...
\draw[color=black] (6.8,0.38405038369046196) node[anchor=center] {$\theta$};
\draw[color=black] (1.3019406252490366,0.39905111028815654) node[anchor=center] {$\theta$};
\draw[color=black] (7.1,2.6) node[anchor=center] {$\theta$};
\draw[color=black] (0.7,2.6) node[anchor=center] {$\theta$};
\end{tikzpicture}
\caption{Sección transversal del canal abierto en forma de trapecio con ángulos iguales $\theta$ en la base; $x$ es la longitud del lado doblado.}
\label{fig:trapecio_canal}
\end{figure}
\textbf{Paso 1. Función de área.}
...
\begin{align*}
A(x,\theta)&=\dfrac{1}{2}\left[ 2x\cos \theta+(50-2x)+(50-2x) \right]x\sen \theta,\\
&=\left(x\cos \theta+(50-2x)\right)x\sen \theta,\\
&=x^2\cos\theta \sen \theta+50x\sen \theta-2x^2\sen \theta.
\end{align*}
...
\begin{itemize}
\item $\dfrac{\partial A}{\partial x}=2x\cos\theta\sen\theta +50\sen \theta -4x\sen \theta.$
\item $\dfrac{\partial A}{\partial \theta}=50 x \cos \theta -2 x^2\cos \theta +x^2\left(cos^{2}\theta-\sen^{2}\theta\right)=50 x \cos \theta -2 x^2\cos \theta +x^2\left(2cos^{2}\theta-1\right).$  
\end{itemize}
\textbf{Paso 3. Puntos críticos.}
Si $\dfrac{\partial A}{\partial x}=0$ entonces $\sen \theta \left( 2x\cos\theta +50-4x \right)=0.$ Como $\sen \theta=0$ si $\theta =0$ o $\theta =\pi$ se descartan esos puntos, por lo cual se tomará el caso cuando $2x\cos\theta +50-4x =0,$ lo cual implica que $\cos \theta=\dfrac{2x-25}{x}.$
...
\textbf{Paso 4. Resultado.}
\[
\boxed{(x,\theta)=\left(\dfrac{50}{3}\,\text{cm},\,\dfrac{\pi}{3}\right)\text{ maximiza el área del canal}.}
\]
\end{myproof}
\end{prob}
...
\begin{myproof}
\textbf{Paso 1. Variables y restricción de costo.}
Sea $x,\ y,\ z$ el ancho, largo y altura de la caja. El costo es $C=2xz+2yz+1.5xy=1000$, despejando $z=\dfrac{1000-1.5xy}{2x+2y}.$

\textbf{Paso 2. Función de volumen.}
$$V=xy\left(\dfrac{1000-1.5xy}{2x+2y}\right)=\dfrac{1000xy-1.5x^2y^2}{2x+2y}.$$
Las derivadas parciales son:
\begin{itemize}
\item $\dfrac{\partial V}{\partial x}=\dfrac{y^{2}\left(2000-3x^2-6xy\right)}{4\left(x+y\right)^{2}}.$
\item $\dfrac{\partial V}{\partial y}=\dfrac{x^{2}\left(2000-3y^2-6xy\right)}{4\left(x+y\right)^{2}}.$
\end{itemize}

\textbf{Paso 3. Puntos críticos.}
Las derivadas son iguales si $y=x$; sustituyendo: $0=\dfrac{x^2(2000-9x^2)}{16x^2}$, luego $x=\dfrac{20\sqrt{5}}{3}$ (el único positivo). Dimensiones: $x=y=\dfrac{20\sqrt{5}}{3}$, $z=5\sqrt{5}$.

\textbf{Paso 4. Resultado.}
\[
\boxed{V_{\max}\approx 2484.52\ \text{u}^3\text{; dimensiones: }x=y=\dfrac{20\sqrt{5}}{3},\;z=5\sqrt{5}}
\]
\end{myproof}
\end{prob}
...
\begin{myproof}
\textbf{Paso 1. Función a maximizar.}
Como la caja está en el primer cuadrante, el volumen es $V=xyz,$ pero se conoce que un vértice de esta se encuentra sobre el plano $x+5y+3z=6.$ Despejando a $z$ se obtiene la función $V=\dfrac{xy}{3}\left( 6-x-5y \right)$ la cual se va a maximizar.
...
\end{align*}
\end{multicols}
\textbf{Paso 3. Puntos críticos y discriminante.}
...
\textbf{Paso 4. Resultado.}
\[
\boxed{V_{\max}=\dfrac{8}{15}\ u^3\text{; punto óptimo: }\left(2,\,\tfrac{2}{5},\,\tfrac{4}{15}\right)}
\]
\end{myproof}
\end{prob}
...
\begin{myproof}
\textbf{Paso 1. Variables y función de costo.}
Sean $x,$ $y$ y $z$ las dimensiones del ancho, el largo y la altura del acuario. El costo es $C=3xy+2xz+2yz$; con $z=V/(xy)$ queda $C=3xy+\dfrac{2V}{y}+\dfrac{2V}{x}.$

\textbf{Paso 2. Derivadas parciales.}
$$\dfrac{\partial C}{\partial x}=3y-\dfrac{2V}{x^2},\qquad \dfrac{\partial C}{\partial y}=3x-\dfrac{2V}{y^2}.$$
\begin{multicols}{2}
...
\end{align*}
\end{multicols}
\textbf{Paso 3. Punto crítico y verificación.}
Resolviendo el sistema: $x=y=\sqrt[3]{\dfrac{2V}{3}}.$
...
Lo cual implica que $\left( \sqrt[3]{\dfrac{2V}{3}},\sqrt[3]{\dfrac{2V}{3}} \right)$ es un mínimo.

\textbf{Paso 4. Resultado.}
\[
\boxed{\text{Dimensiones óptimas: }x=y=\sqrt[3]{\dfrac{2V}{3}},\quad z=\sqrt[3]{\dfrac{9V}{4}}}
\]
\end{myproof}
\end{prob}
...
\begin{myproof}
\textbf{Paso 1. Reducción con $p+q+r=1$.}
Como $p=1-q-r$, la proporción se reduce a $P(q,r)=-2q^2-2r^2-2qr+2q+2r.$

\textbf{Paso 2. Puntos críticos.}
$\dfrac{\partial P}{\partial q}=-4q-2r+2=0$ y $\dfrac{\partial P}{\partial r}=-4r-2q+2=0$. El sistema tiene solución única $q=r=\dfrac{1}{3}$, luego $p=\dfrac{1}{3}$.

\textbf{Paso 3. Verificación máximo.}
$P_{qq}=-4$, $P_{rr}=-4$, $P_{qr}=-2$; $D=(-4)(-4)-(-2)^2=12>0$ y $P_{qq}<0$: es un máximo.

\textbf{Paso 4. Resultado.}
\[
\boxed{P\!\left(\dfrac{1}{3},\dfrac{1}{3},\dfrac{1}{3}\right)=\dfrac{2}{3}}
\]
\end{myproof}
\end{prob}
...
\begin{myproof}
\textbf{Paso 1. Función a minimizar.}
Con $z=\dfrac{12-2x-4y}{3}$ (de la restricción): $d^2=f(x,y)=x^2+y^2+\left(\dfrac{12-2x-4y}{3}\right)^2$.

\textbf{Paso 2. Puntos críticos.}
$\dfrac{\partial f}{\partial x}=0\Rightarrow 13x+8y=24$; $\dfrac{\partial f}{\partial y}=0\Rightarrow 8x+25y=48$. Solución: $\left(\dfrac{24}{29},\dfrac{48}{29}\right)$.

\textbf{Paso 3. Verificación mínimo.}
$f_{xx}=\dfrac{26}{9}$, $f_{yy}=\dfrac{50}{9}$, $f_{xy}=\dfrac{16}{9}$; $D=\dfrac{116}{9}>0$ y $f_{xx}>0$: mínimo.
$z=\dfrac{36}{29}$.

\textbf{Paso 4. Resultado.}
\[
\boxed{\text{Punto más cercano: }\left(\dfrac{24}{29},\,\dfrac{48}{29},\,\dfrac{36}{29}\right)}
\]
\end{myproof}
\end{prob}
\begin{prob}
...
\begin{myproof}
\textbf{Paso 1. Puntos críticos interiores.}
$\dfrac{\partial T}{\partial x}=4x=0$ y $\dfrac{\partial T}{\partial y}=2y-1=0$ dan el único punto crítico interior $\left(0,\dfrac{1}{2}\right)$.

\textbf{Paso 2. Puntos críticos de frontera.}
Sobre $x^2+y^2=1$: $T(y)=-y^2-y+2$, $T'(y)=-2y-1=0\Rightarrow y=-\dfrac{1}{2}$. Candidatos: $(0,-1)$, $(0,1)$, $\left(\pm\dfrac{\sqrt{3}}{2},-\dfrac{1}{2}\right)$.

\textbf{Paso 3. Comparación de valores.}
 \begin{multicols}{3}
 \begin{itemize}
 \item $T\!\left(0,\tfrac{1}{2}\right)=-\tfrac{1}{4}.$
 \item $T\!\left(0,-1\right)=2.$
 \item $T\!\left(0,1\right)=0.$
 \item $T\!\left(\tfrac{\sqrt{3}}{2},-\tfrac{1}{2}\right)=\tfrac{9}{4}.$
 \item $T\!\left(-\tfrac{\sqrt{3}}{2},-\tfrac{1}{2}\right)=\tfrac{9}{4}.$
 \end{itemize}
\end{multicols}
...
\textbf{Paso 4. Resultado.}
\[
\boxed{\text{Máx.: }\left(\pm\dfrac{\sqrt{3}}{2},-\dfrac{1}{2}\right)\text{ con }T=\dfrac{9}{4};\quad\text{Mín.: }\left(0,\dfrac{1}{2}\right)\text{ con }T=-\dfrac{1}{4}}
\]
\end{myproof}
\end{prob}
...
\begin{definition}[Curva regular]\index{curva regular}
...
\begin{theorem}[Función implícita]\index{funcion implicita, teorema de la@función implícita, teorema de la}
...
\begin{theorem}[Multiplicadores de Lagrange, condición necesaria]\index{multiplicadores de Lagrange}
...
\begin{theorem}[Weierstrass]\index{multiplicadores de Lagrange!existencia de extremos}
...
\begin{tikzpicture}[scale=1.2,>=stealth]
  ...
  % curva y^3 = x^2  parametrizada como (t^3, t^2): ambas ramas, cuspide en el origen
  \draw[thick,blue,domain=-1.4:1.4,samples=200] plot ({\x*\x*\x},{\x*\x});
  \fill[red] (0,0) circle (2pt);
  \node[red,anchor=north] at (0,-0.08) {$(0,0),\ d_{\min}=0$};
  \node[blue,right] at (1.5,1.85) {$y^3=x^2$};
  \node[anchor=south west] at (-2.7,1.3) {$\nabla g(0,0)=\mathbf{0}$};
\end{tikzpicture}
...
\label{fig:cuspide_lagrange_falla}
\end{figure}
...
\begin{theorem}[Criterio del hessiano limitado]\index{hessiano!limitado}
...
\textbf{Paso 4. Resultado.}
Como $\mathcal{H}>0$, el candidato es un máximo condicionado:
\[
...
\textbf{Paso 4. Criterio y resultado.} Con $m=1$ restricción y $n=2$ variables,
...
\begin{example}[Paralelepípedo de volumen máximo inscrito en un elipsoide]
Halle el volumen máximo de una caja rectangular de caras paralelas a los
planos coordenados inscrita en el elipsoide
$\dfrac{x^2}{a^2}+\dfrac{y^2}{b^2}+\dfrac{z^2}{c^2}=1$.
\begin{myproof}
\textbf{Paso 1. Función y restricción.}
Por simetría, el vértice en el primer octante es $(x,y,z)$ con $x,y,z>0$ y la
caja tiene volumen $V=8xyz$, sujeto a
$g=\dfrac{x^2}{a^2}+\dfrac{y^2}{b^2}+\dfrac{z^2}{c^2}-1=0$.

\textbf{Paso 2. Ecuaciones de Lagrange.}
De $\nabla V=\lambda\nabla g$:
\[
  8yz=\lambda\frac{2x}{a^2},\qquad
  8xz=\lambda\frac{2y}{b^2},\qquad
  8xy=\lambda\frac{2z}{c^2}.
\]

\textbf{Paso 3. Simetría.}
Multiplicando la primera por $x$, la segunda por $y$ y la tercera por $z$, los
lados izquierdos coinciden ($=8xyz$), de modo que
\[
  \frac{x^2}{a^2}=\frac{y^2}{b^2}=\frac{z^2}{c^2}.
\]
Con la restricción, cada término vale $\tfrac13$, luego
$x=\dfrac{a}{\sqrt3}$, $y=\dfrac{b}{\sqrt3}$, $z=\dfrac{c}{\sqrt3}$.

\textbf{Paso 4. Volumen máximo.}
\[
  \boxed{\ V_{\max}=8\cdot\frac{a}{\sqrt3}\cdot\frac{b}{\sqrt3}\cdot
  \frac{c}{\sqrt3}=\frac{8abc}{3\sqrt3}.\ }
\]
\end{myproof}
\end{example}

\subsection{Optimización con dos restricciones}

Cuando se busca un extremo de $f$ sobre la curva definida por \emph{dos}
restricciones $g=0$ y $h=0$, la condición necesaria involucra dos
multiplicadores.

\begin{theorem}[Multiplicadores de Lagrange con dos restricciones]\index{multiplicadores de Lagrange!dos restricciones}
Sea $f$ con un extremo en $P$ sobre la curva $g(\mathbf x)=0$,
$h(\mathbf x)=0$, y supóngase que $\nabla g(P)$ y $\nabla h(P)$ son
linealmente independientes. Entonces existen escalares $\lambda,\mu$ tales que
\[
  \nabla f(P)=\lambda\,\nabla g(P)+\mu\,\nabla h(P).
\]
\end{theorem}

\begin{example}[Punto más cercano al origen sobre la intersección de dos planos]
Halle el punto de la recta intersección de los planos $x+y+z=1$ y
$x-y+2z=2$ más cercano al origen.
\begin{myproof}
\textbf{Paso 1. Función y restricciones.}
Minimizamos $f=x^2+y^2+z^2$ con $g=x+y+z-1=0$ y $h=x-y+2z-2=0$.
...
\textbf{Paso 3. Sustituir en las restricciones.}
Con $x=\tfrac{\lambda+\mu}{2}$, $y=\tfrac{\lambda-\mu}{2}$,
$z=\tfrac{\lambda+2\mu}{2}$:
\[
  x+y+z=\frac{3\lambda+2\mu}{2}=1,\qquad
  x-y+2z=\frac{2\lambda+6\mu}{2}=2,
\]
esto es $3\lambda+2\mu=2$ y $\lambda+3\mu=2$, de donde $\lambda=\tfrac27$ y
$\mu=\tfrac47$.

\textbf{Paso 4. Punto y distancia.}
\[
  \boxed{\ \left(\tfrac37,\,-\tfrac17,\,\tfrac57\right),\qquad
  d=\sqrt{x^2+y^2+z^2}=\sqrt{\tfrac57}.\ }
\]
\end{myproof}
\end{example}
...
Halle el volumen máximo de una caja rectangular de caras paralelas a los
planos coordenados inscrita en el elipsoide
$\dfrac{x^2}{9}+\dfrac{y^2}{16}+\dfrac{z^2}{4}=1$.
\end{prob}

\begin{prob}[Dos restricciones]
Determine las dimensiones de la caja rectangular de volumen máximo cuya área
superficial es $64$ y la suma de las longitudes de sus $12$ aristas es $48$.
\end{prob}

\begin{prob}[Dos restricciones]
Halle los valores máximo y mínimo de $z$ sobre la curva intersección del plano
$x+y+z=1$ con el cilindro $x^2+y^2=1$.
\end{prob}
...
\begin{prob}[El centroide minimiza la suma de distancias al cuadrado]
Dados tres puntos $P_1=(1,2)$, $P_2=(5,4)$ y $P_3=(3,9)$, halle el punto
$(x,y)$ que minimiza la suma de los cuadrados de sus distancias a los tres
puntos,
\[
f(x,y)=\sum_{i=1}^{3}\bigl[(x-x_i)^2+(y-y_i)^2\bigr].
\]
Muestre que el punto crítico es el centroide
$(x,y)=\tfrac13(P_1+P_2+P_3)$ y que el criterio de la segunda derivada
confirma que se trata de un mínimo.
\end{prob}
