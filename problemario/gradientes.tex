...
\begin{theorem}[Regla de la cadena, Caso 1]\index{regla de la cadena!varias variables, caso 1}
...
\]
\begin{myproof}
\textbf{Paso 1.} Identificar las dependencias. La variable $z$ depende de $t$
únicamente a través de las variables intermedias $x(t) = \sqrt{t^2+1}$ y
$y(t) = e^t$, de modo que aplica el Caso 1 de la regla de la cadena.

\textbf{Paso 2.} Derivadas parciales de $z$.
\[
\frac{\partial z}{\partial x} = 2xy, \quad \frac{\partial z}{\partial y} = x^2 - \cos y.
\]

\textbf{Paso 3.} Derivadas ordinarias de las variables intermedias.
\[
\frac{dx}{dt} = \frac{t}{\sqrt{t^2+1}}, \quad \frac{dy}{dt} = e^t.
\]

\textbf{Paso 4.} Sustituir y sumar.
\[
...
\]
Simplificando (el radical se cancela en el primer sumando):
\[
...
\begin{theorem}[Regla de la cadena, Caso 2]\index{regla de la cadena!varias variables, caso 2}
...
\begin{myproof}
\textbf{Paso 1.} Identificar las dependencias. Ahora $z$ depende de \emph{dos}
variables independientes $s$ y $t$ a través de $x(s,t) = s^2 + t$ e
$y(s,t) = \ln(st)$, de modo que aplica el Caso 2 y hay que calcular dos
parciales. Se supone $s,t>0$ para que $\ln(st)$ esté definido.
...
\textbf{Paso 3.} Derivadas de las variables intermedias. Usando
$\ln(st)=\ln s+\ln t$ en el segundo renglón:
\[
...
\begin{theorem}[Derivación implícita para funciones de varias variables]\index{derivacion implicita@derivación implícita!varias variables}
...
\begin{myproof}
\textbf{Paso 1.} Definir $F$. Pasamos todo a un lado de la ecuación para dejar
$F(x,y,z)=0$:
\[
F(x, y, z) = 3yz^2 - e^{4x}\cos(4z) - 3y^2 - 4.
\]

\textbf{Paso 2.} Derivadas parciales de $F$. En cada una se tratan las otras dos
variables como constantes; nótese la regla de la cadena en $\cos(4z)$ al derivar
respecto de $z$:
\[
F_x = -4e^{4x}\cos(4z),
\]
...
\begin{definition}[Gradiente de una función escalar]\index{gradiente}
...
\begin{theorem}[Propiedades del gradiente]\index{gradiente!propiedades}
...
\begin{definition}[Derivada direccional]\index{derivada direccional}
...
\begin{theorem}[Fórmula del gradiente para la derivada direccional]\index{derivada direccional!formula del gradiente@fórmula del gradiente}
...
\begin{theorem}[Máximo de la derivada direccional]\index{derivada direccional!maximo@máximo}
...
\begin{definition}[Plano tangente y recta normal para superficie implícita]\index{plano tangente!superficie implicita@superficie implícita}\index{recta normal}
