...
\subsection{Regla de la cadena (Caso 1: una variable independiente)}
...
\subsection{Regla de la cadena (Caso 2: dos variables independientes)}
...
\begin{prob}
El voltaje en los extremos de un conductor aumenta a una tasa de $5\ \text{V/min}$ y la resistencia disminuye a razón de $1\ \Omega/\text{min}$. Si $I=V/R$, calcule la tasa a la que cambia la corriente $I$ cuando $R=60\ \Omega$ y $V=75\ \text{V}$.
\end{prob}
...
\end{enumerate}
\begin{myproof}
En cada caso: (i) calcular $\nabla f$ en $p$; (ii) normalizar $\mathbf{v}$; (iii) $D_\mathbf{u}f=\nabla f\cdot\hat{u}$.

\textbf{a)} $f(x,y)=x^2+y^2$, $p=(1,2)$, $\mathbf{v}=\langle 3,4\rangle$.
\[
...
\textbf{b)} $f(x,y)=e^x\cos y$, $p=(0,\pi/3)$, $\mathbf{v}=\langle 1,-1\rangle$.
\[
\nabla f=\langle e^x\cos y,\,-e^x\sin y\rangle\big|_{(0,\pi/3)}
=\left\langle\frac{1}{2},\,-\frac{\sqrt{3}}{2}\right\rangle.
\]
\[
...
\textbf{c)} $f(x,y)=\sqrt{xy}$, $p=(4,9)$, $\mathbf{v}=\langle -1,2\rangle$.
\[
...
\textbf{a)} $f(x,y)=3x^2-2xy+y^2$.
\[
\nabla f=\langle 6x-2y,\,-2x+2y\rangle\big|_{(1,2)}=\langle 2,\,2\rangle.
\]
Dirección: $\langle 2,2\rangle$ (o $\langle 1,1\rangle$ normalizado $\langle\frac{1}{\sqrt{2}},\frac{1}{\sqrt{2}}\rangle$).
Tasa máxima: $\|\langle 2,2\rangle\|=\boxed{2\sqrt{2}}$.

\textbf{b)} $f(x,y)=\sinh(xy)$.
\[
...
Tasa máxima: $\|\nabla f\|=\cosh\!\left(\dfrac{\pi}{2}\right)\dfrac{\sqrt{4+\pi^2}}{2}$.
$\boxed{\text{Tasa máxima}=\dfrac{\sqrt{4+\pi^2}}{2}\cosh\!\left(\dfrac{\pi}{2}\right)}$.

\textbf{c)} $f(x,y,z)=x^2+y^2+z^2-xy$.
\[
\nabla f=\langle 2x-y,\,2y-x,\,2z\rangle\big|_{(1,1,1)}=\langle 1,\,1,\,2\rangle.
\]
Dirección: $\langle 1,1,2\rangle$.
Tasa máxima: $\|\langle 1,1,2\rangle\|=\boxed{\sqrt{6}}$.
\end{myproof}
\end{prob}
...
recta normal $\mathbf{r}(t)=p+t\,\nabla F(p)$.

\textbf{a)} $F=x^2+y^2+z^2-14$, $p=(1,2,3)$.
\[
...
\textbf{b)} $F=x^2+2y^2-z^2-4$, $p=(2,1,-1)$.
\[
...
&=\left(\dfrac{2}{3}\pi (800) (200)\right)\left(5\right)+\left(\dfrac{1}{3}\pi (800)^2\right)\left(-10\right),\\&=\left(\dfrac{1600000}{3}\pi\right)+\left(\dfrac{-6400000}{3}\pi\right)\\&=\left(\dfrac{-4800000}{3}\pi\right)\ m^3/s.
\end{align*}
\end{myproof}
\end{prob}
...
\dfrac{dS}{dt}&=\left(24\right)\left(-0.5\right)+\left(16\right)\left(0.3\right)+\left(28\right)\left(0.3\right),\\\dfrac{dS}{dt}&=6/5\approx 1.2\ m^2/s.
\end{align*}
\end{myproof}
\end{prob}
...
\begin{myproof}
Sea $a$ la longitud del cateto de $10\ cm$ con $\dfrac{da}{dt}=1\ cm/min$, y $b=12\ cm$ la longitud del otro cateto (constante en ese instante, $\dfrac{db}{dt}=0$). Sea $\alpha$ el ángulo opuesto al cateto $a$. De la trigonometría del triángulo rectángulo:
\[
\tan\alpha = \frac{a}{b}.
\]
Diferenciando ambos lados respecto a $t$ por la regla de la cadena:
\[
\sec^2\!\alpha\;\frac{d\alpha}{dt}
= \frac{b\,\dfrac{da}{dt} - a\,\dfrac{db}{dt}}{b^2}
= \frac{\dfrac{da}{dt}}{b}.
\]
En el instante dado, $a=10\ cm$ y $b=12\ cm$, por lo que
\[
\tan\alpha = \frac{10}{12} = \frac{5}{6}
\implies
\sec^2\!\alpha = 1+\tan^2\!\alpha = 1 + \frac{25}{36} = \frac{61}{36}.
\]
Sustituyendo $\dfrac{da}{dt}=1$ y $b=12$:
\[
\frac{61}{36}\;\frac{d\alpha}{dt} = \frac{1}{12}
\implies
...
\begin{tikzpicture}[scale=0.8]
  \coordinate (O) at (0,0);
  \coordinate (P) at (1.87,2.35);
  \coordinate (Q) at (5.58,2.20);
  \draw[thick] (O) -- (P) -- (Q) -- cycle;
  \filldraw[fill=gray!20, draw=black, thick]
    (O) -- ({0.4*cos(21.5)},{0.4*sin(21.5)})
    arc[start angle=21.5, end angle=51.5, radius=0.4] -- cycle;
  \node at (0.52,0.40) {$\theta$};
  \node[left] at (0.93,1.18) {$x$};
  \node[below] at (2.93,0.90) {$y$};
  \fill (O) circle (2.5pt);
  \fill (P) circle (2.5pt);
  \fill (Q) circle (2.5pt);
\end{tikzpicture}
