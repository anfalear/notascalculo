...
\begin{definition}[Derivadas parciales primeras]\index{derivada parcial}
...
\begin{definition}[Derivadas parciales de segundo orden]\index{derivada parcial!de segundo orden}
...
\begin{theorem}[Teorema de Clairaut (igualdad de derivadas cruzadas)]\index{Clairaut, teorema de}
...
Para una función de dos variables $z=f(x,y)$, cuya gráfica es una superficie en $\mathbb{R}^3$, el análogo a la recta tangente es el \textbf{plano tangente}\index{plano tangente}. Este plano es la mejor aproximación lineal a la superficie en un punto $(x_0, y_0, z_0)$.
...
\begin{pasos}[Protocolo para hallar el plano tangente a una superficie]\label{pasos:plano_tangente} Sea $z=f(x,y)$ una superficie y $P=(x_0, y_0, f(x_0, y_0))$ un punto sobre ella, estos pasos permiten calcular el plano tangente a $z$ en $P.$
\begin{enumerate}
...
\begin{myproof}
\textbf{Paso 1. Escribir cada superficie como gráfica $z=f(x,y)$ cerca de $(1,1,2)$.}
Ambas ecuaciones son cuadráticas en $z$, de modo que cada una define dos ramas; en cada caso se elige la que pasa por el punto dado, donde $z=2$.

Para el elipsoide, $z^2=9-3x^2-2y^2$ y como $z=2>0$ la rama es
\[
f(x,y)=\sqrt{9-3x^2-2y^2},\qquad f(1,1)=\sqrt{9-3-2}=\sqrt{4}=2.\quad\checkmark
\]
Para la esfera, completando cuadrados,
\[
x^2+y^2+z^2-8x-6y-8z+24=(x-4)^2+(y-3)^2+(z-4)^2-17,
\]
así que la superficie es la esfera de centro $(4,3,4)$ y radio $\sqrt{17}$. Como $z=2<4$, la rama que pasa por el punto es la inferior:
\[
g(x,y)=4-\sqrt{17-(x-4)^2-(y-3)^2},\qquad g(1,1)=4-\sqrt{17-9-4}=4-2=2.\quad\checkmark
\]

\textbf{Paso 2. Derivadas parciales del elipsoide en $(1,1)$.}
Derivando $f$ mediante la regla de la cadena (con la otra variable constante):
\[
f_x=\frac{-6x}{2\sqrt{9-3x^2-2y^2}}=\frac{-3x}{\sqrt{9-3x^2-2y^2}},
\qquad
f_y=\frac{-4y}{2\sqrt{9-3x^2-2y^2}}=\frac{-2y}{\sqrt{9-3x^2-2y^2}}.
\]
En $(1,1)$ el radicando vale $4$, luego
\[
f_x(1,1)=-\frac{3}{2},\qquad f_y(1,1)=-1.
\]

\textbf{Paso 3. Derivadas parciales de la esfera en $(1,1)$.}
De la misma forma, escribiendo $u(x,y)=17-(x-4)^2-(y-3)^2$:
\[
g_x=-\frac{-2(x-4)}{2\sqrt{u}}=\frac{x-4}{\sqrt{u}},
\qquad
g_y=\frac{y-3}{\sqrt{u}}.
\]
En $(1,1)$ se tiene $u=4$, luego
\[
g_x(1,1)=\frac{1-4}{2}=-\frac{3}{2},\qquad g_y(1,1)=\frac{1-3}{2}=-1.
\]

\textbf{Paso 4. Conclusión.}
Las dos superficies pasan por el mismo punto y tienen las mismas derivadas parciales ahí, de modo que la fórmula de la Construcción~\ref{pasos:plano_tangente} produce, para ambas, el mismo plano:
\[
z-2=-\frac{3}{2}(x-1)-1\,(y-1)
\quad\Longleftrightarrow\quad
3x+2y+2z=9.
\]
\[
\boxed{\text{Las superficies son tangentes en }(1,1,2)\text{: comparten el plano tangente }3x+2y+2z=9.}
\]
\end{myproof}
\end{prob}
...
\begin{myproof}
\textbf{Paso 1. Escribir la superficie como gráfica.}
Aquí la variable ya despejada es $y$, no $z$: el paraboloide es la gráfica de
\[
y=f(x,z)=x^2+z^2,
\]
definida y diferenciable en todo el plano $xz$. La Construcción~\ref{pasos:plano_tangente} se aplica sin cambios permutando el papel de las variables: en el punto $(x_0,y_0,z_0)$ con $y_0=f(x_0,z_0)$, el plano tangente es
\[
y-y_0=f_x(x_0,z_0)(x-x_0)+f_z(x_0,z_0)(z-z_0).
\]

\textbf{Paso 2. Plano tangente en un punto genérico.}
Como $f_x=2x$ y $f_z=2z$,
\[
y-y_0=2x_0(x-x_0)+2z_0(z-z_0).
\]
Desarrollando y usando que $x_0^2+z_0^2=y_0$, los términos constantes se agrupan en
$-2x_0^2-2z_0^2+y_0=-2y_0+y_0=-y_0$, de modo que la ecuación se reduce a
\[
2x_0\,x-y+2z_0\,z=y_0.
\]

\textbf{Paso 3. Condición de paralelismo.}
Dos planos son paralelos exactamente cuando los coeficientes de $x$, $y$ y $z$ son proporcionales. Comparando con $x+2y+3z=1$:
\[
(2x_0,\,-1,\,2z_0)=k\,(1,\,2,\,3).
\]
La segunda componente da $-1=2k$, es decir $k=-\tfrac{1}{2}$, y con ese valor
\[
2x_0=-\tfrac{1}{2}\implies x_0=-\tfrac{1}{4},
\qquad
2z_0=-\tfrac{3}{2}\implies z_0=-\tfrac{3}{4}.
\]

\textbf{Paso 4. Resultado.}
Sustituyendo en el paraboloide,
\[
...
\begin{myproof}
\textbf{Paso 1. Escribir el hiperboloide como gráfica.}
De $x^2=1+y^2+z^2\ge 1$ se sigue que $|x|\ge 1$: la superficie tiene dos hojas y cada una es la gráfica de
\[
x=f(y,z)=\pm\sqrt{1+y^2+z^2}
\]
sobre todo el plano $yz$. En particular, $x_0\neq 0$ en cualquier punto de la superficie, lo que permite dividir por $x_0$ más adelante.

\textbf{Paso 2. Parciales y plano tangente en un punto genérico.}
En cualquiera de las dos hojas,
\[
f_y=\pm\frac{y}{\sqrt{1+y^2+z^2}}=\frac{y}{x},
\qquad
f_z=\pm\frac{z}{\sqrt{1+y^2+z^2}}=\frac{z}{x},
\]
donde el signo se absorbe porque precisamente $x=\pm\sqrt{1+y^2+z^2}$. Aplicando la Construcción~\ref{pasos:plano_tangente} con $x$ como variable despejada:
\[
x-x_0=\frac{y_0}{x_0}(y-y_0)+\frac{z_0}{x_0}(z-z_0).
\]
Multiplicando por $x_0$ y usando que $x_0^2-y_0^2-z_0^2=1$:
\[
x_0\,x-y_0\,y-z_0\,z=x_0^2-y_0^2-z_0^2=1.
\]

\textbf{Paso 3. Condición de paralelismo.}
El plano $z=x+y$ se escribe $x+y-z=0$. Los coeficientes deben ser proporcionales:
\[
(x_0,\,-y_0,\,-z_0)=k\,(1,\,1,\,-1)
\implies
...
\textbf{Paso 4. Sustitución y conclusión.}
Estos valores deben satisfacer la ecuación del hiperboloide:
\[
x_0^2-y_0^2-z_0^2=k^2-(-k)^2-k^2=-k^2=1\implies k^2=-1,
\]
lo cual es imposible en $\mathbb{R}$.
\[
...
\begin{definition}[Linealización]\index{linealizacion@linealización}
...
\begin{definition}[Diferencial total]\index{diferencial total}
...
\begin{theorem}[Diferenciabilidad y continuidad]\index{diferenciabilidad!implica continuidad}
...
\begin{theorem}[Criterio de diferenciabilidad]\index{diferenciabilidad!criterio}
