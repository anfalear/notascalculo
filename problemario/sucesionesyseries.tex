...
\begin{definition}[Sucesión de números reales]\index{sucesion@sucesión}
...
\begin{theorem}[Principio de Inducción]\index{induccion, principio de@inducción, principio de}
...
\begin{definition}[Límite de una sucesión]\index{sucesion@sucesión!limite@límite}
...
\begin{theorem}[Unicidad del límite]\index{sucesion@sucesión!unicidad del limite@unicidad del límite}
...
\begin{theorem}[Propiedades aritméticas del límite de una sucesión]\index{sucesion@sucesión!propiedades aritmeticas del limite@propiedades aritméticas del límite}
...
\begin{theorem}[Teorema del emparedado]\index{teorema del sandwich@teorema del sándwich!para sucesiones}
...
\begin{proof}
Sea $\varepsilon>0$. Por hipótesis existen $N_1,N_2$ tales que
$|a_n-L|<\varepsilon$ para $n>N_1$ y $|b_n-L|<\varepsilon$ para $n>N_2$. Sea
$N=\max(N_0,N_1,N_2)$. Para $n>N$ se tiene
$L-\varepsilon<a_n\leq c_n\leq b_n<L+\varepsilon$, es decir,
$|c_n-L|<\varepsilon$. Como $\varepsilon>0$ es arbitrario,
$\lim_{n\to\infty}c_n=L$.
\end{proof}
...
\begin{definition}[Sucesión monótona y acotada]\index{sucesion@sucesión!monotona y acotada@monótona y acotada}
La sucesión $\{a_n\}$ es:
\begin{itemize}
...
\begin{theorem}[Convergencia monótona]\index{convergencia monotona, teorema de@convergencia monótona, teorema de}
\label{thm:mono}
...
\begin{definition}[Sucesión de Cauchy]\index{sucesion@sucesión!de Cauchy}
La sucesión $\{a_n\}$ es de \textbf{Cauchy} si para todo $\varepsilon > 0$
existe $N \in \mathbb{N}$ tal que $|a_n - a_m| < \varepsilon$ para todo
$n, m > N$.
\end{definition}

\begin{theorem}[Criterio de Cauchy]\index{Cauchy!criterio de convergencia}
...
\begin{definition}[Infinitesimales equivalentes]\index{infinitesimales!equivalentes}
...
\begin{theorem}[Stolz-Cesàro]\index{Stolz-Cesaro, criterio de@Stolz-Cesàro, criterio de}
...
\begin{definition}[Serie numérica y suma parcial]\index{serie!numerica@numérica}\index{suma parcial}
...
\begin{theorem}[Criterio del término $n$-ésimo]\index{serie!criterio del termino $n$-esimo@criterio del término $n$-ésimo}
...
\begin{theorem}[Serie geométrica]\index{serie!geometrica@geométrica}
...
\begin{theorem}[Series $p$]\index{serie!$p$}
...
\begin{theorem}[Criterio de la integral]\index{serie!criterio de la integral}
...
\begin{theorem}[Comparación directa y en el límite]\index{serie!criterio de comparacion@criterio de comparación}
...
\begin{proof}[Esquema]
Para la \textbf{comparación directa}, sean $A_n=\sum_{k=1}^n a_k$ y
$B_n=\sum_{k=1}^n b_k$ las sumas parciales. Al ser todos los términos no
negativos, ambas sucesiones son no decrecientes. Si $\sum b_n$ converge,
$\{B_n\}$ está acotada, y de $a_n\leq b_n$ (para $n\geq N$) se sigue que
$\{A_n\}$ también está acotada; una sucesión no decreciente y acotada
superiormente converge, luego $\sum a_n$ converge. La afirmación sobre
divergencia es el contrarrecíproco. Para la \textbf{comparación en el límite},
si $a_n/b_n\to L\in(0,\infty)$, existen constantes $0<c<C$ y un índice $N$
tales que $c\,b_n\leq a_n\leq C\,b_n$ para $n\geq N$; aplicando la comparación
directa en ambos sentidos se concluye que las dos series tienen la misma
naturaleza.
\end{proof}
...
\begin{theorem}[Criterio del cociente — D'Alembert]\index{serie!criterio del cociente}
...
\begin{proof}[Esquema]
Supóngase $L<1$ y elíjase $r$ con $L<r<1$. Como $a_{n+1}/a_n\to L$, existe $N$
tal que $a_{n+1}/a_n<r$ para $n\geq N$; iterando, $a_{N+k}<r^k a_N$. Así, la
cola $\sum_{k\geq 0}a_{N+k}$ queda dominada por la serie geométrica convergente
$a_N\sum_{k\geq 0} r^k$, y por comparación directa $\sum a_n$ converge. Si
$L>1$, entonces $a_{n+1}>a_n$ a partir de cierto índice, de modo que $a_n\not\to
0$ y la serie diverge por el criterio del término $n$-ésimo. El caso $L=1$ es
inconcluso, como muestran $\sum 1/n$ (diverge) y $\sum 1/n^2$ (converge), ambas
con $L=1$.
\end{proof}

\begin{theorem}[Criterio de la raíz — Cauchy]\index{serie!criterio de la raiz@criterio de la raíz}
...
\begin{definition}[Convergencia absoluta y condicional]\index{serie!convergencia absoluta y condicional} Dada la serie $\sum a_n$ diremos que:
\begin{itemize}
...
\begin{theorem}[Criterio de Leibniz para series alternadas]\index{serie!criterio de Leibniz}
...
\begin{theorem}[Criterio de Raabe]\index{serie!criterio de Raabe}
