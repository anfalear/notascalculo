...
\begin{definition}[Curva cerrada simple y orientación positiva]\index{curva cerrada simple}\index{orientacion positiva@orientación positiva}
...
2\sen t \rangle$, $0 \le t \le 2\pi$. Entonces
\[
\mathbf{F}(\mathbf{r}(t)) = \langle -2\sen t, 2\cos t \rangle,
\quad \mathbf{r}'(t) = \langle -2\sen t, 2\cos t \rangle.
\]
Por tanto,
\[
\oint_C \mathbf{F}\cdot d\mathbf{r}
= \int_0^{2\pi} \langle -2\sen t, 2\cos t \rangle \cdot
\langle -2\sen t, 2\cos t \rangle \, dt
= \int_0^{2\pi} 4(\sen^2 t + \cos^2 t)\,dt
...
\begin{example}[Green sobre una curva abierta]
Evalúe $\displaystyle\int_C \mathbf F\cdot d\mathbf r$ para
$\mathbf F(x,y)=\langle x^2+y,\ 4x-y^2\rangle$, donde $C$ es el arco de la
curva $y=\sen x$ recorrido desde $(0,0)$ hasta $(\pi,0)$.
\begin{myproof}
\textbf{Paso 1. Cerrar la curva.}
$C$ es abierta; la cerramos con el segmento $S$ del eje $x$, de modo que
$C\cup S$ encierra la región $R=\{0\le x\le\pi,\ 0\le y\le\sen x\}$
(figura~\ref{fig:green_curva_abierta}). La \emph{frontera orientada
positivamente} $\partial R$ recorre el segmento del eje $x$ de $(0,0)$ a
$(\pi,0)$ y luego el arco de seno de $(\pi,0)$ de regreso a $(0,0)$ (así $R$
queda a la izquierda).

\textbf{Paso 2. Aplicar Green.}
Con $M=x^2+y$ y $N=4x-y^2$: $N_x-M_y=4-1=3$. Por tanto
\[
  \oint_{\partial R}\mathbf F\cdot d\mathbf r=\iint_R 3\,dA
  =3\int_0^\pi \sen x\,dx=3\cdot 2=6.
\]

\textbf{Paso 3. Descomponer y usar la orientación.}
$\partial R$ consta del segmento de $(0,0)$ a $(\pi,0)$ y del arco de $(\pi,0)$
a $(0,0)$, que es $C$ \emph{recorrido al revés}; así
$\oint_{\partial R}=\int_{\text{seg}}-\int_C$. Sobre el segmento ($y=0$,
$dy=0$, $x:0\to\pi$): $\mathbf F\cdot d\mathbf r=x^2\,dx$, luego
$\int_{\text{seg}}=\int_0^\pi x^2\,dx=\dfrac{\pi^3}{3}$.

\textbf{Paso 4. Despejar la integral pedida.}
De $6=\dfrac{\pi^3}{3}-\int_C$,
\[
...
\begin{tikzpicture}[scale=1.5]
  % region bajo el arco
  \fill[blue!12] plot[domain=0:3.1416,samples=60,variable=\x]
    ({\x},{sin(deg(\x))}) -- (0,0) -- cycle;
  % ejes
  \draw[->,thin] (-0.3,0)--(3.8,0) node[right]{$x$};
  \draw[->,thin] (0,-0.3)--(0,1.4) node[above]{$y$};
  % arco C
  \draw[very thick,blue!70!black,domain=0:3.1416,samples=60,smooth,variable=\x]
    plot ({\x},{sin(deg(\x))});
  \draw[->,very thick,blue!70!black] (1.62,0.9962)--(1.45,0.9925);
  \node[blue!70!black] at (1.3,1.12) {$C$};
  % segmento S
  \draw[very thick,red!70!black] (3.1416,0)--(0,0);
  \draw[->,very thick,red!70!black] (1.4,0)--(1.75,0);
  \node[red!70!black,below] at (2.4,-0.03) {$S$};
  % marcas
  \node[below] at (3.1416,-0.03) {$\pi$};
  \filldraw (0,0) circle(0.6pt); \filldraw (3.1416,0) circle(0.6pt);
  \node[blue!40!black] at (1.55,0.4) {$R$};
\end{tikzpicture}
\caption{Arco $C:\,y=\sen x$ de $(0,0)$ a $(\pi,0)$ (azul), cerrado con el
  segmento $S$ del eje $x$ (rojo). La región $R$ queda encerrada; su frontera
  se recorre en sentido positivo tomando el segmento de $(0,0)$ a $(\pi,0)$ y
  luego el arco de regreso.}
\label{fig:green_curva_abierta}
\end{figure}

%%==========================================================
\begin{prob}[Teorema de Green en una región con agujero]
Verifique el teorema de Green para el campo
$\mathbf F(x,y)=\langle 2x^3-y^3,\ x^3+y^3\rangle$ sobre el anillo
$R:\,1\le x^2+y^2\le 4$.
\begin{myproof}
\textbf{Paso 1. Integrando de la integral doble.}
Con $M=2x^3-y^3$ y $N=x^3+y^3$,
\[
\frac{\partial N}{\partial x}-\frac{\partial M}{\partial y}
=3x^2-(-3y^2)=3(x^2+y^2).
\]

\textbf{Paso 2. Integral doble en polares.}
El anillo es $1\le r\le 2$, $0\le\theta\le 2\pi$, con $x^2+y^2=r^2$:
\[
\iint_R 3(x^2+y^2)\,dA
=3\int_0^{2\pi}\!\!\int_1^2 r^2\cdot r\,dr\,d\theta
=3\cdot 2\pi\cdot\Bigl[\tfrac{r^4}{4}\Bigr]_1^2
=6\pi\cdot\frac{15}{4}=\frac{45\pi}{2}.
\]

\textbf{Paso 3. La frontera orientada.}
La frontera positiva de $R$ consta de la circunferencia exterior $C_2$ (radio
$2$, sentido antihorario) y la interior $C_1$ (radio $1$, sentido horario),
de modo que la región queda siempre a la izquierda
(figura~\ref{fig:green_anillo}). Por tanto
\[
\oint_{\partial R}\mathbf F\cdot d\mathbf r
=\oint_{C_2}\mathbf F\cdot d\mathbf r-\oint_{C_1}\mathbf F\cdot d\mathbf r,
\]
donde ambas integrales de la derecha se recorren en sentido antihorario.

\textbf{Paso 4. Integral de línea sobre una circunferencia de radio $R$.}
Con $x=R\cos t$, $y=R\sen t$, $t\in[0,2\pi]$, un cálculo directo da
\[
\oint_{x^2+y^2=R^2}\mathbf F\cdot d\mathbf r
=R^4\int_0^{2\pi}\bigl(\cos^4 t+\sen^4 t\bigr)dt
=R^4\cdot\frac{3\pi}{2},
\]
pues los términos $-2\cos^3 t\,\sen t$ y $\sen^3 t\,\cos t$ integran $0$ en el
periodo, mientras $\int_0^{2\pi}\cos^4 t\,dt=\int_0^{2\pi}\sen^4 t\,dt=\tfrac{3\pi}{4}$.

\textbf{Paso 5. Comparar.}
\[
\oint_{\partial R}\mathbf F\cdot d\mathbf r
=2^4\cdot\frac{3\pi}{2}-1^4\cdot\frac{3\pi}{2}
=24\pi-\frac{3\pi}{2}=\frac{45\pi}{2}.
\]
Coincide con la integral doble: $\boxed{\dfrac{45\pi}{2}=\dfrac{45\pi}{2}}$.
\end{myproof}
\end{prob}
...
\begin{tikzpicture}[scale=1.1]
  % anillo
  \fill[blue!12,even odd rule] (0,0) circle (2) (0,0) circle (1);
  % ejes
  \draw[->,thin] (-2.6,0)--(2.6,0) node[right]{$x$};
  \draw[->,thin] (0,-2.6)--(0,2.6) node[above]{$y$};
  % circunferencia exterior C2 (antihoraria)
  \draw[very thick,blue!70!black] (0,0) circle (2);
  \draw[->,very thick,blue!70!black] (2,-0.03)--(2,0.03);
  \node[blue!70!black] at (2.1,0.6) {$C_2$};
  % circunferencia interior C1 (horaria)
  \draw[very thick,red!70!black] (0,0) circle (1);
  \draw[->,very thick,red!70!black] (1,0.03)--(1,-0.03);
  \node[red!70!black] at (0.32,0.82) {$C_1$};
  \node[blue!40!black] at (1.45,1.15) {$R$};
\end{tikzpicture}
\caption{Anillo $1\le x^2+y^2\le 4$. La frontera exterior $C_2$ se recorre en
  sentido antihorario y la interior $C_1$ en sentido horario, dejando la
  región $R$ siempre a la izquierda.}
\label{fig:green_anillo}
\end{figure}
...
\begin{definition}[Divergencia]\index{divergencia}
...
\begin{definition}[Rotacional]\index{rotacional}
...
\begin{enumerate}[(a)]
    ...
    \item $\mathbf{F}(x,y)=\langle \cos y, -x\sen y \rangle$
    \item $\mathbf{F}(x,y)=\langle e^x \cos y, -e^x \sen y \rangle$
    \item $\mathbf{F}(x,y,z)=\langle 2x, 2y, 2z \rangle$
    \item $\mathbf{F}(x,y,z)=\langle yz, xz, xy \rangle$
\end{enumerate}
\end{multicols}
\begin{myproof}
\textbf{Paso 1. Estrategia.} Para cada campo verificamos $\partial M/\partial y=\partial N/\partial x$ (en $\mathbb{R}^2$) o $\nabla\times\mathbf{F}=\mathbf{0}$ (en $\mathbb{R}^3$); si el dominio es simplemente conexo, el campo es conservativo. Hallamos $f$ tal que $\nabla f=\mathbf{F}$.

\textbf{Paso 2. Casos (a)--(d) en $\mathbb{R}^2$.}
...
\[
\int_C\mathbf{F}\cdot d\mathbf{r}=f(1,2)-f(0,0)=1^2\cdot2^2-0=4.
\]
\textbf{Paso 4. Resultado.}
\[
\boxed{\int_C\mathbf{F}\cdot d\mathbf{r}=4.}
\]
\end{myproof}
\end{prob}
...
\]
\textbf{Paso 2. Parametrizar la elipse.} $C$: $x=a\cos t$, $y=b\sen t$, $0\le t\le2\pi$:
\[
x\,dy-y\,dx=(a\cos t)(b\cos t\,dt)-(b\sen t)(-a\sen t\,dt)=ab(\cos^2t+\sen^2t)\,dt=ab\,dt.
\]
\textbf{Paso 3. Calcular la integral.}
\[
A=\frac{1}{2}\int_0^{2\pi}ab\,dt=\frac{ab}{2}\cdot2\pi=\pi ab.
\]
\textbf{Paso 4. Resultado.}
\[
\boxed{A(\text{elipse})=\pi ab.}
\]
\end{myproof}
\end{prob}
...
\textbf{Paso 2. Parametrizar la circunferencia unidad.} $\mathbf{r}(t)=\langle\cos t,\sen t\rangle$, $d\mathbf{r}=\langle-\sen t,\cos t\rangle dt$. Sobre $C$, $x^2+y^2=1$, luego $\mathbf{F}=\langle-\sen t,\cos t\rangle$.

\textbf{Paso 3. Calcular la integral.}
\[
\oint_C\mathbf{F}\cdot d\mathbf{r}=\int_0^{2\pi}\bigl(\sen^2 t+\cos^2 t\bigr)dt=\int_0^{2\pi}dt=2\pi.
\]
\textbf{Paso 4. Conclusión.} Aunque $\partial N/\partial x=\partial M/\partial y$, el dominio $D=\mathbb{R}^2\setminus\{(0,0)\}$ tiene un agujero en el origen: \emph{no es simplemente conexo}. El TFC exige dominio simplemente conexo, condición que no se cumple. $\mathbf{F}$ no tiene potencial global en $D$, por lo que $\oint_C\mathbf{F}\cdot d\mathbf{r}=2\pi\ne0$ no contradice el TFC.
\end{myproof}
\end{prob}

\begin{prob}[Identidad vectorial de la divergencia de un producto cruz]
Demuestre la identidad
\[
\operatorname{div}(\mathbf F\times\mathbf G)
=\mathbf G\cdot(\nabla\times\mathbf F)-\mathbf F\cdot(\nabla\times\mathbf G),
\]
válida para campos $\mathbf F,\mathbf G$ de clase $C^1$ en $\mathbb R^3$.
(Sugerencia: escriba $\mathbf F\times\mathbf G$ por componentes, aplique la
divergencia junto con la regla del producto para derivadas y reagrupe los
términos en los dos productos punto.)
\end{prob}
...
\begin{definition}[Superficie paramétrica suave]\index{superficie!parametrica suave@paramétrica suave}
...
\begin{definition}[Orientación de una superficie]\index{superficie!orientacion@orientación}
...
\begin{definition}[Integral de superficie escalar]\index{integral de superficie!de un campo escalar}
...
Pasamos a coordenadas polares: $x=r\cos\theta,\; y=r\sen\theta$, con $0\le r\le 2$, $0\le\theta\le 2\pi$. La región es $R$, así que $dA = r\,dr\,d\theta$. El integrando se convierte en
\[
\bigl(r\cos\theta + r^2\sen^2\theta\bigr)\sqrt{1+4r^2}\cdot r.
\]
...
Queda
\[
I = \int_0^{2\pi}\sen^2\theta\,d\theta \int_0^2 r^3\sqrt{1+4r^2}\,dr.
\]
...
\[
\mathbf{r}(\phi,\theta)=\langle 2\sen\phi\cos\theta,\; 2\sen\phi\sen\theta,\; 2\cos\phi \rangle,
\]
con $\phi\in[0,\pi]$, $\theta\in[0,2\pi]$.

\textbf{Paso 2: Calcular el elemento de área.}
Sabemos que para una esfera de radio $R$, $\|\mathbf{r}_\phi \times \mathbf{r}_\theta\| = R^2\sen\phi$. Aquí $R=2$, así que
\[
dS = 4\sen\phi\,d\phi\,d\theta.
\]

\textbf{Paso 3: Formar el integrando.}
Sobre la esfera, $x^2+y^2 = 4\sen^2\phi$. Por tanto,
\[
(x^2+y^2)\,dS = (4\sen^2\phi)(4\sen\phi)\,d\phi\,d\theta = 16\sen^3\phi\,d\phi\,d\theta.
\]

\textbf{Paso 4: Reducir a integral doble.}
\[
I = \int_0^{2\pi}\int_0^\pi 16\sen^3\phi\,d\phi\,d\theta.
\]

\textbf{Paso 5: Evaluar.}
\[
I = 16(2\pi)\int_0^\pi \sen^3\phi\,d\phi
= 32\pi \cdot \frac{4}{3}
= \frac{128\pi}{3}.
\]
\[
...
\begin{myproof}
\textbf{Paso 1.}
Usamos $\mathbf{r}(\phi,\theta)=\langle R\sen\phi\cos\theta, R\sen\phi\sen\theta, R\cos\phi\rangle$, con $\phi,\theta\in[0,\pi/2]$. El elemento de área es $dS=R^2\sen\phi\,d\phi\,d\theta$.

\textbf{Paso 2.}
\[
m = \iint_S z\,dS
= \int_0^{\pi/2}\int_0^{\pi/2} (R\cos\phi)(R^2\sen\phi)\,d\phi\,d\theta
= R^3 \cdot \frac{\pi}{2} \int_0^{\pi/2} \sen\phi\cos\phi\,d\phi
...
\]
\[
M_{xy} = \int_0^{\pi/2}\int_0^{\pi/2} (R^2\cos^2\phi)(R^2\sen\phi)\,d\phi\,d\theta
= R^4 \cdot \frac{\pi}{2} \int_0^{\pi/2} \cos^2\phi\sen\phi\,d\phi.
\]
...
\[
M_{yz}
= \int_0^{\pi/2}\int_0^{\pi/2}(R\sen\phi\cos\theta)(R\cos\phi)(R^2\sen\phi)\,d\phi\,d\theta
= R^4\!\int_0^{\pi/2}\!\cos\theta\,d\theta\cdot\int_0^{\pi/2}\!\sen^2\phi\cos\phi\,d\phi
...
\begin{definition}[Flujo de un campo vectorial]\index{flujo de un campo vectorial}\index{integral de superficie!de un campo vectorial}
...
\[
\mathbf{r}(\theta,z)=\langle \cos\theta,\; \sen\theta,\; z \rangle,
\quad 0\le\theta\le 2\pi,\; 0\le z\le 2.
\]

\textbf{Paso 3: Calcular el vector normal.}
\[
\mathbf{r}_\theta = \langle -\sen\theta,\; \cos\theta,\; 0 \rangle,\quad
...
Sobre el cilindro, $\mathbf{F}(\mathbf{r}(\theta,z)) = \langle \cos\theta, \sen\theta, z\rangle$. Entonces
\[
\mathbf{F}\cdot(\mathbf{r}_\theta \times \mathbf{r}_z)
= \langle \cos\theta,\sen\theta,z\rangle \cdot \langle \cos\theta,\sen\theta,0\rangle
...
\begin{theorem}[Teorema de la divergencia]\index{divergencia, teorema de la}
...
Usamos coordenadas cilíndricas: $x=r\cos\theta$, $y=r\sen\theta$, $z=z$. El paraboloide es $z=1-r^2$, con $0\le r\le 1$, $0\le\theta\le 2\pi$, $0\le z\le 1-r^2$. El elemento de volumen es $dV = r\,dz\,dr\,d\theta$.
\[
\iiint_Q 2z\,dV
...
=\int_0^{2\pi}(-\sen^2 t+\cos t)\,dt=-\pi+0=-\pi.
\]
\textbf{Paso 4. Conclusión.} Ambos cálculos coinciden, verificando el teorema de Stokes.
\[
\boxed{\iint_S(\nabla\times\mathbf{F})\cdot d\mathbf{S}=\oint_{\partial S}\mathbf{F}\cdot d\mathbf{r}=-\pi.}
\]
\end{myproof}
\end{prob}
...
\]
\textbf{Paso 2. Identificar el sólido y su volumen.} El sólido $Q$ es la bola de radio 2:
\[
V=\frac{4}{3}\pi(2)^3=\frac{32\pi}{3}.
\]
\textbf{Paso 3. Aplicar el teorema de la divergencia.}
\[
\iint_{\partial Q}\mathbf{F}\cdot\mathbf{n}\,dS=\iiint_Q 4\,dV=4\cdot\frac{32\pi}{3}=\frac{128\pi}{3}.
\]
\textbf{Paso 4. Resultado.}
\[
\boxed{\iint_{\partial Q}\mathbf{F}\cdot\mathbf{n}\,dS=\frac{128\pi}{3}.}
\]
\end{myproof}
\end{prob}
...
\begin{theorem}[Teorema de Stokes]\index{Stokes, teorema de}
...
Parametrizamos el borde: $\mathbf{r}(t)=\langle 2\cos t, 2\sen t, 0\rangle$, $0\le t\le 2\pi$. Entonces $\mathbf{F}=\langle -2\sen t, 2\cos t, 0\rangle$, $d\mathbf{r}=\langle -2\sen t, 2\cos t, 0\rangle dt$. El producto punto es
\[
(-2\sen t)(-2\sen t) + (2\cos t)(2\cos t) = 4\sen^2 t + 4\cos^2 t = 4.
\]
Luego
\[
...
\iint_S z\,dS.
\]
Usamos coordenadas esféricas para el hemisferio de radio 1: $z=\cos\phi$, $dS=\sen\phi\,d\phi\,d\theta$, con $\phi\in[0,\pi/2]$, $\theta\in[0,2\pi]$.
\[
\iint_S z\,dS
= \int_0^{2\pi}\int_0^{\pi/2} \cos\phi\sen\phi\,d\phi\,d\theta
...
\begin{enumerate}[(a)]
    ...
    \item $\mathbf{F}(x,y,z)=\langle yz, xz, xy \rangle$
    \item $\mathbf{F}(x,y,z)=\langle e^x, \sen y, \cos z \rangle$
...
\begin{prob}
Interprete físicamente el siguiente hecho: si $\mathbf{F}$ es el campo
de velocidades de un fluido, ¿qué significa que
...
-e^x \sen y + x \rangle$ actúa sobre una partícula.
\begin{enumerate}[(a)]
