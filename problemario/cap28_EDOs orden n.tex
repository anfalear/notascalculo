...
\begin{theorem}[Principio de superposición]
...
\item Si $r = \alpha \pm i\beta$ es un par de raíces complejas conjugadas simples, las soluciones son $e^{\alpha x}\cos(\beta x)$ y $e^{\alpha x}\sen(\beta x)$. Para multiplicidad $k$, se multiplican por $1, x, \ldots, x^{k-1}$.
\end{enumerate}
\end{theorem}
...
m x'' + c x' + k x = F(t).
\]
Aquí $x(t)$ es el desplazamiento desde el equilibrio.
...
\end{prob}
\begin{myproof}
Homogénea: $r=\pm 3i$, $y_h=c_1\cos 3x+c_2\sen 3x$. Solapa, $y_p=x(A\cos 3x+B\sen 3x)$. Derivando: $y_p''+9y_p = -6A\sen 3x+6B\cos 3x = \cos 3x \Rightarrow A=0, B=1/6$. $\boxed{y=c_1\cos 3x+c_2\sen 3x+\frac{1}{6}x\sen 3x}$
\end{myproof}
...
\begin{prob}
Resuelva $y'' + y = \tan x$ (variación de parámetros).
\end{prob}
\begin{myproof}
$y_1=\cos x, y_2=\sen x, W=1$. $u_1'=-\tan x \sen x = -\frac{\sen^2 x}{\cos x} = -(\sec x - \cos x)$; $u_2'=\tan x \cos x = \sen x$. Integrando: $u_1=-\ln|\sec x+\tan x|+\sen x$; $u_2=-\cos x$. $y_p=u_1\cos x+u_2\sen x = -\cos x\ln|\sec x+\tan x|+\sen x\cos x-\cos x\sen x = -\cos x\ln|\sec x+\tan x|$. $\boxed{y=c_1\cos x+c_2\sen x-\cos x\ln|\sec x+\tan x|}$
\end{myproof}
...
\begin{prob}
Resuelva $x^2 y'' + 2x y' - 6y = 0$ (Cauchy-Euler).
\end{prob}
\begin{myproof}
$m(m-1)+2m-6 = m^2+m-6=(m+3)(m-2)=0$, $y=c_1 x^{-3}+c_2 x^2$. $\boxed{y=c_1 x^{-3}+c_2 x^2}$
\end{myproof}
...
\end{prob}
\begin{myproof}
Homogénea: $m(m-1)-m+1=(m-1)^2$, $y_h=x(c_1+c_2\ln x)$. Proponemos $y_p=Ax$ (solapa, usamos $Ax\ln x$). Sustituyendo: $y_p=A x\ln x$; $y_p'=A(\ln x+1)$; $y_p''=A/x$. Entonces $x^2(A/x)-x A(\ln x+1)+A x\ln x = Ax - Ax\ln x - Ax + Ax\ln x = 0$, no sirve. Probamos $y_p=B x^2$: $x^2(2B)-x(2Bx)+B x^2 = (2B-2B+B)x^2=Bx^2=x \Rightarrow B=1/x$? No. Mejor variación: $y_1=x, y_2=x\ln x$, $W=x$. $g=1$ (forma estándar: $y''-\frac{1}{x}y'+\frac{1}{x^2}y=1/x$). $u_1'=-\frac{(1/x)(x\ln x)}{x}=-\frac{\ln x}{x}$; $u_2'=\frac{(1/x)(x)}{x}=1/x$. $u_1=-\frac{1}{2}(\ln x)^2$; $u_2=\ln x$. $y_p= -\frac{x}{2}(\ln x)^2 + x(\ln x)^2 = \frac{x}{2}(\ln x)^2$. $\boxed{y=x(c_1+c_2\ln x)+\frac{x}{2}(\ln x)^2}$
\end{myproof}

\begin{prob}
Oscilador: $x''+4x=0$, $x(0)=1, x'(0)=-2$.
\end{prob}
\begin{myproof}
$r^2+4=0$, $x=c_1\cos 2t+c_2\sen 2t$. $x(0)=c_1=1$; $x'=-2c_1\sen 2t+2c_2\cos 2t$, $x'(0)=2c_2=-2 \Rightarrow c_2=-1$. $\boxed{x=\cos 2t-\sen 2t}$
\end{myproof}
...
\begin{prob}
Circuito RLC: $Q''+2Q'+5Q=10$, condiciones iniciales cero.
\end{prob}
\begin{myproof}
Homogénea: $r=-1\pm 2i$, $Q_h=e^{-t}(c_1\cos 2t+c_2\sen 2t)$. Particular constante: $Q_p=A \Rightarrow 5A=10 \Rightarrow A=2$. $Q(0)=c_1+2=0 \Rightarrow c_1=-2$; $Q'=e^{-t}[(-c_1+2c_2)\cos 2t+(-2c_1-c_2)\sen 2t]$, $Q'(0)=-c_1+2c_2=2+2c_2=0 \Rightarrow c_2=-1$. $\boxed{Q=2-e^{-t}(2\cos 2t+\sen 2t)}$
\end{myproof}
...
\begin{prob}
EDO de cuarto orden para viga con carga puntual: $EI y'''' = F\delta(x-a)$ (opcional). Se omite por brevedad, pero se plantea: integre con condiciones de borde.
\end{prob}
\begin{myproof}
Solución por funciones de Green; no se desarrolla. $\boxed{\text{Ver teoría de funciones de Green.}}$
\end{myproof}