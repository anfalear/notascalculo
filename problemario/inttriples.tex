...
\begin{definition}[Integral triple]\index{integral triple}
...
\begin{theorem}[Teorema de Fubini para integrales triples] \label{th:fubini3d}\index{Fubini, teorema de!integrales triples}
...
\begin{definition}[Masa por unidad de volumen]\index{densidad}
...
\begin{definition}[Centro de masa en 3D]\index{centro de masa!en el espacio}
...
\end{definition}
\begin{definition}[Momentos de inercia]\index{momento de inercia!en el espacio}
...
%%==========================================================
\begin{prob}[Centro de masa con densidad variable]
Un sólido ocupa la caja $D=[0,2]\times[0,1]\times[0,1]$ y tiene densidad
$\rho(x,y,z)=x$. Determine su masa y su centro de masa.
\begin{myproof}
\textbf{Paso 1. Masa.}
\[
M=\iiint_D x\,dV=\int_0^2\!\!\int_0^1\!\!\int_0^1 x\,dz\,dy\,dx
=\int_0^2 x\,dx=2.
\]

\textbf{Paso 2. Momento respecto al plano $yz$ (da $\bar x$).}
\[
M_{yz}=\iiint_D x\cdot\rho\,dV=\int_0^2\!\!\int_0^1\!\!\int_0^1 x^2\,dz\,dy\,dx
=\int_0^2 x^2\,dx=\frac83.
\]

\textbf{Paso 3. Momentos respecto a los planos $xz$ y $xy$.}
Como $\rho=x$ separa las variables,
\[
M_{xz}=\iiint_D y\,x\,dV=\Bigl(\int_0^2 x\,dx\Bigr)\Bigl(\int_0^1 y\,dy\Bigr)\Bigl(\int_0^1 dz\Bigr)=2\cdot\tfrac12\cdot1=1,
\]
\[
M_{xy}=\iiint_D z\,x\,dV=\Bigl(\int_0^2 x\,dx\Bigr)\Bigl(\int_0^1 dy\Bigr)\Bigl(\int_0^1 z\,dz\Bigr)=2\cdot1\cdot\tfrac12=1.
\]

\textbf{Paso 4. Centro de masa.}
\[
\bar x=\frac{M_{yz}}{M}=\frac{8/3}{2}=\frac43,\qquad
\bar y=\frac{M_{xz}}{M}=\frac12,\qquad
\bar z=\frac{M_{xy}}{M}=\frac12.
\]
\[
\boxed{\ (\bar x,\bar y,\bar z)=\Bigl(\tfrac43,\ \tfrac12,\ \tfrac12\Bigr).\ }
\]
El centro de masa está desplazado hacia $x$ grande respecto al centroide
geométrico $x=1$, como era de esperar, pues la densidad crece con $x$.
\end{myproof}
\end{prob}
...
%%==========================================================
\begin{prob}[Sólido cuyo techo cambia de superficie]
Calcule el volumen del sólido acotado por $z=\sqrt{x}$, $z=0$, los planos
$y=0$, $y=3$ y el plano $x+z=2$.
\begin{myproof}
\textbf{Paso 1. Analizar la sección en el plano $xz$.}
El sólido tiene grosor constante en $y$ (de $0$ a $3$), de modo que $V=3A$,
con $A$ el área de la región $R$ del plano $xz$ acotada por $z=\sqrt x$,
$z=0$ y $z=2-x$. Las curvas $z=\sqrt x$ y $z=2-x$ se cortan donde
\[
\sqrt x=2-x\ \Rightarrow\ x=(2-x)^2\ \Rightarrow\ x^2-5x+4=0\ \Rightarrow\ x=1
\]
(la raíz $x=4$ se descarta, pues daría $z=2-x=-2<0$); el corte es
$(x,z)=(1,1)$ (figura~\ref{fig:solido_techo}).

\textbf{Paso 2. Partir la región en $x$.}
El techo cambia de superficie en $x=1$: para $0\le x\le 1$ la tapa es
$z=\sqrt x$; para $1\le x\le 2$ es la recta $z=2-x$. Entonces
\[
A=\int_0^1\sqrt{x}\,dx+\int_1^2(2-x)\,dx.
\]

\textbf{Paso 3. Evaluar.}
\[
\int_0^1\sqrt x\,dx=\frac23,\qquad
\int_1^2(2-x)\,dx=\Bigl[2x-\tfrac{x^2}{2}\Bigr]_1^2=2-\tfrac32=\frac12,
\]
de modo que $A=\dfrac23+\dfrac12=\dfrac76$.

\textbf{Paso 4. Volumen.}
\[
V=3A=3\cdot\frac76=\boxed{\dfrac{7}{2}}.
\]
\end{myproof}
\end{prob}
...
\begin{tikzpicture}[scale=1.9]
  % region R
  \fill[blue!12] plot[domain=0:1,samples=40,variable=\x]({\x},{sqrt(\x)})
     -- plot[domain=1:2,samples=40,variable=\x]({\x},{2-\x}) -- (0,0) -- cycle;
  % ejes
  \draw[->,thin] (-0.2,0)--(2.4,0) node[right]{$x$};
  \draw[->,thin] (0,-0.2)--(0,1.4) node[above]{$z$};
  % curva z=sqrt(x)
  \draw[very thick,blue!70!black,domain=0:1,samples=40,smooth,variable=\x]
    plot ({\x},{sqrt(\x)});
  \node[blue!70!black] at (0.5,1.0) {$z=\sqrt{x}$};
  % recta z=2-x
  \draw[very thick,red!70!black] (1,1)--(2,0);
  \node[red!70!black] at (1.75,0.6) {$z=2-x$};
  % punto de corte
  \filldraw (1,1) circle(0.6pt);
  \draw[dashed,thin] (1,0)--(1,1);
  \node[below] at (1,-0.03) {$1$};
  \node[below] at (2,-0.03) {$2$};
  \node[blue!40!black] at (0.78,0.28) {$R$};
\end{tikzpicture}
\caption{Sección del sólido en el plano $xz$. El techo es $z=\sqrt x$ para
  $0\le x\le1$ y $z=2-x$ para $1\le x\le2$; ambas curvas se cortan en
  $(1,1)$. El volumen es el área de $R$ multiplicada por el grosor $3$ en $y$.}
\label{fig:solido_techo}
\end{figure}
...
\begin{definition}[Jacobiano en $\mathbb{R}^3$]\index{jacobiano!en $\mathbb{R}^3$}
...
\begin{theorem}[Cambio de variables en integrales triples]\index{cambio de variables!integrales triples}
...
\begin{definition}[Coordenadas cilíndricas]\index{coordenadas cilindricas@coordenadas cilíndricas}
...
\begin{theorem}[Integral triple en coordenadas cilíndricas]\index{coordenadas cilindricas@coordenadas cilíndricas!integral triple}
...
\begin{definition}[Coordenadas esféricas]\index{coordenadas esfericas@coordenadas esféricas}
...
\begin{tikzpicture}[scale=1.7]
  ...
  \draw[dashed,gray] (0.574,0.819)--(0.574,0)
    node[anchor=north,font=\scriptsize] {$\rho\sen\varphi$};
...
\begin{theorem}[Integral triple en coordenadas esféricas]\index{coordenadas esfericas@coordenadas esféricas!integral triple}
...
% [F9T.06 | origen: EX.09 | region entre esfera centrada y esfera desplazada, esfericas]
\begin{prob}
Use coordenadas esféricas para determinar el volumen del sólido que está dentro de la esfera $\rho=2$, fuera de la esfera $\rho=2\cos\varphi$ y sobre el plano $xy$.
\begin{myproof}
\textbf{Paso 1. Describir la región en esféricas.}
La condición ``sobre el plano $xy$'' da $\varphi\in[0,\pi/2]$. La superficie $\rho=2\cos\varphi$ es la esfera $x^2+y^2+(z-1)^2=1$ (multiplicando por $\rho$: $\rho^2=2\rho\cos\varphi$, es decir $x^2+y^2+z^2=2z$), que queda completamente en $z\ge0$. Para cada $\varphi\in[0,\pi/2]$, el radio varía entre la esfera interior y la exterior:
\[
2\cos\varphi\le\rho\le2,\qquad 0\le\varphi\le\frac{\pi}{2},\qquad 0\le\theta\le2\pi.
\]

\textbf{Paso 2. Plantear la integral.}
\[
V=\int_0^{2\pi}\!\int_0^{\pi/2}\!\int_{2\cos\varphi}^{2}\rho^2\sen\varphi\,d\rho\,d\varphi\,d\theta.
\]

\textbf{Paso 3. Integral interior en $\rho$.}
\[
\int_{2\cos\varphi}^{2}\rho^2\,d\rho
=\frac{\rho^3}{3}\Big|_{2\cos\varphi}^{2}
=\frac{8}{3}\left(1-\cos^3\varphi\right).
\]

\textbf{Paso 4. Integrales en $\varphi$ y $\theta$.}
\[
\int_0^{\pi/2}\frac{8}{3}\left(1-\cos^3\varphi\right)\sen\varphi\,d\varphi
=\frac{8}{3}\left[-\cos\varphi+\frac{\cos^4\varphi}{4}\right]_0^{\pi/2}
=\frac{8}{3}\left[0-\left(-1+\frac{1}{4}\right)\right]
=\frac{8}{3}\cdot\frac{3}{4}=2.
\]
\[
V=\int_0^{2\pi}2\,d\theta=4\pi.
\]
\textbf{Verificación:} la semibola de radio $2$ tiene volumen $\tfrac{16\pi}{3}$ y la esfera $\rho=2\cos\varphi$ (radio $1$) tiene volumen $\tfrac{4\pi}{3}$; la diferencia es $\tfrac{12\pi}{3}=4\pi$. $\checkmark$
\[
\boxed{V=4\pi.}
\]
\end{myproof}
...
\begin{tikzpicture}[scale=1.15]
    % Region sombreada: semidisco de radio 2 menos disco de radio 1 centrado en (0,1)
    \fill[blue!20, opacity=0.60] (2,0) arc (0:180:2) -- cycle;
    \fill[white] (0,1) circle (1);
    % Ejes
    \draw[thin,->] (-2.5,0) -- (2.6,0) node[right] {$x$};
    \draw[thin,->] (0,-0.4) -- (0,2.6) node[above] {$z$};
    % Esfera exterior rho=2 (semicirculo superior)
    \draw[thick, blue!70!black] (2,0) arc (0:180:2);
    \node[anchor=west, blue!70!black] at (1.55,1.55) {\small $\rho=2$};
    % Esfera interior rho=2cos(phi): circunferencia de radio 1 centrada en (0,1)
    \draw[thick, red!70!black] (0,1) circle (1);
    \node[anchor=west, red!70!black] at (0.25,1.35) {\small $\rho=2\cos\varphi$};
    % Etiqueta de region
    \node at (-1.3,0.55) {\small $\Omega$};
    % Marcas de eje
    \foreach \xv in {-2,-1,1,2}
        \draw[thin] (\xv,0.07)--(\xv,-0.07) node[below] {\small $\xv$};
    \foreach \zv in {1,2}
        \draw[thin] (0.07,\zv)--(-0.07,\zv) node[left] {\small $\zv$};
\end{tikzpicture}
\caption{Corte en el plano $xz$ del sólido $\Omega$: dentro de la esfera $\rho=2$ (azul), fuera de la esfera desplazada $\rho=2\cos\varphi$ (roja) y sobre el plano $xy$}
\label{fig:esfericas_esfera_desplazada}
\end{figure}
...
\begin{prob}[Volumen entre dos paraboloides elípticos]
Determine el volumen del sólido comprendido entre los paraboloides
$z=x^2+3y^2$ y $z=8-x^2-3y^2$. (Sugerencia: iguale las superficies para
hallar la proyección elíptica sobre el plano $xy$ y use el cambio
$x=2r\cos\theta$, $y=\tfrac{2}{\sqrt3}\,r\,\sen\theta$, cuyo jacobiano es
$\tfrac{4}{\sqrt3}\,r$.)
\end{prob}
