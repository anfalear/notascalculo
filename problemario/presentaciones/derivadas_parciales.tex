...
\setpie{Derivadas parciales --- Cálculo III}
\setbloques{6}
...
\title{Derivadas parciales}
\subtitle{Diferenciación de funciones de varias variables}
...
\begin{frame}{Agenda de la unidad}
  \begin{enumerate}
    ...
    \item \textbf{Gradiente} y derivada direccional \hfill{\small\color{gray60}(30 min)}
    \item \textbf{Superficies implícitas}: revisita del plano tangente \hfill{\small\color{gray60}(5 min)}
    \item \textbf{Optimización}: extremos libres y multiplicadores de Lagrange \hfill{\small\color{gray60}(45 min)}
  \end{enumerate}
  \vfill
  \begin{center}
...
\begin{frame}{Aplicación --- una sonda térmica en tres dimensiones}
  \begin{block}{Problema}
    {\small Una fuente puntual en el origen calienta una sala: la temperatura, en $^\circ$C con $x,y,z$ en metros, es
    $T(x,y,z)=\dfrac{200}{x^2+y^2+z^2+1}$. Una sonda está en $P(1,1,1)$.
    \textbf{(a)} \textquestiondown{}A qué razón cambia $T$ si avanza hacia $Q(3,2,1)$?}
  \end{block}
  \pause
  \ddtracker{1}
  \[
    \nabla T=-\frac{400\,\langle x,y,z\rangle}{\left(x^2+y^2+z^2+1\right)^2}.
  \]
  \pause
  En $P$ el denominador vale $x^2+y^2+z^2+1=4$, luego $T(P)=50^\circ$C y
  $\nabla T(1,1,1)=-\tfrac{400}{16}\langle 1,1,1\rangle=-25\,\langle 1,1,1\rangle$.
\end{frame}

\begin{frame}{La sonda --- rumbo a $Q$}
  \ddtracker{3}
  \[
    \overrightarrow{PQ}=\langle 2,1,0\rangle,\qquad
    \bigl\|\overrightarrow{PQ}\bigr\|=\sqrt5,\qquad
    \mathbf u=\frac{1}{\sqrt5}\langle 2,1,0\rangle.
  \]
  \pause
  \[
    D_{\mathbf u}T(P)=-25\,\langle 1,1,1\rangle\cdot\frac{1}{\sqrt5}\langle 2,1,0\rangle
    =-\frac{25(2+1+0)}{\sqrt5}=-\frac{75}{\sqrt5}=-15\sqrt5.
  \]
  \pause
  \[
    \boxed{D_{\mathbf u}T(1,1,1)=-15\sqrt5\approx-33.5\ \frac{^\circ\mathrm{C}}{\mathrm m}}
  \]
  \pause
  \begin{center}\small\color{gray60} El signo negativo es información física: por ese rumbo la sonda \textbf{se enfría}.\end{center}
\end{frame}

\begin{frame}{La sonda --- \textquestiondown{}hacia dónde conviene moverse?}
  \begin{columns}
    \begin{column}{0.56\textwidth}
      \small
      \textbf{(b) Calentamiento máximo.} Es la dirección de $\nabla T(P)$:
      \[
        \mathbf u=-\frac{1}{\sqrt3}\langle 1,1,1\rangle,
        \qquad
        \|\nabla T(P)\|=25\sqrt3\approx43.3\ \frac{^\circ\mathrm C}{\mathrm m}.
      \]
      \onslide<2->
      \begin{center}\color{tealx} Esa dirección apunta exactamente al origen: el gradiente \emph{localiza la fuente de calor}.\end{center}
      \onslide<3->
      \textbf{(c) Sin cambio.} $D_{\mathbf u}T=0$ exige $\mathbf u\perp\langle 1,1,1\rangle$: ya no son dos direcciones, sino todo un \textbf{plano} de ellas, tangente a la isoterma esférica
      \[
        x^2+y^2+z^2=3\qquad (T=50^\circ\mathrm C).
      \]
    \end{column}
    \begin{column}{0.42\textwidth}
      \onslide<1->
      ...
      \begin{tikzpicture}[scale=0.85]
        \foreach \r in {0.8,1.6,2.4}{\draw[thin,navy!50] (0,0) circle (\r);}
        \fill[brick] (0,0) circle (2.2pt);
        \node[brick,font=\scriptsize,anchor=north east] at (-0.05,-0.05) {fuente};
        \coordinate (P) at (1.697,1.697);
        \fill[navy] (P) circle (2.2pt);
        \node[navy,font=\scriptsize,anchor=south west] at (1.75,1.75) {$P$};
        \draw[-Stealth,very thick,brick] (P) -- (1.0,1.0);
        \node[brick,font=\scriptsize,anchor=east] at (0.95,1.30) {$\nabla T$};
        \draw[-Stealth,thick,tealx,dashed] (P) -- (1.06,2.33);
        \draw[-Stealth,thick,tealx,dashed] (P) -- (2.33,1.06);
        \node[tealx,font=\scriptsize,anchor=south] at (1.06,2.38) {$D_{\mathbf u}T=0$};
      \end{tikzpicture}\\[2pt]
      {\footnotesize\color{gray60} corte de las isotermas esféricas}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Revisita: el gradiente es el vector normal}
  En el Bloque 2 el plano tangente exigía tener $z$ \textbf{despejada}. Con el gradiente esa exigencia desaparece.
  \pause
  \vspace{4pt}
  \begin{block}{Definición y ecuaciones}
    Sea $F(x,y,z)=k$ con $F$ diferenciable en $P(x_0,y_0,z_0)$ y $\nabla F(P)\neq\mathbf 0$. Entonces $\nabla F(P)$ es \textbf{normal} a la superficie en $P$, y
    \[
      \textbf{plano tangente:}\quad
      \nabla F(P)\cdot\langle x-x_0,\,y-y_0,\,z-z_0\rangle=0,
    \]
    \[
      \textbf{recta normal:}\quad
      (x,y,z)=(x_0,y_0,z_0)+t\,\nabla F(P),\qquad t\in\mathbb R.
    \]
  \end{block}
  \pause
  \vspace{2pt}
  ...
  \pause
  \vspace{-2pt}
  \begin{center}\small\color{tealx} El mismo plano tangente de siempre, pero sin despejar ni elegir rama.\end{center}
\end{frame}

\begin{frame}{Tres superficies de un vistazo}
  \small
  ...
  \]
  \pause
  \begin{center}\small\color{gray60} Tres superficies, un solo procedimiento: escribir $F=k$, calcular $\nabla F$, evaluar en $P$.\end{center}
\end{frame}
...
\begin{frame}{Ejemplo --- $f(x,y)=x^3+y^3-3xy$}
  \pctracker{1}
  \[
  ...
  \]
  \pause
  Sustituyendo, $x=(x^2)^2=x^4$, o sea $x(x^3-1)=0$: hay \textbf{dos} puntos críticos, $(0,0)$ y $(1,1)$.
  \pause
  \[
  ...
  \begin{tabular}{cccl}
    ...
    \textbf{Punto} & $D$ & $f_{xx}$ & \textbf{Conclusión} \\
    \midrule
    $(0,0)$ & $-9<0$   & ---    & punto de silla, $f=0$ \\
    $(1,1)$ & $27>0$   & $6>0$  & mínimo local, $f=-1$ \\
    \bottomrule
  \end{tabular}
  \end{center}
  \pause
  \begin{center}\small\color{gray60} Aquí la sustitución da $x=x^4$: solo dos raíces reales, no tres.\end{center}
\end{frame}
...
\begin{frame}{Ejemplo --- $f(x,y)=x^2-2xy+2y$ sobre $R=[0,3]\times[0,2]$}
  \textbf{Paso 1 --- interior.}
  \[
    f_x=2x-2y=0,\qquad f_y=-2x+2=0
    \qquad\Longrightarrow\qquad (x,y)=(1,1),\quad f(1,1)=1.
  \]
  \par\pause\smallskip
  \textbf{Paso 2 --- frontera.} Cada lado reduce $f$ a una función de \emph{una} variable:
  \vspace{2pt}
  \begin{columns}[t]
    \begin{column}{0.48\textwidth}
      \small
      ...
      $L_1:\ y=0,\ 0\le x\le 3$
      \[ f=x^2\ \text{ creciente} \]
      candidatos $(0,0)$ y $(3,0)$.
      \pause
      \vspace{4pt}
      $L_2:\ x=3,\ 0\le y\le 2$
      \[ f=9-6y+2y=9-4y\ \text{ decreciente} \]
      candidatos $(3,0)$ y $(3,2)$.
    \end{column}
    \begin{column}{0.48\textwidth}
      \small
      \pause
      $L_3:\ y=2,\ 0\le x\le 3$
      \[ f=x^2-4x+4=(x-2)^2 \]
      candidatos $(2,2)$, $(0,2)$ y $(3,2)$.
      \pause
      \vspace{4pt}
      $L_4:\ x=0,\ 0\le y\le 2$
      \[ f=2y\ \text{ creciente} \]
      candidatos $(0,0)$ y $(0,2)$.
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{El rectángulo --- evaluar y comparar}
  \begin{center}
  \begin{tabular}{lcccccc}
    \toprule
    \textbf{Punto} & $(1,1)$ & $(0,0)$ & $(3,0)$ & $(3,2)$ & $(2,2)$ & $(0,2)$ \\
    \midrule
    $f$ & $1$ & $0$ & $9$ & $1$ & $0$ & $4$ \\
    \bottomrule
  \end{tabular}
  ...
  \vspace{4pt}
  \[
    \boxed{\max_R f=9\ \text{en}\ (3,0);\qquad \min_R f=0\ \text{en}\ (0,0)\ \text{y}\ (2,2)}
  \]
  \pause
  \vspace{2pt}
  \small
  \begin{itemize}
    \item El mínimo se alcanza en \textbf{dos} puntos distintos: nada obliga a que el extremo absoluto sea único.
    \pause
    \item El crítico interior $(1,1)$ no es ninguno de los dos --- de hecho $D=(2)(0)-(-2)^2=-4<0$: es un punto de silla.
    \pause
    \item Aquí ambos extremos viven en la frontera. Por eso el paso 2 nunca se puede omitir.
  \end{itemize}
\end{frame}
...
\begin{frame}{Dos advertencias sobre el método}
  \begin{columns}[t]
    \begin{column}{0.47\textwidth}
      \textbf{(1) La hipótesis $\nabla g\neq\mathbf 0$}\\[3pt]
      \small
      Minimizar $f=x^2+y^2$ sujeta a $g=x^2-y^3=0$.
      \pause
      \begin{itemize}
        \item La restricción es $y=x^{2/3}$: tiene una \textbf{cúspide} en el origen.
        \item El mínimo está en $(0,0)$, pues $f\ge0$ y $f(0,0)=0$.
        \item Pero $\nabla g=(2x,-3y^2)$ se anula ahí y el sistema se reduce a $0=0$.
      \end{itemize}
      \pause
      \begin{center}\color{brick} El método \textbf{no localiza} ese extremo.\end{center}
    \end{column}
    \begin{column}{0.47\textwidth}
      \pause
      \textbf{(2) Necesaria, no suficiente}\\[3pt]
      \small
      \textquestiondown{}Tiene extremos $f=y-x^5$ sujeta a $g=y-x^3=0$?
      \pause
      \begin{itemize}
        \item Sobre la restricción, $h(x)=x^3-x^5$; el origen \textbf{no} es extremo: $h>0$ a la derecha y $h<0$ a la izquierda.
        \item Y sin embargo, con $\nabla f=(-5x^4,1)$ y $\nabla g=(-3x^2,1)$, el sistema da $\lambda=1$ y $x=0$.
      \end{itemize}
      \pause
      \begin{center}\color{brick} $(0,0)$ es candidato y no es extremo.\end{center}
    \end{column}
  \end{columns}
  \vspace{2pt}
  \alerta{Verifique siempre $\nabla g\neq\mathbf 0$ --- y clasifique después los candidatos: por compacidad, por el hessiano limitado o mirando $f$ sobre la curva.}
\end{frame}

\begin{frame}{Ejemplo --- caja sin tapa de volumen máximo}
  \begin{block}{Problema}
    Se dispone de $12\ \mathrm{m}^2$ de lámina. \textquestiondown{}Cuál es el volumen máximo de una caja rectangular \textbf{sin tapa} que puede construirse sin desperdicio?
  \end{block}
  \pause
  \lgtracker{1}
  Con base $x\times y$ y altura $z$, todas positivas, el material es la base más cuatro paredes:
  \[
    f(x,y,z)=xyz,
    \qquad
...
\begin{frame}{La caja --- resolver el sistema}
  \lgtracker{3}
  Multiplicando la primera ecuación por $x$, la segunda por $y$ y la tercera por $z$, el lado izquierdo es $xyz$ en las tres:
  \[
    \lambda(xy+2xz)=\lambda(xy+2yz)=\lambda(2xz+2yz).
  \]
  \pause
  Si $\lambda=0$ el sistema forzaría $xyz=0$, imposible. Cancelando $\lambda$:
  \[
    2xz=2yz\implies x=y,
    \qquad
    xy=2yz\implies x=2z.
  ...
  \pause
  \lgtracker{4}
  Con $x=y=2z$ en la restricción: $4z^2+4z^2+4z^2=12z^2=12$, luego $z=1$.
  \pause
  \[
    \boxed{\text{caja de } 2\times2\times1\ \mathrm m,\qquad V_{\max}=4\ \mathrm m^3}
  \]
  \pause
  \begin{center}\small\color{gray60} La base resulta el \textbf{doble de ancha que alta}: sin tapa, la geometría óptima ya no es el cubo.\end{center}
\end{frame}

\begin{frame}{Los dos métodos juntos --- el disco cerrado}
  \begin{block}{Problema}
    Halle los extremos absolutos de $f(x,y)=x^2+2y^2-x$ sobre el disco $x^2+y^2\le1$. Es una región cerrada y acotada, pero de frontera \textbf{curva}: el interior se trata con el criterio $D$ y la frontera con multiplicadores.
  \end{block}
  \pause
  \textbf{Interior} ($x^2+y^2<1$):
  \[
    f_x=2x-1=0,\quad f_y=4y=0
    \implies \left(\tfrac12,0\right),\ \text{que sí está dentro}.
  \]
  \vspace{-4pt}
  \[
    D=f_{xx}f_{yy}-f_{xy}^2=(2)(4)-0^2=8>0,\quad f_{xx}=2>0
    \implies \text{mínimo local},\ f=-\tfrac14.
  \]
  \par\pause\smallskip
  \textbf{Frontera} ($g=x^2+y^2-1=0$), con $\nabla f=\lambda\nabla g$:
  \[
    \langle 2x-1,\ 4y\rangle=\lambda\langle 2x,\ 2y\rangle
    \implies
    \begin{cases} 2x-1=2\lambda x,\\ 4y=2\lambda y.\end{cases}
  \]
\end{frame}

\begin{frame}{El disco --- las dos ramas de la frontera}
  La segunda ecuación, $4y=2\lambda y$, se factoriza como $2y(2-\lambda)=0$: o bien $y=0$, o bien $\lambda=2$.
  \par\pause\smallskip
  \textbf{Rama $y=0$.} La restricción da $x=\pm1$, de modo que
  $f(1,0)=0$ y $f(-1,0)=2$.
  \par\pause\smallskip
  \textbf{Rama $\lambda=2$.} Entonces $2x-1=4x$, luego $x=-\tfrac12$ y $y^2=1-\tfrac14=\tfrac34$, de donde
  \[
    f\!\left(-\tfrac12,\pm\tfrac{\sqrt3}{2}\right)=\tfrac14+2\cdot\tfrac34+\tfrac12=\tfrac94.
  \]
  \pause
  \begin{center}\small
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Punto} & $\left(\frac12,0\right)$ & $(1,0)$ & $(-1,0)$ & $\left(-\frac12,\pm\frac{\sqrt3}{2}\right)$ \\
    \midrule
    $f$ & $-\frac14$ & $0$ & $2$ & $\frac94$ \\
    \bottomrule
  \end{tabular}
  ...
  \vspace{-4pt}
  \[
    \boxed{\max f=\tfrac94\ \text{en}\ \left(-\tfrac12,\pm\tfrac{\sqrt3}{2}\right);
           \qquad \min f=-\tfrac14\ \text{en}\ \left(\tfrac12,0\right)}
  \]
\end{frame}
