...
\begin{theorem}[Principio unificador de las aplicaciones de la integral]\index{aplicaciones de la integral!principio unificador}
...
\begin{definition}[Área bajo una curva]\index{area@área!bajo una curva}
...
La Figura~\ref{fig:area_con_signo} distingue ambas regiones.
...
\end{axis}
\end{tikzpicture}
\caption{Región limitada por la curva y el eje $x$ en $[-1,1]$. El tramo situado sobre el eje (azul) aporta área positiva y el situado bajo el eje (rojo) aporta área negativa, de modo que el área geométrica exige integrar el valor absoluto en cada subintervalo.}
\label{fig:area_con_signo}
\end{figure}
...
\begin{theorem}[Área entre dos curvas]\index{area@área!entre dos curvas}
...
La Figura~\ref{fig:area_parabola_recta} muestra la región.
...
\end{axis}
\end{tikzpicture}
\caption{Región acotada por la parábola $y=x^2$ y la recta $y=x+2$. Las curvas se cortan en $x=-1$ y $x=2$, y en todo ese intervalo la recta queda por encima de la parábola.}
\label{fig:area_parabola_recta}
\end{figure}
...
\begin{example}
Halle el área de la región acotada por $f(x)=\sen x$ y $g(x)=\cos x$ en
$[0,2\pi]$.
...
Analizando signos:
\begin{itemize}
  \item En $\bigl[0,\tfrac{\pi}{4}\bigr]$: $\cos x\geq\sen x$.
  \item En $\bigl[\tfrac{\pi}{4},\tfrac{5\pi}{4}\bigr]$: $\sen x\geq\cos x$.
  \item En $\bigl[\tfrac{5\pi}{4},2\pi\bigr]$: $\cos x\geq\sen x$.
\end{itemize}
La Figura~\ref{fig:area_seno_coseno} muestra los tres tramos.
...
\addplot[domain=0:6.2832, samples=300, blue!80!black, thick]
        {sin(deg(x))}
        node[pos=0.28, above, font=\small] {$\sen x$};
...
\end{axis}
\end{tikzpicture}
\caption{Región comprendida entre $y=\sen x$ y $y=\cos x$ en $[0,2\pi]$. La curva superior cambia en $x=\pi/4$ y $x=5\pi/4$, lo que obliga a partir el intervalo en tres tramos.}
\label{fig:area_seno_coseno}
\end{figure}
\textbf{Paso 2: Elemento diferencial.}
$dA = |\sen x - \cos x|\,dx$, dividido en tres tramos.
...
\textbf{Paso 4: Evaluación.}
\begin{align*}
  A &= \int_0^{\pi/4}(\cos x-\sen x)\,dx
     + \int_{\pi/4}^{5\pi/4}(\sen x-\cos x)\,dx
     + \int_{5\pi/4}^{2\pi}(\cos x-\sen x)\,dx \\
    &= \bigl[\sen x+\cos x\bigr]_0^{\pi/4}
     + \bigl[-\cos x-\sen x\bigr]_{\pi/4}^{5\pi/4}
     + \bigl[\sen x+\cos x\bigr]_{5\pi/4}^{2\pi} \\
...
\begin{definition}\index{area@área!por integracion respecto a $y$@por integración respecto a $y$}
...
La Figura~\ref{fig:area_respecto_y} muestra la región y el sentido de integración.
...
\end{axis}
\end{tikzpicture}
\caption{Región acotada por la parábola $x=y^2$ y la recta $x=y+2$. Integrando respecto a $y$ en $[-1,2]$ la región es de un solo tipo y basta una integral.}
\label{fig:area_respecto_y}
\end{figure}
\textbf{Paso 2: Elemento diferencial.}
...
con $y\in[0,2]$.
La Figura~\ref{fig:area_raiz_recta} muestra la región.
...
\end{axis}
\end{tikzpicture}
\caption{Región acotada por $y=\sqrt{x}$, la recta $y=x-2$ y el eje $x$. Al integrar respecto a $y$ en $[0,2]$ se evita partir el cálculo en dos integrales.}
\label{fig:area_raiz_recta}
\end{figure}
\textbf{Paso 2: Elemento diferencial.}
...
\begin{theorem}[Volumen por discos respecto al eje $x$]\index{volumen!metodo de los discos@método de los discos}
...
La Figura~\ref{fig:cono_discos} ilustra la generatriz y un disco genérico.
...
\end{axis}
\end{tikzpicture}
\caption{Generación del cono de radio $R$ y altura $h$ al girar la recta $y=\frac{R}{h}x$ alrededor del eje $x$. Cada corte perpendicular al eje es un disco de radio $r(x)=\frac{R}{h}x$.}
\label{fig:cono_discos}
\end{figure}
...
La Figura~\ref{fig:esfera_discos} muestra la semicircunferencia generatriz.
...
\end{axis}
\end{tikzpicture}
\caption{Generación de la esfera de radio $R$ al girar la semicircunferencia $y=\sqrt{R^2-x^2}$ alrededor del eje $x$. El disco en la posición $x$ tiene radio $\sqrt{R^2-x^2}$.}
\label{fig:esfera_discos}
\end{figure}
...
\begin{theorem}[Volumen por arandelas]\index{volumen!metodo de las arandelas@método de las arandelas}
...
La Figura~\ref{fig:arandelas_raiz_cuadrado} muestra la región y los dos radios.
...
\end{axis}
\end{tikzpicture}
\caption{Región entre $y=\sqrt{x}$ y $y=x^2$ en $[0,1]$. Al girar alrededor del eje $x$, cada corte es una arandela de radio externo $R(x)=\sqrt{x}$ y radio interno $r(x)=x^2$.}
\label{fig:arandelas_raiz_cuadrado}
\end{figure}
\textbf{Paso 2: Elemento diferencial.}
...
La Figura~\ref{fig:arandelas_eje_desplazado} muestra el eje desplazado.
...
\end{axis}
\end{tikzpicture}
\caption{Giro alrededor de la recta $y=2$, situada por encima de la región. Los radios se miden como distancias al eje de rotación, no al eje $x$.}
\label{fig:arandelas_eje_desplazado}
\end{figure}
...
\begin{theorem}[Volumen por cascarones girando respecto al eje $y$]\index{volumen!metodo de los cascarones@método de los cascarones}
...
\begin{myproof}
La Figura~\ref{fig:cascarones_raiz} muestra la franja generadora.
...
\end{axis}
\end{tikzpicture}
\caption{Región bajo $y=\sqrt{x}$ en $[0,4]$ girada alrededor del eje $y$. Cada franja vertical genera un cascarón cilíndrico de radio $x$ y altura $\sqrt{x}$.}
\label{fig:cascarones_raiz}
\end{figure}
...
La Figura~\ref{fig:cascarones_eje_x5} muestra el radio $5-x$.
...
\end{axis}
\end{tikzpicture}
\caption{Giro alrededor de la recta vertical $x=5$, situada a la derecha de la región. La franja en $x$ genera un cascarón de radio $r(x)=5-x$ y altura $h(x)=4x-x^2$.}
\label{fig:cascarones_eje_x5}
\end{figure}
...
\begin{definition}[Longitud de una curva suave]\index{longitud de arco!curva suave}
...
La Figura~\ref{fig:longitud_parabola} muestra el arco y el elemento $ds$.
...
\end{axis}
\end{tikzpicture}
\caption{Arco de parábola cuya longitud se calcula con $ds=\sqrt{1+[f'(x)]^2}\,dx$: el elemento de arco es la hipotenusa del triángulo diferencial de catetos $dx$ y $dy$.}
\label{fig:longitud_parabola}
\end{figure}
...
La Figura~\ref{fig:longitud_catenaria} muestra la curva.
...
\end{axis}
\end{tikzpicture}
\caption{Catenaria $y=\cosh x$, simétrica respecto al eje $y$. Sus propiedades hiperbólicas hacen que el radicando $1+(y')^2$ sea un cuadrado perfecto.}
\label{fig:longitud_catenaria}
\end{figure}
...
\begin{definition}[Área superficial de revolución respecto al eje $x$]\index{area superficial de revolucion@área superficial de revolución}
...
La Figura~\ref{fig:superficie_esfera} muestra la banda elemental.
...
\end{axis}
\end{tikzpicture}
\caption{Superficie esférica generada al girar la semicircunferencia $y=\sqrt{R^2-x^2}$ alrededor del eje $x$. La banda elemental tiene área $dS=2\pi y\,ds$.}
\label{fig:superficie_esfera}
\end{figure}
...
\begin{theorem}[Superficie alrededor del eje $y$]\index{area superficial de revolucion@área superficial de revolución!eje $y$}
...
La Figura~\ref{fig:superficie_paraboloide} muestra el radio de giro.
...
\end{axis}
\end{tikzpicture}
\caption{Superficie generada al girar la parábola $y=x^2$ alrededor del eje $y$. El radio de giro es la distancia horizontal al eje, $r(x)=x$.}
\label{fig:superficie_paraboloide}
\end{figure}
...
\begin{definition}[Trabajo de una fuerza variable]\index{trabajo}
...
La Figura~\ref{fig:trabajo_resorte} muestra esa área.
...
\end{axis}
\end{tikzpicture}
\caption{Trabajo como área bajo la recta $F(x)=100x$ en $[0,\,0.3]$. Al ser la fuerza proporcional al estiramiento, el área es la de un triángulo.}
\label{fig:trabajo_resorte}
\end{figure}
...
La Figura~\ref{fig:trabajo_resorte_adicional} separa ambas áreas.
...
\end{axis}
\end{tikzpicture}
\caption{Trabajo adicional (azul) para estirar el resorte de $x=0.05$ a $x=0.20$ m. El área gris corresponde al trabajo ya realizado y no se contabiliza.}
\label{fig:trabajo_resorte_adicional}
\end{figure}
...
La Figura~\ref{fig:trabajo_tanque_conico} muestra la rebanada genérica.
...
\end{tikzpicture}
\caption{Tanque cónico invertido: la rebanada a altura $y$ tiene radio $r(y)=y/2$ y debe elevarse la distancia $6-y$ hasta el borde.}
\label{fig:trabajo_tanque_conico}
\end{figure}
...
\begin{align*}
  W &= \frac{9800\pi}{4}\int_0^6(6y^2-y^3)\,dy
    ...
    &= 2450\pi\!\left(2\cdot216 - 324\right)
     = 2450\pi(108)
     = \boxed{264600\pi\approx 8.31\times10^5\,\text{J}}.
\end{align*}
\end{myproof}
\end{example}
...
La Figura~\ref{fig:trabajo_tanque_cilindrico} muestra la rebanada genérica.
...
\begin{tikzpicture}[scale=0.78]
  ...
  \node[below left, font=\small] at (0,0) {$O$};
\end{tikzpicture}
\caption{Tanque cilíndrico: a diferencia del cónico, la sección transversal es constante ($A=4\pi\,\text{m}^2$) y solo varía la distancia de elevación $5-y$.}
\label{fig:trabajo_tanque_cilindrico}
\end{figure}
...
\[
  W = \int_0^5 39200\pi(5-y)\,dy
    ...
    = \boxed{490000\pi\approx 1.54\times10^6\,\text{J}}.
\]
\end{myproof}
\end{example}
...
\begin{definition}[Momentos y centro de masa de una lámina plana]\index{centro de masa!lamina plana@lámina plana}\index{momentos de una lamina@momentos de una lámina}
...
\begin{theorem}[Teorema de Pappus para el volumen]\index{Pappus, teorema de}
...
$(0,0)$, $(1,0)$ y $(0,1)$.
La Figura~\ref{fig:centroide_triangulo} muestra la región.
...
\end{axis}
\end{tikzpicture}
\caption{Región triangular de vértices $(0,0)$, $(1,0)$ y $(0,1)$ con densidad constante. El centroide es el punto de equilibrio de la lámina.}
\label{fig:centroide_triangulo}
\end{figure}
...
La Figura~\ref{fig:centroide_semicirculo} muestra la simetría.
...
\end{axis}
\end{tikzpicture}
\caption{Lámina semicircular de radio $R$. La simetría respecto al eje $y$ obliga a $\bar x=0$, de modo que solo hay que calcular $\bar y$.}
\label{fig:centroide_semicirculo}
\end{figure}
...
\begin{definition}[Momento de inercia de una lámina]\index{momento de inercia}
...
La Figura~\ref{fig:inercia_varilla_extremo} muestra la disposición.
...
\end{axis}
\end{tikzpicture}
\caption{Varilla homogénea de longitud $L$ girando respecto a un eje que pasa por uno de sus extremos ($x=0$).}
\label{fig:inercia_varilla_extremo}
\end{figure}
...
La Figura~\ref{fig:inercia_varilla_centro} muestra la nueva disposición.
...
\end{axis}
\end{tikzpicture}
\caption{Misma varilla girando respecto a un eje que pasa por su centro: al situar el origen en el punto medio, la integral se extiende a $[-L/2,\,L/2]$ y el momento de inercia resulta menor.}
\label{fig:inercia_varilla_centro}
\end{figure}
...
\begin{definition}[Fuerza hidrostática sobre una placa plana]\index{fuerza hidrostatica@fuerza hidrostática}
...
constante $w=4\,\text{m}$.
La Figura~\ref{fig:fuerza_compuerta} muestra la compuerta y una franja.
...
\end{axis}
\end{tikzpicture}
\caption{Compuerta rectangular sumergida entre las profundidades $h=2$ m y $h=5$ m. La presión crece linealmente con la profundidad, de modo que la fuerza se obtiene integrando franjas horizontales.}
\label{fig:fuerza_compuerta}
\end{figure}
...
La Figura~\ref{fig:prob_region_recta_parabola} muestra la región.
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{Región $R$ limitada por $y=x+6$, $y=-x^2-6x-6$ y $x=0$.}
\label{fig:prob_region_recta_parabola}
\end{figure}
\end{prob}
...
\begin{prob}
Calcule el área de la región encerrada entre $y = \sen x$ y $y = \frac{1}{2}$ para $0 \leq x \leq \frac{17\pi}{6}$.
\end{prob}

\subsection{Volúmenes de sólidos de revolución}
...
La Figura~\ref{fig:prob_volumen_discos} muestra la región.
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{Región encerrada por $y=2x+4$, $y=-2x^2-4x$ y $x=0$, que se hace girar alrededor del eje $x$.}
\label{fig:prob_volumen_discos}
\end{figure}
\end{prob}
...
\subsection{Longitud de arco y superficies de revolución}
...
\subsection{Centro de masa, centroide y momento de inercia}
...
\subsection{Presión y fuerza hidrostática}
...
% Banco extendido (fuera de la cuota de 40)
